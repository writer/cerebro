...
const defaultOutputDir = "internal/findings"
const generatedFileBase = "policy_rule_catalog"
const generatedFileSuffix = "_gen.go"
const defaultOutputPath = defaultOutputDir + "/" + generatedFileBase + generatedFileSuffix
...
func main() {
	...
	if *write {
		if err := removeStaleGeneratedFiles(cleanRoot, files); err != nil {
			fmt.Fprintf(os.Stderr, "policyrulegen: cleanup: %v\n", err)
			os.Exit(1)
		}
		for _, file := range files {
			if err := writeRepoFile(cleanRoot, file.rel, file.content); err != nil {
	...
	if *check {
		stale, err := checkStaleGeneratedFiles(cleanRoot, files)
		if err != nil {
			fmt.Fprintf(os.Stderr, "policyrulegen: %v\n", err)
			os.Exit(1)
		}
		...
		for _, file := range files {
			existing, err := readRepoFile(cleanRoot, file.rel)
			if err != nil {
				fmt.Fprintf(os.Stderr, "policyrulegen: read %s: %v\n", file.rel, err)
				os.Exit(1)
			}
			if !bytes.Equal(bytes.TrimSpace(existing), bytes.TrimSpace(file.content)) {
...
func generateFiles(root string) ([]generatedFile, error) {
	...
	domainPolicies := map[string][]policyFile{}
	for i := range policies {
		d := policies[i].domain
		domainPolicies[d] = append(domainPolicies[d], policies[i])
	}

	domains := make([]string, 0, len(domainPolicies))
	for d := range domainPolicies {
	...
	for _, domain := range domains {
		varName := "generated" + domainGoIdent(domain) + "Rules"
		rel := defaultOutputDir + "/" + generatedFileBase + "_" + domainFileSlug(domain) + generatedFileSuffix

		var rulesBuf bytes.Buffer
		for _, policy := range domainPolicies[domain] {
			ext := extensions.extensionFor(policy)
			writePolicyRuleConfig(&rulesBuf, policy, ext, controlFamilies.familiesFor(policy.Frameworks))
		}

		var buf bytes.Buffer
		buf.WriteString("// Code generated by go run ./tools/policyrulegen --write; DO NOT EDIT.\n")
		buf.WriteString("package findings\n\n")
		if strings.Contains(rulesBuf.String(), "ports.") {
			buf.WriteString("import \"github.com/writer/cerebro/internal/ports\"\n\n")
		}
		fmt.Fprintf(&buf, "var %s = []policyRuleConfig{\n", varName)
		buf.Write(rulesBuf.Bytes())
		buf.WriteString("}\n")

		formatted, err := format.Source(buf.Bytes())
		if err != nil {
			return nil, fmt.Errorf("format generated Go for domain %s: %w", domain, err)
		}
		files = append(files, generatedFile{rel: rel, content: formatted})
	}

	var mainBuf bytes.Buffer
	mainBuf.WriteString("// Code generated by go run ./tools/policyrulegen --write; DO NOT EDIT.\n")
	mainBuf.WriteString("package findings\n\n")
	fmt.Fprintf(&mainBuf, "var generatedPolicyRuleCatalog = func() []policyRuleConfig {\n")
	fmt.Fprintf(&mainBuf, "var all []policyRuleConfig\n")
	for _, domain := range domains {
		varName := "generated" + domainGoIdent(domain) + "Rules"
		fmt.Fprintf(&mainBuf, "all = append(all, %s...)\n", varName)
	}
	mainBuf.WriteString("return all\n")
	mainBuf.WriteString("}()\n")

	formatted, err := format.Source(mainBuf.Bytes())
	if err != nil {
		return nil, fmt.Errorf("format generated Go main: %w", err)
	}
	files = append(files, generatedFile{rel: defaultOutputPath, content: formatted})
...
func policyContractAttributes(policy policyFile) map[string]string {
	...
	addContractList(attributes, "policy_evidence_requirement_refs", policy.Evidence.RequirementRefs)
...
func domainGoIdent(domain string) string {
...
func domainFileSlug(domain string) string {
	return strings.ReplaceAll(domain, "-", "_")
}

func removeStaleGeneratedFiles(root string, expected []generatedFile) error {
	stale, err := findStaleGeneratedFiles(root, expected)
	if err != nil {
		return err
	}
	repoRoot, err := os.OpenRoot(root)
	...
	for _, rel := range stale {
		if err := repoRoot.Remove(rel); err != nil && !os.IsNotExist(err) {
...
func checkStaleGeneratedFiles(root string, expected []generatedFile) (bool, error) {
	stale, err := findStaleGeneratedFiles(root, expected)
	if err != nil {
		return false, err
	}
	for _, rel := range stale {
		fmt.Fprintf(os.Stderr, "policyrulegen: stale generated file %s should be removed; run `make policy-rule-generate`\n", rel)
	}
	return len(stale) > 0, nil
}

func findStaleGeneratedFiles(root string, expected []generatedFile) ([]string, error) {
	expectedSet := make(map[string]struct{}, len(expected))
	for _, f := range expected {
		expectedSet[f.rel] = struct{}{}
	}
	dirPath := filepath.Join(root, defaultOutputDir)
	entries, err := os.ReadDir(dirPath)
	if err != nil {
		return nil, fmt.Errorf("read %s: %w", defaultOutputDir, err)
	}
	prefix := generatedFileBase + "_"
	var stale []string
	for _, entry := range entries {
		name := entry.Name()
		if !strings.HasPrefix(name, prefix) || !strings.HasSuffix(name, generatedFileSuffix) {
			continue
		}
		rel := defaultOutputDir + "/" + name
