...
func hasEveryFamilyTest(path string, familyNames []string) bool {
	...
	for _, marker := range []string{
		"TestCatalogFixtureReplaysGeneratedFamilies",
...
func generatedScaffold(sourceRoot string) bool {
	sourceTest := filepath.Join(sourceRoot, "source_test.go")
	return (hasGenericRecordTest(sourceTest) || fileContains(sourceTest, "TestCatalogFixtureReplaysGeneratedFamilies")) &&
		fileContains(filepath.Join(sourceRoot, "source.go"), "jsonapi.New")
}

func evidenceFiles(sourceRoot string) []string {
	candidates := []string{"catalog.yaml", "deploy.yaml", "source.go", "source_test.go"}
	files := []string{}
	...
	fixturePath := filepath.Join(sourceRoot, "fixture.go")
	if fileExists(fixturePath) && !fileContains(fixturePath, "Code generated by sourcegen") {
		files = append(files, filepath.ToSlash(fixturePath))
	}
