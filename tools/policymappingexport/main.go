...
func loadControlEvidenceRequirements(root string) (controlEvidenceRequirementCatalog, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return controlEvidenceRequirementCatalog{}, nil
		}
...
func logicRows() [][]string {
	return [][]string{
		...
		{"15", "coverage candidates", "Create author review rows for controls that need source backing, source-backed findings, mapping review, or scope decisions before coverage can be claimed."},
		{"16", "control evidence requirements", "Expand first-class evidence requirements from internal/compliance/control_evidence_requirements.yaml across every framework control, then join them with source capabilities and catalog evidence expectations."},
