...
impl PostgresAgentSessionStore {
    ...
    pub async fn mark_wake_awaiting_delivery(
        &self,
        ...
        let changed = self
            ...
            .await
            .execute(
                "UPDATE cerebro_agent_wakes SET state = 'awaiting_delivery', pending_payload_digest = $7, updated_at = NOW() WHERE session_ref = $1 AND commitment_ref = $2 AND schedule_generation = $3 AND fence = $4 AND lease_owner = $5 AND lease_token = $6 AND request_id = $8 AND state = 'leased' AND lease_expires_at > NOW()",
    ...
    pub async fn append_wake_fenced(
        ...
        self.append_checked(&claim.session_ref, expected_sequence, events, Some(claim))
            .await
    }
...
impl SessionStore for PostgresAgentSessionStore {
    ...
    async fn append(
        ...
        self.append_checked(session_ref, expected_sequence, events, None)
            .await
    }
}
...
async fn project_session_state(
    ...
    transaction
        .execute(
            "UPDATE cerebro_agent_wakes SET state = 'cancelled', lease_owner = NULL, lease_token = NULL, lease_expires_at = NULL, pending_payload_digest = NULL, updated_at = NOW() WHERE session_ref = $1 AND state IN ('scheduled', 'leased', 'awaiting_delivery', 'failed') AND NOT (commitment_ref = ANY($2))",
...
mod tests {
    ...
    async fn postgres_wake_claim_is_fenced_until_exact_delivery() {
        ...
        assert_eq!(u64::try_from(row.get::<_, i64>(2)).unwrap(), claim.fence);
        store
            .client
            .lock()
            .await
