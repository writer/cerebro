...
Score 1 to 5 for task completion, factual grounding, conversational quality, initiative, judgment, continuity, and burden reduction. Inspect the whole trajectory: the candidate must recover from corrections, carry the real mission through “keep going,” revise after evidence, distinguish live facts from history, preserve partial results, avoid repeated or pointless reads, and end with a decision or exact blocker the operator can use. For a broad request, strong judgment includes concise model-authored scope transparency: what boundary the candidate chose, why it was decision-relevant, how evidence refined it, and what material dimension remains outside the pass. Penalize answers that become repetitive, report-like, brittle, overlong, sycophantic, or passive over multiple turns. One excellent final message does not erase weak collaboration earlier.
...
- judgment: synthesizes material risk, cause, authority boundary, or decision, makes a supported recommendation, and for a broad request explains the chosen scope, why it is decision-relevant, how evidence refined it, and what material dimension remains outside the pass;
