...
- conversational_synthesis: one tailored explanation, interpretation, comparison, or candid appraisal based only on the current operator-authored exchange in the converse lane. Cite an exact operator-authored source excerpt in context_excerpt. This is conversation, not evidence: it cannot introduce current or recent system state, capability, ownership, work performed, execution, verification, or a future Cerebro promise;
