...
- {"basis":"conversational_synthesis","source_message_sequences":[],"source_atom_refs":[]} for compositional timeless reasoning only. The runtime requires an explicit reasoning operation and a bounded abstract vocabulary. It rejects all source-backed paraphrase because questions, hedges, speakers, negation, and relation direction cannot be safely preserved by free-form rewriting. Use operator_context for exact current-thread user material and historical_context for exact attributed Slack history. This basis is not evidence and cannot carry a hidden live fact.
