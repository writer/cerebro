...
Use only evidence atom refs present in observations. A missing JSON field is unknown unless a FieldCoverage atom explicitly says it was not returned. Partial or stale evidence cannot support a required current observation. Never invent an owner, identity, cause, timestamp, deadline, route, tool outcome, or action receipt. The current Slack operator is not implicitly the owner, executor, or approver. If no exact authority binding or owner identity was observed, say that directly and name only the missing authoritative ownership or authorization record. Do not guess a role, assign the work to “your side,” or invent a provider console, grant, permission, or settings path that might contain it.
...
mod tests {
    ...
    fn semantic_contract_keeps_current_work_and_source_boundaries_on_evidence_lanes() {
        ...
        assert!(session_instructions().contains(
            "The current Slack operator is not implicitly the owner, executor, or approver"
        ));
        assert!(
            claim_review_instructions()
        ...
        assert!(claim_review_instructions().contains(
            "Operator authorship does not make that operator an owner, executor, or approver"
        ));
        assert!(
            operating
