...
use super::{
    AddigyFamily,
    source_execution::{ADDIGY_SOURCE_EXECUTION_ADAPTERS, AddigySourceExecutionAdapter},
};
...
fn adapter(family: AddigyFamily) -> &'static AddigySourceExecutionAdapter {
    ADDIGY_SOURCE_EXECUTION_ADAPTERS
        .iter()
        .find(|adapter| adapter.family_id() == family.as_str())
        .unwrap()
}

fn fixture(family: AddigyFamily) -> Value {
...
fn response(family: AddigyFamily, records: Vec<Value>, page: u32) -> Vec<u8> {
    let response = if family.root_array() {
        Value::Array(records)
    } else {
...
fn context(cursor: &str, page: u32) -> SourceWorkerExecutionContextV1 {
    SourceWorkerExecutionContextV1 {
        ...
        runtime_id: "addigy-runtime".to_owned(),
        logical_page_id: format!("source-page-v2:addigy-{page}"),
...
fn metadata(family: AddigyFamily) -> SourceWorkerRuntimeMetadataV2 {
    SourceWorkerRuntimeMetadataV2 {
        public_config: HashMap::from([
            ("family".to_owned(), family.as_str().to_owned()),
            ("organization_id".to_owned(), "org-1".to_owned()),
        ]),
...
fn plan_page(
    adapter: &AddigySourceExecutionAdapter,
...
#[test]
fn addigy_plans_all_five_families_without_credentials() {
    for family in AddigyFamily::ALL {
        let adapter = adapter(family);
        let plan = adapter.compiled_plan();
        assert_eq!(plan.source_id, "addigy");
        assert_eq!(plan.family_id, family.as_str());
        assert_eq!(plan.provider_kernel, family.event_kind());
        assert_eq!(plan.origin, "https://api.addigy.com/api/v2");
        assert_eq!(
            plan.path,
            format!("/api/v2{}", family.path(Some("{organization_id}")).unwrap())
        );
        ...
        let execution = plan_page(adapter, &plan, &context, &metadata);
        assert_eq!(execution.credential_operation, "source.x_api_key");
        assert_eq!(execution.allowed_origin, "https://api.addigy.com");
        assert_eq!(execution.body, b"{}".to_vec());
        assert_eq!(
            execution.declared_headers,
        ...
        let suffix = family.path(Some("org-1")).unwrap();
        let query = if family.paginated() {
            "?per_page=100&page=1"
        } else {
            ""
        };
        let expected_url = format!("https://api.addigy.com/api/v2{suffix}{query}");
...
#[test]
fn addigy_decodes_every_fixture_with_exact_contract_and_seals() {
    for family in AddigyFamily::ALL {
        let adapter = adapter(family);
        let plan = adapter.compiled_plan();
        let context = context("", 1);
        let metadata = metadata(family);
        let execution = plan_page(adapter, &plan, &context, &metadata);
        let body = response(family, vec![fixture(family)], 1);
...
#[test]
fn addigy_pagination_and_provider_failures_are_typed() {
    let paged_family = AddigyFamily::Devices;
    ...
    let body = response(
        ...
        (1..=100)
            .map(|number| {
                ...
                record["agentid"] = Value::String(format!("device-{number}"));
                record
            })
            .collect(),
        1,
...
#[test]
fn addigy_rejects_bad_scope_cursor_duplicates_and_secret_material() {
    ...
    let group_adapter = ADDIGY_SOURCE_EXECUTION_ADAPTERS
        .iter()
        .find(|candidate| candidate.family_id() == AddigyFamily::Groups.as_str())
        .unwrap();
    let group_plan = group_adapter.compiled_plan();
    let mut missing_organization = metadata.clone();
    missing_organization
        .public_config
        .insert("family".to_owned(), "groups".to_owned());
    missing_organization.public_config.remove("organization_id");
    assert_eq!(
        group_adapter.plan_v2(&SourceWorkerPlanEnvelopeV2 {
            request: Some(SourceWorkerPlanRequestV1 {
                plan: Some(group_plan),
                context: Some(context("", 1)),
            }),
            metadata: Some(missing_organization),
        }),
    ...
    let duplicate_body = response(family, vec![original.clone(), original.clone()], 1);
    let result = decode_page(
        adapter,
        &plan,
        &execution_context,
    ...
    conflicting["audit_date"] = Value::from("2026-06-02T00:00:00Z");
    let conflicting_body = response(family, vec![original.clone(), conflicting], 1);
    assert_eq!(
        decode_page(
            adapter,
            &plan,
    ...
    let secret_body = response(family, vec![secret], 1);
    ...
    let untrusted_scope_body = response(family, vec![untrusted_scope], 1);
    assert_eq!(
        decode_page(
            adapter,
            &plan,
