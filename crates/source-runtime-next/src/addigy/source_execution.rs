//! Addigy bridge for the closed source-execution protocol.
//!
...
use super::{
    AddigyError, AddigyFamily, AddigyKernel, AddigyRuntimeDefinition, normalize::event_id,
};

const SOURCE_ID: &str = "addigy";
const DEFAULT_BASE_URL: &str = "https://api.addigy.com/api/v2";
const METHOD: &str = "POST";
const ACCEPT: &str = "application/json";
const MAX_RESPONSE_BYTES: u64 = 8 << 20;

/// Credential-free adapter for one cataloged Addigy family.
#[derive(Clone, Copy, Debug)]
pub(crate) struct AddigySourceExecutionAdapter {
    family: AddigyFamily,
}

/// Provider-local adapter set. Shared dispatcher registration and authority
/// promotion remain separate delivery steps.
pub(crate) static ADDIGY_SOURCE_EXECUTION_ADAPTERS: [AddigySourceExecutionAdapter; 5] = [
    AddigySourceExecutionAdapter::new(AddigyFamily::AuditEvents),
    AddigySourceExecutionAdapter::new(AddigyFamily::Devices),
    AddigySourceExecutionAdapter::new(AddigyFamily::Groups),
    AddigySourceExecutionAdapter::new(AddigyFamily::Policies),
    AddigySourceExecutionAdapter::new(AddigyFamily::Users),
];

impl AddigySourceExecutionAdapter {
    const fn new(family: AddigyFamily) -> Self {
        Self { family }
    }

    pub(crate) fn compiled_plan(&self) -> SourceExecutionPlanV1 {
        let family_id = self.family.as_str();
        let definition = AddigyRuntimeDefinition::compile(self.family)
            .expect("closed Addigy family must compile");
        let mut plan = SourceExecutionPlanV1 {
            ...
            path: family_path(self.family, "{organization_id}"),
    ...
    fn kernel(
    ...
    ) -> Result<(AddigyKernel, String), SourceExecutionError> {
        ...
        let organization_id = public_value(&metadata.public_config, "organization_id");
        if matches!(self.family, AddigyFamily::Groups | AddigyFamily::Users)
            && organization_id.is_none()
        {
            return Err(SourceExecutionError::MissingConfiguration);
        }
        let observed_at = timestamp(context.observed_at_unix_millis)?;
        let kernel = AddigyKernel::new(
            base_url,
            &context.tenant_id,
            self.family,
            organization_id,
...
impl SourceExecutionAdapter for AddigySourceExecutionAdapter {
    ...
    fn plan_v2(
        ...
        if provider_request.method() != plan.method
            || provider_request.url().path()
                != family_path(
                    self.family,
                    public_value(&metadata.public_config, "organization_id").unwrap_or_default(),
                )
            || provider_request.authentication_header() != "x-api-key"
        ...
        let body = serde_json::to_vec(provider_request.body())
...
const fn record_selector(family: AddigyFamily) -> &'static str {
    match family {
...
fn family_path(family: AddigyFamily, organization_id: &str) -> String {
    let suffix = match family {
        AddigyFamily::AuditEvents => "/system-events/search".to_owned(),
        AddigyFamily::Devices => "/devices".to_owned(),
        AddigyFamily::Groups => {
            format!("/o/{organization_id}/end-users/groups/query")
        }
        AddigyFamily::Policies => "/oa/policies/query".to_owned(),
        AddigyFamily::Users => format!("/o/{organization_id}/users/query"),
    };
    format!("/api/v2{suffix}")
}
...
fn map_error(error: AddigyError) -> SourceExecutionError {
    match error {
        AddigyError::InvalidFamily => SourceExecutionError::UnknownAdapter,
        AddigyError::InvalidConfiguration(_)
        | AddigyError::InvalidOrigin
        | AddigyError::MissingOrganizationId => SourceExecutionError::MissingConfiguration,
        AddigyError::InvalidTenantId => SourceExecutionError::InvalidExecutionContext,
        AddigyError::MissingCredentialReference => SourceExecutionError::MissingCredentialReference,
        AddigyError::CredentialUnavailable => SourceExecutionError::CredentialUnavailable,
        AddigyError::InvalidCursor => SourceExecutionError::InvalidCursor,
        AddigyError::RequestScopeMismatch => SourceExecutionError::InvalidPlan,
        AddigyError::AuthenticationRejected => SourceExecutionError::AuthenticationRejected,
        AddigyError::RequiredScopeMissing => SourceExecutionError::RequiredProviderScopeMissing,
        AddigyError::EgressDenied => SourceExecutionError::EgressDenied,
        AddigyError::DnsFailure | AddigyError::ConnectionFailure => {
            SourceExecutionError::ConnectionFailure
        }
        AddigyError::ProviderTimeout => SourceExecutionError::ProviderTimeout,
        AddigyError::RateLimited { .. } => SourceExecutionError::ProviderRateLimit,
        AddigyError::ProviderResourceNotFound
        | AddigyError::ProviderUnavailable { .. }
        | AddigyError::UnexpectedStatus { .. }
        | AddigyError::InvalidRetryAfter => SourceExecutionError::UnexpectedProviderStatus,
        AddigyError::ResponseTooLarge => SourceExecutionError::ResponseTooLarge,
        AddigyError::TooManyRecords => SourceExecutionError::ResultTooLarge,
        AddigyError::MalformedResponse => SourceExecutionError::MalformedResponse,
        AddigyError::InvalidProviderRecord | AddigyError::CredentialMaterial => {
            SourceExecutionError::InvalidProviderRecord
        }
        AddigyError::MissingStableIdentity => SourceExecutionError::MissingStableIdentity,
        AddigyError::OrganizationMismatch | AddigyError::TenantMismatch => {
            SourceExecutionError::TenantMismatch
        }
        AddigyError::ConflictingDuplicate => SourceExecutionError::DuplicateConflict,
        AddigyError::EventContractRejection => SourceExecutionError::EventContractRejected,
        AddigyError::AppendFailure => SourceExecutionError::AppendFailed,
        AddigyError::ProjectionFailure => SourceExecutionError::ProjectionFailed,
        AddigyError::LeaseLoss => SourceExecutionError::LeaseLost,
        AddigyError::StaleAuthority => SourceExecutionError::StaleAuthority,
        AddigyError::InternalRuntimeFailure => SourceExecutionError::InternalRuntime,
    }
}
