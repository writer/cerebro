...
#[test]
fn closed_dispatcher_registers_exactly_the_abnormal_security_families() {
    let dispatcher = SourceExecutionDispatcher;
    assert_eq!(ABNORMAL_SECURITY_SOURCE_EXECUTION_ADAPTERS.len(), 5);
    for adapter in &ABNORMAL_SECURITY_SOURCE_EXECUTION_ADAPTERS {
        let compiled = dispatcher
            .compile_plan(&SourceExecutionSelectionRequestV1 {
                source_id: "abnormal_security".to_owned(),
                family_id: adapter.family_id().to_owned(),
            })
            .unwrap();
        assert_eq!(compiled, adapter.compiled_plan());
        assert_eq!(compiled.source_id, "abnormal_security");
