//! Google Workspace `audit` bridge for the closed source-execution protocol.
//!
//! The adapter receives only authenticated execution context, public Reports API
...
const FAMILY_ID: &str = "audit";
const PROVIDER_KERNEL: &str = "google_workspace.audit";
const PLAN_ID: &str = "source-plan-v1:google_workspace:audit";
const DEFAULT_BASE_URL: &str = "https://admin.googleapis.com";
const METHOD: &str = "GET";
const PATH: &str = "/admin/reports/v1/activity/users/all/applications/{applicationName}";
const RECORD_SELECTOR: &str = "$.items[*]";
const ID_FIELD: &str = "id.uniqueQualifier";
const MAX_RESPONSE_BYTES: u64 = 8 << 20;
const EVENT_KIND: &str = "google_workspace.audit";
const SCHEMA_REF: &str = "google_workspace/audit/v1";

/// Credential-free adapter for the Google Workspace audit family.
#[derive(Clone, Copy, Debug, Default)]
pub(crate) struct GoogleWorkspaceAuditSourceExecutionAdapter;

/// Shared instance ready for closed-dispatcher registration.
pub(crate) static GOOGLE_WORKSPACE_AUDIT_SOURCE_EXECUTION_ADAPTER:
    GoogleWorkspaceAuditSourceExecutionAdapter = GoogleWorkspaceAuditSourceExecutionAdapter;

impl GoogleWorkspaceAuditSourceExecutionAdapter {
    pub(crate) fn compiled_plan(&self) -> SourceExecutionPlanV1 {
        let mut plan = SourceExecutionPlanV1 {
            ...
            required_attributes: ["domain", "family", "event_type"]
                .into_iter()
                .map(str::to_owned)
                .collect(),
    ...
    fn kernel(
        ...
        let application = public_value(&metadata.public_config, "application").map(str::to_owned);
...
impl SourceExecutionAdapter for GoogleWorkspaceAuditSourceExecutionAdapter {
    ...
    fn validate_record_identity(
        ...
        if record.event_id != format!("google-workspace-audit-{}", record.provider_id)
    ...
    fn plan_v2(
        ...
        let provider_request = kernel
            ...
            .map_err(map_error)?;
        if provider_request.authorization_scheme() != "Bearer"
        ...
        let records = page
            ...
            .map(|event| {
                ...
                let provider_id = event
                    .event_id
                    .strip_prefix("google-workspace-audit-")
                    .map(str::to_owned)
