//! ADP Workforce Now bridge for the closed source-execution protocol.
//!
...
//! OAuth credential redemption, Bearer authentication, mutual TLS identity,
...
use super::{AdpError, AdpFamily, AdpKernel, AdpRuntimeDefinition, normalize::event_id};

const SOURCE_ID: &str = "adp_workforce_now";
const DEFAULT_BASE_URL: &str = "https://api.adp.com";
const METHOD: &str = "GET";
const ACCEPT: &str = "application/json";
const MAX_RESPONSE_BYTES: u64 = 8 << 20;

/// Credential-free adapter for one cataloged ADP Workforce Now family.
#[derive(Clone, Copy, Debug)]
pub(crate) struct AdpSourceExecutionAdapter {
    family: AdpFamily,
}

/// Provider-local adapter set. Shared dispatcher registration and authority
/// promotion remain separate delivery steps.
pub(crate) static ADP_WORKFORCE_NOW_SOURCE_EXECUTION_ADAPTERS: [AdpSourceExecutionAdapter; 2] = [
    AdpSourceExecutionAdapter::new(AdpFamily::EventNotifications),
    AdpSourceExecutionAdapter::new(AdpFamily::Users),
];

impl AdpSourceExecutionAdapter {
    const fn new(family: AdpFamily) -> Self {
        Self { family }
    }

    pub(crate) fn compiled_plan(&self) -> SourceExecutionPlanV1 {
        let family_id = self.family.as_str();
        let definition = AdpRuntimeDefinition::compile(self.family)
            .expect("closed ADP Workforce Now family must compile");
    ...
    fn kernel(
    ...
    ) -> Result<(AdpKernel, String), SourceExecutionError> {
        ...
        let kernel = AdpKernel::new(base_url, &context.tenant_id, self.family, &observed_at)
            .map_err(map_error)?;
...
impl SourceExecutionAdapter for AdpSourceExecutionAdapter {
...
const fn record_selector(family: AdpFamily) -> &'static str {
    match family {
...
const fn id_field(family: AdpFamily) -> &'static str {
    family.id_field()
}
...
fn map_error(error: AdpError) -> SourceExecutionError {
    match error {
        AdpError::InvalidFamily => SourceExecutionError::UnknownAdapter,
        AdpError::InvalidConfiguration(_) | AdpError::InvalidOrigin => {
            SourceExecutionError::MissingConfiguration
        }
        AdpError::InvalidTenantId => SourceExecutionError::InvalidExecutionContext,
        AdpError::MissingCredentialReference | AdpError::MissingMutualTlsReference => {
            SourceExecutionError::MissingCredentialReference
        }
        AdpError::CredentialUnavailable | AdpError::MutualTlsUnavailable => {
            SourceExecutionError::CredentialUnavailable
        }
        AdpError::InvalidCursor => SourceExecutionError::InvalidCursor,
        AdpError::RequestScopeMismatch => SourceExecutionError::InvalidPlan,
        AdpError::AuthenticationRejected => SourceExecutionError::AuthenticationRejected,
        AdpError::RequiredScopeMissing => SourceExecutionError::RequiredProviderScopeMissing,
        AdpError::EgressDenied => SourceExecutionError::EgressDenied,
        AdpError::DnsFailure | AdpError::ConnectionFailure => {
            SourceExecutionError::ConnectionFailure
        }
        AdpError::ProviderTimeout => SourceExecutionError::ProviderTimeout,
        AdpError::RateLimited { .. } => SourceExecutionError::ProviderRateLimit,
        AdpError::ProviderResourceNotFound
        | AdpError::ProviderUnavailable { .. }
        | AdpError::UnexpectedStatus { .. }
        | AdpError::InvalidRetryAfter => SourceExecutionError::UnexpectedProviderStatus,
        AdpError::ResponseTooLarge => SourceExecutionError::ResponseTooLarge,
        AdpError::TooManyRecords => SourceExecutionError::ResultTooLarge,
        AdpError::MalformedResponse => SourceExecutionError::MalformedResponse,
        AdpError::InvalidProviderRecord | AdpError::CredentialMaterial => {
            SourceExecutionError::InvalidProviderRecord
        }
        AdpError::MissingStableIdentity => SourceExecutionError::MissingStableIdentity,
        AdpError::TenantMismatch => SourceExecutionError::TenantMismatch,
        AdpError::ConflictingDuplicate => SourceExecutionError::DuplicateConflict,
        AdpError::EventContractRejection => SourceExecutionError::EventContractRejected,
        AdpError::AppendFailure => SourceExecutionError::AppendFailed,
        AdpError::ProjectionFailure => SourceExecutionError::ProjectionFailed,
        AdpError::LeaseLoss => SourceExecutionError::LeaseLost,
        AdpError::StaleAuthority => SourceExecutionError::StaleAuthority,
        AdpError::InternalRuntimeFailure => SourceExecutionError::InternalRuntime,
    }
}
