//! ActivTrak bridge for the closed source-execution protocol.
//!
...
use super::{
    ActivTrakError, ActivTrakFamily, ActivTrakKernel, ActivTrakRuntimeDefinition,
    normalize::event_id,
};

const SOURCE_ID: &str = "activtrak";
const DEFAULT_BASE_URL: &str = "https://api.activtrak.com";
const METHOD: &str = "GET";
const ACCEPT: &str = "application/json";
const MAX_RESPONSE_BYTES: u64 = 8 << 20;

/// Credential-free adapter for one cataloged ActivTrak family.
#[derive(Clone, Copy, Debug)]
pub(crate) struct ActivTrakSourceExecutionAdapter {
...
pub(crate) static ACTIVTRAK_SOURCE_EXECUTION_ADAPTERS: [ActivTrakSourceExecutionAdapter; 5] = [
    ActivTrakSourceExecutionAdapter::new(ActivTrakFamily::ActivityLog),
    ActivTrakSourceExecutionAdapter::new(ActivTrakFamily::Clients),
    ActivTrakSourceExecutionAdapter::new(ActivTrakFamily::Consumers),
    ActivTrakSourceExecutionAdapter::new(ActivTrakFamily::Groups),
    ActivTrakSourceExecutionAdapter::new(ActivTrakFamily::Users),
];

impl ActivTrakSourceExecutionAdapter {
    const fn new(family: ActivTrakFamily) -> Self {
        Self { family }
    }

    pub(crate) fn compiled_plan(&self) -> SourceExecutionPlanV1 {
        let family_id = self.family.as_str();
        let definition = ActivTrakRuntimeDefinition::compile(self.family)
            .expect("closed ActivTrak family must compile");
    ...
    fn kernel(
    ...
    ) -> Result<(ActivTrakKernel, String), SourceExecutionError> {
        ...
        let kernel = ActivTrakKernel::new(base_url, &context.tenant_id, self.family, &observed_at)
            .map_err(map_error)?;
...
impl SourceExecutionAdapter for ActivTrakSourceExecutionAdapter {
...
const fn record_selector(family: ActivTrakFamily) -> &'static str {
    match family {
        ActivTrakFamily::ActivityLog => "$.activity[*]",
        ActivTrakFamily::Clients => "$.clients[*]",
        ActivTrakFamily::Consumers => "$.consumers[*]",
        ActivTrakFamily::Groups | ActivTrakFamily::Users => "$.resources[*]",
    }
}
...
fn map_error(error: ActivTrakError) -> SourceExecutionError {
    match error {
        ActivTrakError::InvalidFamily => SourceExecutionError::UnknownAdapter,
        ActivTrakError::InvalidConfiguration(_) | ActivTrakError::InvalidOrigin => {
            SourceExecutionError::MissingConfiguration
        }
        ActivTrakError::InvalidTenantId => SourceExecutionError::InvalidExecutionContext,
        ActivTrakError::MissingCredentialReference => {
            SourceExecutionError::MissingCredentialReference
        }
        ActivTrakError::CredentialUnavailable => SourceExecutionError::CredentialUnavailable,
        ActivTrakError::InvalidCursor => SourceExecutionError::InvalidCursor,
        ActivTrakError::RequestScopeMismatch => SourceExecutionError::InvalidPlan,
        ActivTrakError::AuthenticationRejected => SourceExecutionError::AuthenticationRejected,
        ActivTrakError::RequiredScopeMissing => SourceExecutionError::RequiredProviderScopeMissing,
        ActivTrakError::EgressDenied => SourceExecutionError::EgressDenied,
        ActivTrakError::DnsFailure | ActivTrakError::ConnectionFailure => {
            SourceExecutionError::ConnectionFailure
        }
        ActivTrakError::ProviderTimeout => SourceExecutionError::ProviderTimeout,
        ActivTrakError::RateLimited { .. } => SourceExecutionError::ProviderRateLimit,
        ActivTrakError::ProviderResourceNotFound
        | ActivTrakError::ProviderUnavailable { .. }
        | ActivTrakError::UnexpectedStatus { .. }
        | ActivTrakError::InvalidRetryAfter => SourceExecutionError::UnexpectedProviderStatus,
        ActivTrakError::ResponseTooLarge => SourceExecutionError::ResponseTooLarge,
        ActivTrakError::TooManyRecords => SourceExecutionError::ResultTooLarge,
        ActivTrakError::MalformedResponse => SourceExecutionError::MalformedResponse,
        ActivTrakError::InvalidProviderRecord | ActivTrakError::CredentialMaterial => {
            SourceExecutionError::InvalidProviderRecord
        }
        ActivTrakError::MissingStableIdentity => SourceExecutionError::MissingStableIdentity,
        ActivTrakError::TenantMismatch => SourceExecutionError::TenantMismatch,
        ActivTrakError::ConflictingDuplicate => SourceExecutionError::DuplicateConflict,
        ActivTrakError::EventContractRejection => SourceExecutionError::EventContractRejected,
        ActivTrakError::AppendFailure => SourceExecutionError::AppendFailed,
        ActivTrakError::ProjectionFailure => SourceExecutionError::ProjectionFailed,
        ActivTrakError::LeaseLoss => SourceExecutionError::LeaseLost,
        ActivTrakError::StaleAuthority => SourceExecutionError::StaleAuthority,
        ActivTrakError::InternalRuntimeFailure => SourceExecutionError::InternalRuntime,
    }
}
