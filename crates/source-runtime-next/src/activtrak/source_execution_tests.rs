...
use super::{
    ActivTrakFamily,
    source_execution::{ACTIVTRAK_SOURCE_EXECUTION_ADAPTERS, ActivTrakSourceExecutionAdapter},
};
...
fn adapter(family: ActivTrakFamily) -> &'static ActivTrakSourceExecutionAdapter {
    ACTIVTRAK_SOURCE_EXECUTION_ADAPTERS
        .iter()
        .find(|adapter| adapter.family_id() == family.as_str())
        .unwrap()
}

fn fixture(family: ActivTrakFamily) -> Value {
...
fn context(cursor: &str, page: u32) -> SourceWorkerExecutionContextV1 {
    SourceWorkerExecutionContextV1 {
        ...
        runtime_id: "activtrak-runtime".to_owned(),
        logical_page_id: format!("source-page-v2:activtrak-{page}"),
...
fn metadata(family: ActivTrakFamily) -> SourceWorkerRuntimeMetadataV2 {
...
fn plan_page(
    adapter: &ActivTrakSourceExecutionAdapter,
...
fn activtrak_plans_all_five_families_without_credentials() {
    for family in ActivTrakFamily::ALL {
        ...
        assert_eq!(execution.allowed_origin, "https://api.activtrak.com");
        ...
        let expected_url = match family {
            ActivTrakFamily::ActivityLog => {
                "https://api.activtrak.com/reports/v2/activitylog?pageSize=150".to_owned()
            }
            ActivTrakFamily::Clients | ActivTrakFamily::Consumers => {
                format!("https://api.activtrak.com{}", family.path())
            }
            ActivTrakFamily::Groups | ActivTrakFamily::Users => {
                format!(
                    "https://api.activtrak.com{}?count=100&startIndex=1",
                    family.path()
                )
            }
        };
...
#[test]
fn activtrak_decodes_every_fixture_with_exact_contract_and_seals() {
    for family in ActivTrakFamily::ALL {
        let adapter = adapter(family);
        let plan = adapter.compiled_plan();
        let context = context("", 1);
        ...
        assert!(record.event_id.starts_with("activtrak-tenant-"));
...
#[test]
fn activtrak_pagination_and_provider_failures_are_typed() {
    let paged_family = ActivTrakFamily::ActivityLog;
...
#[test]
fn activtrak_rejects_bad_scope_cursor_duplicates_and_secret_material() {
...
#[test]
fn closed_dispatcher_registers_exactly_the_activtrak_families() {
    let dispatcher = SourceExecutionDispatcher;
    assert_eq!(ACTIVTRAK_SOURCE_EXECUTION_ADAPTERS.len(), 5);
    for adapter in &ACTIVTRAK_SOURCE_EXECUTION_ADAPTERS {
        let compiled = dispatcher
            .compile_plan(&SourceExecutionSelectionRequestV1 {
                source_id: "activtrak".to_owned(),
                family_id: adapter.family_id().to_owned(),
            })
            .unwrap();
        assert_eq!(compiled, adapter.compiled_plan());
        assert_eq!(compiled.source_id, "activtrak");
