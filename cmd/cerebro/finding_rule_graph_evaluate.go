...
func runFindingRuleGraphEvaluate(args []string) error {
	...
	).WithGraphStore(sourceProjectionGraphStore(deps.GraphStore)).WithRawCypherQueryStore(deps.GraphReads.RawCypher).WithTrustedSourceResolution().WithAppendLog(deps.AppendLog)
