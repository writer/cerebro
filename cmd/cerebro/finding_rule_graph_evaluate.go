...
func runFindingRuleGraphEvaluate(args []string) error {
	...
	).WithGraphStore(sourceProjectionGraphStore(deps.GraphStore)).WithEntityCatalogStore(deps.GraphReads.Catalog).WithRawCypherQueryStore(deps.GraphReads.RawCypher).WithTrustedSourceResolution().WithAppendLog(deps.AppendLog)
