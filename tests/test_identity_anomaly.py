...
def mock_baselines():
    ...
    return {
        ...
        "user2": BehavioralBaseline(
            ...
            permission_levels={"admin.full": 3, "user.read": 1, "user.write": 2},  # Admin perms
...
class TestIdentityAnomalyDetector:
    ...
    async def test_permission_anomaly_detection(self, detector, mock_baselines):
        ...
        if anomalies:
            admin_anomaly = next((a for a in anomalies if a.principal_id == "user2"), None)
            if admin_anomaly:
...
class TestIntegrationScenarios:
    ...
    async def test_full_anomaly_analysis_flow(self, detector, mock_baselines):
        ...
        detector.query_engine.execute_query = AsyncMock(return_value=MagicMock(
            rows=[{"username": "test", "last_login": "2024-01-15T10:00:00Z", "status": "active"}],
            total_rows=1
        ))
        ...
        try:
            ...
            assert "analyze_identity_anomalies" in str(type(e).__name__) or isinstance(e, Exception)
