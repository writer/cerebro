...
def test_collect_organization_happy_path(client: TestClient, admin_token: str, test_org, monkeypatch):
    ...
    monkeypatch.setattr("cerebro.collectors.manager.CollectorManager.collect_organization", fake_collect)
...
def test_collect_organization_missing_org(client: TestClient, admin_token: str, monkeypatch):
    async def fake_collect(self, org_id: str, providers=None, resource_types=None):
...
def test_collect_organization_background_missing_org(client: TestClient, admin_token: str):
    response = client.post(
