...
def test_compute_vendor_portfolio_trend_direction() -> None:
    ...
    trend = compute_vendor_portfolio_trend(
        [
            VendorPortfolioSnapshot(
                timestamp=REFERENCE_NOW - timedelta(days=7),
                vendors=[overdue],
            ),
            VendorPortfolioSnapshot(
                timestamp=REFERENCE_NOW,
                vendors=[vendor, overdue],
            ),
        ]
    )
...
def test_compute_customer_health_trend_direction() -> None:
    ...
    trend = compute_customer_health_trend(
        [
            CustomerHealthSnapshot(
                timestamp=REFERENCE_NOW - timedelta(days=30),
                customers=[at_risk],
            ),
            CustomerHealthSnapshot(
                timestamp=REFERENCE_NOW,
                customers=[healthy, at_risk],
            ),
        ]
    )
...
def test_build_vendor_risk_dashboard_includes_critical_vendors() -> None:
    ...
    dashboard = build_vendor_risk_dashboard(
        [vendor, overdue],
        REFERENCE_NOW,
        critical_limit=3,
    )
...
def test_build_customer_risk_dashboard_prioritizes_at_risk_accounts() -> None:
    ...
    dashboard = build_customer_risk_dashboard(
        [healthy, at_risk],
        REFERENCE_NOW,
        at_risk_limit=3,
    )
...
def test_analyze_vendor_snapshots_surface_alerts() -> None:
    ...
    snapshots = [
        VendorPortfolioSnapshot(
            timestamp=REFERENCE_NOW - timedelta(days=35),
            vendors=[vendor],
        ),
        VendorPortfolioSnapshot(
            timestamp=REFERENCE_NOW - timedelta(days=5),
            vendors=[vendor, overdue],
        ),
        VendorPortfolioSnapshot(
            timestamp=REFERENCE_NOW,
...
def test_analyze_customer_snapshots_surface_alerts() -> None:
    ...
    snapshots = [
        CustomerHealthSnapshot(
            timestamp=REFERENCE_NOW - timedelta(days=40),
            customers=[healthy],
        ),
        CustomerHealthSnapshot(
            timestamp=REFERENCE_NOW - timedelta(days=10),
            customers=[healthy],
        ),
        CustomerHealthSnapshot(
            timestamp=REFERENCE_NOW,
