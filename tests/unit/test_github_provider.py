...
async def test_authenticate_success(monkeypatch):
    fake_org = MagicMock()
    fake_user = MagicMock()
    fake_github = MagicMock(get_user=MagicMock(return_value=fake_user), get_organization=MagicMock(return_value=fake_org))

    monkeypatch.setattr("cerebro.providers.github.provider.settings.github_token", "token")
    monkeypatch.setattr("cerebro.providers.github.provider.Github", MagicMock(return_value=fake_github))
...
async def test_discover_resources_yields_repos(monkeypatch):
    fake_repo = MagicMock(full_name="acme/repo", name="repo", private=False, archived=False, disabled=False, fork=False, default_branch="main")
    fake_org = MagicMock(get_repos=MagicMock(return_value=[fake_repo]), get_teams=MagicMock(return_value=[]))
    fake_github = MagicMock(get_user=MagicMock(), get_organization=MagicMock(return_value=fake_org))
...
async def test_discover_resources_respects_filter(monkeypatch):
    fake_repo = MagicMock(full_name="acme/repo", name="repo", private=False, archived=False, disabled=False, fork=False, default_branch="main")
    fake_team = MagicMock(id=1, name="secops", slug="secops", description="", privacy="closed", permission="push")
    fake_org = MagicMock(get_repos=MagicMock(return_value=[fake_repo]), get_teams=MagicMock(return_value=[fake_team]))
    fake_github = MagicMock(get_user=MagicMock(), get_organization=MagicMock(return_value=fake_org))
...
async def test_discover_iam_edges_for_repo(monkeypatch):
    ...
    repo = MagicMock(get_collaborators=MagicMock(return_value=[collaborator]), get_collaborator_permission=MagicMock(return_value=MagicMock(permission="admin")))
    fake_github = MagicMock(get_user=MagicMock(), get_repo=MagicMock(return_value=repo))
    membership = MagicMock(role="admin")
    member = MagicMock(login="jane")
    fake_org = MagicMock(get_members=MagicMock(return_value=[member]), get_membership=MagicMock(return_value=membership))
    ...
    org_edge = [edge for edge in edges if edge.resource_external_id is None][0]
