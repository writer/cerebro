...
from cerebro.collectors.collector import CollectionResult, ConfigCollector
from cerebro.core.bulk_operations import BulkOperations, compute_config_hash
from cerebro.core.models import Principal, Resource
...
class TestBulkOperations:
    ...
    @pytest.mark.asyncio
    async def test_bulk_insert_config_snapshots(
        self,
        test_db,
        test_org,
        test_aws_account,
...
class TestConcurrentCollection:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_config_collection_mock(
        self,
        test_db,
        test_org,
        test_aws_account,
        ...
        async def mock_get_config(resource_info):
            return MagicMock(
                captured_at=datetime.utcnow(),
                normalized_config={
                    "bucket": resource_info.external_id,
                    "versioning": True,
                },
            )
...
async def test_database_performance_tuning(test_db):
    ...
    max_overflow = (
        ...
        else getattr(engine.pool, "max_overflow", 0)
    )
