"""Test P0 security fixes across configuration, users, rules, and API auth."""
...
from cerebro.api.routers import (
    accounts,
    collectors,
    findings,
    organizations,
...
class TestConfigurationSecurity:
    ...
    def test_secret_key_validation_production(self):
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "production"}):
            ...
            with pytest.raises(
                ValueError,
                match="SECRET_KEY must be configured for non-development environments",
            ):
                Settings(secret_key="", kms_provider="aws")

    def test_secret_key_validation_development(self):
        """Test SECRET_KEY validation allows defaults in development."""
        with patch.dict(os.environ, {"ENVIRONMENT": "development"}):
            config = Settings(
                secret_key="SecureDevKey!1234567890",
                kms_provider="aws",
            )
    ...
    def test_kms_provider_validation_production(self):
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "production"}):
            ...
            with pytest.raises(
                ValueError,
                match="Local KMS provider is INSECURE and not allowed",
            ):
                Settings(
    ...
    def test_provider_env_fallback_validation_production(self):
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "production"}):
            ...
            with pytest.raises(
                ValueError,
                match="Provider environment variable fallback",
            ):
                Settings(
                    secret_key="a-very-secure-secret-key-that-is-32-chars-long",
                    kms_provider="aws",
