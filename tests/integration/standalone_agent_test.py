...
async def test_agent_models():
    ...
    try:
        ...
        from cerebro.agents.models import (
            AgentSession, AgentMessage, AgentType, MessageRole,
            ToolInvocation, ToolInvocationStatus, ToolApproval, ApprovalStatus
        )
        
        # Test AgentType enum
        expected_types = [
            "security_analyst", "incident_responder", "identity_advisor",
            "compliance_advisor", "attack_path_analyst"
        ]
...
async def test_database_operations():
    ...
    try:
        ...
        session_messages = [
            msg for msg in mock_db["messages"].values()
...
async def test_streaming_simulation():
    ...
    try:
        ...
        large_response = MockClaudeResponse(
...
async def test_complete_workflow():
    ...
    try:
        from cerebro.agents.models import (
            AgentSession, AgentMessage, ToolInvocation, ToolApproval,
            AgentType, MessageRole, ToolInvocationStatus, ApprovalStatus
        )
        ...
        initial_messages = [
            AgentMessage(
                session_id=session_id,
                role=MessageRole.USER,
                content={"text": "We've detected a potential data breach. Multiple failed login attempts followed by successful authentication from unusual locations."}
            ),
            AgentMessage(
                session_id=session_id,
                role=MessageRole.ASSISTANT,
                content={"text": "I understand this is a critical security incident. I'll start by analyzing the authentication patterns and querying related security findings."}
            ),
            AgentMessage(
                session_id=session_id,
                role=MessageRole.ASSISTANT,
                content={"text": "Based on my analysis, I've identified a pattern of credential stuffing attacks targeting high-privilege accounts. I recommend immediate password resets for affected users and implementing additional MFA requirements."}
            )
        ]
        ...
        investigation_tools = [
            ...
            ToolInvocation(
                ...
                input_data={
                    ...
                    "affected_resources": ["user/suspicious_account", "service/auth_api"]
                },
                status=ToolInvocationStatus.SUCCESS
            ),
            ToolInvocation(
...
async def main():
    ...
    for suite_name, results in all_results:
        ...
        print(f"{suite_name}: {status} ({results.passed} passed, {results.failed} failed)")
        logger.info(f"{suite_name}: {status} ({results.passed} passed, {results.failed} failed)")
