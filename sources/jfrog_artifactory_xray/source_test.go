...
func TestSourceCheckAndRead(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.RequestURI() == "/xray/api/v1/system/ping" {
	...
	defer server.Close()
	cfgValues := map[string]string{"tenant_id": "tenant", "base_url": server.URL, "family": defaultFamily, "token": "test-token", "jfrog_url": "test-jfrog_url"}
	...
	event := pull.Events[0]
	if event.Kind != "jfrog_artifactory_xray.assets" {
