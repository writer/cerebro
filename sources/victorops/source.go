...
const (
	...
	defaultHealthPath      = "/api-reporting/v1/team/${config.team}/oncall/log"
	defaultBaseURLTemplate = "https://api.victorops.com"
	tokenHeader            = ""
	tokenScheme            = "Bearer"
	familyLog              = "log"
	familyTeam             = "team"
	familyUser             = "user"
	familyIncident         = "incident"
)

var templateKeys = []string{"team", "token"}
	...
	inner, err := jsonapi.New(spec, jsonapi.Options{
		...
		Families: []jsonapi.Family{
			...
			},
			{
				...
				StaticAttributes: map[string]string{"record_class": "identity_user", "schema": "user", "source_system": "victorops"},
			},
			{
				Name:             familyIncident,
				Path:             "/api-reporting/v2/incidents",
				URNKind:          "victorops_incident",
				IDKeys:           []string{"alertCount", "alert_id", "id", "sid", "incident_id", "uuid"},
				PageSizeParams:   []string{"limit"},
				ListKeys:         []string{"incidents"},
				TimestampKeys:    []string{"observed_at", "updated_at", "last_seen_at", "created_at"},
				Attributes:       map[string]string{"alert_description": "description|summary|message|body", "alert_fired_at": "fired_at|triggered_at|created_at|occurred_at|timestamp", "alert_id": "alertCount", "alert_name": "alertCount", "alert_resolved_at": "resolved_at|closed_at|acknowledged_at", "alert_severity": "alertCount", "alert_source": "source|alert_source|monitor|check", "alert_status": "alertCount", "alert_type": "alert_type|type|category|kind", "evidence_cas_commit_id": "evidence_cas.commit_id|evidence_cas_commit_id|commit_id", "evidence_cas_digest": "evidence_cas.digest|evidence_cas_digest|digest", "evidence_cas_merkle_root": "evidence_cas.merkle_root|evidence_cas_merkle_root|merkle_root", "evidence_cas_ref_type": "evidence_cas.ref_type|evidence_cas_ref_type|ref_type", "evidence_cas_uri": "evidence_cas.uri|evidence_cas_uri|uri", "id": "alertCount", "name": "alertCount", "observed_at": "observed_at|updated_at|last_seen_at", "resource_id": "resource_id|id|metadata.resource_id", "resource_name": "name|display_name|hostname|metadata.resource_name", "resource_type": "resource_type|type|metadata.resource_type", "resource_urn": "resource_urn|urn|metadata.resource_urn", "source_event_id": "event_id|id|metadata.event_id", "tenant_id": "tenant_id|metadata.tenant_id"},
				StaticAttributes: map[string]string{"record_class": "alert", "schema": "incident", "source_system": "victorops"},
			},
		},
	})
	if err != nil {
