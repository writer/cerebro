...
const (
	...
	defaultHealthPath      = "/2010-04-01/Accounts.json"
		...
		Families: []jsonapi.Family{
			{
				...
				StaticAttributes: map[string]string{"record_class": "identity_user", "schema": "accounts", "source_system": "twilio"},
			},
			{
				Name:             familyKeys,
				Path:             "/2010-04-01/Accounts/${config.account_sid}/Keys.json",
				URNKind:          "runtime_keys",
				...
				StaticAttributes: map[string]string{"record_class": "secret", "schema": "keys", "source_system": "twilio"},
			},
			{
				Name:             familyAuditEvents,
				Path:             "/v1/Events",
				URNKind:          "runtime_audit_events",
				IDKeys:           []string{"event_sid", "sid", "id", "event_id", "uuid", "request_id"},
				CursorParam:      "cursor",
				PageSizeParams:   []string{"limit"},
				ListKeys:         []string{"events", "data"},
				TimestampKeys:    []string{"event_date", "observed_at", "updated_at", "last_seen_at", "created_at"},
				Attributes:       map[string]string{"actor_email": "actor_email|actor.email|email|user.email", "actor_id": "actor_sid|actor_id|actor.id|actorId|user_id|user.id", "actor_name": "actor_name|actor.name|user.name", "event_type": "event_type|event_name|action|type", "evidence_cas_commit_id": "evidence_cas.commit_id|evidence_cas_commit_id|commit_id", "evidence_cas_digest": "evidence_cas.digest|evidence_cas_digest|digest", "evidence_cas_merkle_root": "evidence_cas.merkle_root|evidence_cas_merkle_root|merkle_root", "evidence_cas_ref_type": "evidence_cas.ref_type|evidence_cas_ref_type|ref_type", "evidence_cas_uri": "evidence_cas.uri|evidence_cas_uri|uri", "observed_at": "event_date|observed_at|updated_at|last_seen_at", "resource_email": "resource_email|target_email|target.email", "resource_id": "resource_sid|resource_id|target_id|target.id|resource.id|object_id", "resource_name": "resource_name|target_name|target.name|resource.name|object_name", "resource_type": "resource_type|target_type|target.type|object_type", "resource_urn": "resource_urn|urn|metadata.resource_urn", "source_event_id": "event_sid|sid|event_id|id|metadata.event_id", "tenant_id": "tenant_id|metadata.tenant_id"},
				StaticAttributes: map[string]string{"record_class": "audit_event", "schema": "audit_events", "source_system": "twilio"},
			},
		},
	})
	if err != nil {
