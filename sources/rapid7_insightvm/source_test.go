...
func TestSourceCheckAndRead(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.RequestURI() == "/vm/v4/integration/asset" {
	...
	defer server.Close()
	cfgValues := map[string]string{"tenant_id": "tenant", "base_url": server.URL, "family": defaultFamily, "api_token": "test-token", "region": "test-region"}
	...
	event := pull.Events[0]
	if event.Kind != "rapid7_insightvm.assets" {
