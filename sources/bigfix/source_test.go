...
func TestSourceCheckAndRead(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.RequestURI() == "/api/sites" {
			w.WriteHeader(http.StatusNoContent)
			return
		}
		if r.URL.Path != "/api/computers" {
	...
	defer server.Close()
	cfgValues := map[string]string{"tenant_id": "tenant", "base_url": server.URL, "family": defaultFamily, "token": "test-token", "endpoint": "test-endpoint"}
	...
	event := pull.Events[0]
	if event.Kind != "bigfix.computers" {
