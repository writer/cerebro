...
// Source is the in-process GitHub Source CDK skeleton backed by golden fixtures.
type Source struct {
...
// New constructs the fixture-backed GitHub source skeleton.
func New() (*Source, error) {
	specBytes, err := fixtures.ReadFile("catalog.yaml")
	if err != nil {
	...
	urnBytes, err := fixtures.ReadFile("testdata/discover.json")
	if err != nil {
	...
	readBytes, err := fixtures.ReadFile("testdata/read.json")
	if err != nil {
	...
	return &Source{spec: spec, urns: urns, events: events}, nil
}
...
// Check validates that required source configuration is present.
...
// Discover returns fixture-backed GitHub URNs.
...
// Read pages through the embedded fixture events one record at a time.
