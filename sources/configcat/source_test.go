...
func TestSourceCheckAndRead(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/v1/organizations/test-organizationid/auditlogs" {
	...
	defer server.Close()
	cfgValues := map[string]string{"tenant_id": "tenant", "base_url": server.URL, "family": defaultFamily, "token": "test-token", "organizationid": "test-organizationid", "productid": "test-productid"}
	...
	event := pull.Events[0]
	if event.Kind != "configcat.auditlog" {
