...
func openAIFamilies() []jsonapi.Family {
	return []jsonapi.Family{
		...
		openAIListFamily("admin_api_key", "/organization/admin_api_keys", "openai_admin_api_key", []string{"id"}, []string{"created_at", "last_used_at"}, map[string]string{"api_key_id": "id", "name": "name", "owner_id": "owner.id", "owner_name": "owner.name", "owner_object": "owner.object", "owner_role": "owner.role", "owner_type": "owner.type", "status": "status", "created_at": "created_at", "last_used_at": "last_used_at"}, withStaticAttributes(map[string]string{"key_class": "admin", "privileged": "true"})),
