...
const (
	...
	defaultHealthPath          = "/api/v2/databases"
	defaultBaseURLTemplate     = "https://${config.account}.snowflakecomputing.com"
	tokenHeader                = ""
	tokenScheme                = "Bearer"
	oauthTokenURLTemplate      = "https://${config.account}.snowflakecomputing.com/oauth/token" // #nosec G101 -- token endpoint URL template, not credential material.
	oauthScopeSeparator        = " "
	oauthTokenExpirationBuffer = 60 * time.Second
	familyAssets               = "assets"
	familyCortexSearchServices = "cortex_search_services"
	familyAuditEvents          = "audit_events"
	familyVulnerabilities      = "vulnerabilities"
)

var templateKeys = []string{"account", "client_id", "client_secret"}
...
func New() (*Source, error) {
	...
	inner, err := jsonapi.New(spec, jsonapi.Options{
		...
		OAuthTokenURL:   "https://${config.account}.snowflakecomputing.com/oauth/token",
		Families: []jsonapi.Family{
			{
				Name:             familyAssets,
				Path:             "/v1/assets",
				URNKind:          "snowflake_assets",
				...
				StaticAttributes: map[string]string{"record_class": "asset", "schema": "assets", "source_system": "snowflake"},
			},
			{
				Name:             familyCortexSearchServices,
				Path:             "/v1/cortex/search/services",
				URNKind:          "snowflake_cortex_search_services",
				...
				StaticAttributes: map[string]string{"record_class": "asset", "schema": "cortex_search_services", "source_system": "snowflake"},
			},
			{
				Name:             familyAuditEvents,
				Path:             "/v1/audit/events",
				URNKind:          "snowflake_audit_events",
				...
				StaticAttributes: map[string]string{"record_class": "audit_event", "schema": "audit_events", "source_system": "snowflake"},
			},
			{
				Name:             familyVulnerabilities,
				Path:             "/v1/vulnerabilities",
				URNKind:          "snowflake_vulnerabilities",
				...
				StaticAttributes: map[string]string{"record_class": "finding", "schema": "vulnerabilities", "source_system": "snowflake"},
			},
		},
	})
	if err != nil {
