...
func TestSourceCheckAndRead(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("xi-api-key") != "test-token" {
	...
	defer server.Close()
	cfgValues := map[string]string{"tenant_id": "tenant", "base_url": server.URL, "family": defaultFamily, "api_token": "test-token", "service_account_user_id": "test-service_account_user_id"}
	...
	event := pull.Events[0]
	if event.Kind != "elevenlabs.model_catalog" {
