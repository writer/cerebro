...
const (
	sourceID               = "argo_cd"
	defaultFamily          = familyPipelines
	defaultHealthPath      = "/api/v1/session/userinfo"
	defaultBaseURLTemplate = "https://${config.server_url}"
	tokenHeader            = ""
	tokenScheme            = "Bearer"
	familyPipelines        = "pipelines"
	familyFindings         = "findings"
	familyAuditEvents      = "audit_events"
)

var templateKeys = []string{"token", "server_url"}
...
func New() (*Source, error) {
	...
	inner, err := jsonapi.New(spec, jsonapi.Options{
		...
		Families: []jsonapi.Family{
			{
				Name:             familyPipelines,
				Path:             "/v1/pipelines",
				URNKind:          "argo_cd_pipelines",
				...
				StaticAttributes: map[string]string{"record_class": "asset", "schema": "pipelines", "source_system": "argo_cd"},
			},
			{
				Name:             familyFindings,
				Path:             "/v1/findings",
				URNKind:          "argo_cd_findings",
				...
				StaticAttributes: map[string]string{"record_class": "finding", "schema": "findings", "source_system": "argo_cd"},
			},
			{
				Name:             familyAuditEvents,
				Path:             "/v1/audit/events",
				URNKind:          "argo_cd_audit_events",
				...
				StaticAttributes: map[string]string{"record_class": "audit_event", "schema": "audit_events", "source_system": "argo_cd"},
			},
		},
	})
	if err != nil {
