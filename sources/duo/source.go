...
func New() (*Source, error) {
	...
	inner, err := jsonapi.New(spec, jsonapi.Options{
		...
		Families: []jsonapi.Family{
			...
			{Name: "application", Path: "/admin/v3/integrations", AuthModel: "duo_hmac_v5", URNKind: "duo_application", IDKeys: []string{"integration_key", "ikey", "key"}, ListKeys: []string{"response"}, TimestampKeys: []string{"created"}, Attributes: map[string]string{"resource_id": "integration_key|ikey|key", "resource_name": "name", "resource_type": "type|integration_type", "integration_key": "integration_key|ikey|key", "name": "name", "type": "type|integration_type"}, StaticAttributes: map[string]string{"source_product": "duo"}},
			{Name: "audit_event", Path: "/admin/v2/logs/activity", URNKind: "duo_audit_event", IDKeys: []string{"activity_id", "txid", "id"}, CursorParam: "next_offset", NextCursorKeys: []string{"response.metadata.next_offset"}, ListKeys: []string{"response.items"}, TimestampKeys: []string{"ts", "timestamp"}, Config: jsonapi.FamilyConfig{ConfigQuery: map[string]string{"mintime": "mintime", "maxtime": "maxtime", "sort": "sort"}}, Attributes: map[string]string{"event_type": "action|eventtype|event_type", "actor_id": "actor.key|actor_id", "actor_name": "actor.name|actor_name", "actor_type": "actor.type|actor_type", "resource_id": "target.key|application.key|resource_id", "resource_name": "target.name|application.name|resource_name", "resource_type": "target.type|application.type|resource_type"}, StaticAttributes: map[string]string{"source_product": "duo"}},
			{Name: "authentication_log", Path: "/admin/v2/logs/authentication", URNKind: "duo_authentication_log", IDKeys: []string{"txid", "id", "timestamp"}, CursorParam: "next_offset", NextCursorKeys: []string{"response.metadata.next_offset"}, ListKeys: []string{"response.authlogs"}, TimestampKeys: []string{"timestamp"}, Config: jsonapi.FamilyConfig{ConfigQuery: map[string]string{"mintime": "mintime", "maxtime": "maxtime", "sort": "sort"}}, Attributes: map[string]string{"event_type": "event_type|eventtype", "actor_id": "user.key|user_id", "actor_name": "user.name|username", "resource_id": "application.key|integration_key", "resource_name": "application.name|integration", "resource_type": "application.type|application", "factor": "factor", "result": "result", "reason": "reason"}, StaticAttributes: map[string]string{"source_product": "duo"}},
		},
	})
	if err != nil {
		return nil, err
