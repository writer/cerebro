...
func New() (*Source, error) {
	...
	inner, err := jsonapi.New(spec, jsonapi.Options{
		...
		Families: []jsonapi.Family{
			...
			{Name: "authentication_log", Path: "/admin/v2/logs/authentication", URNKind: "duo_authentication_log", IDKeys: []string{"txid", "id", "timestamp"}, CursorParam: "next_offset", NextCursorKeys: []string{"response.metadata.next_offset"}, ListKeys: []string{"response.authlogs"}, TimestampKeys: []string{"isotimestamp", "timestamp"}, Config: jsonapi.FamilyConfig{ConfigQuery: map[string]string{"mintime": "mintime", "maxtime": "maxtime", "sort": "sort"}}, Attributes: map[string]string{"event_type": "event_type|eventtype", "actor_id": "user.key|user_id", "actor_name": "user.name|username", "resource_id": "application.key|integration_key", "resource_name": "application.name|integration", "resource_type": "application.type|application", "factor": "factor", "result": "result", "reason": "reason"}, StaticAttributes: map[string]string{"source_product": "duo"}},
		},
	})
	if err != nil {
		return nil, err
