...
const (
	sourceID               = "fleetdm"
	defaultFamily          = familyHosts
	defaultHealthPath      = "/api/v1/fleet/health"
	defaultBaseURLTemplate = "https://${config.endpoint}"
	tokenHeader            = ""
	tokenScheme            = "Bearer"
	familyHosts            = "hosts"
	familyPolicies         = "policies"
	familyAuditActivities  = "audit_activities"
)

var templateKeys = []string{"token", "endpoint"}
		...
		Families: []jsonapi.Family{
			{
				...
				StaticAttributes: map[string]string{"record_class": "asset", "schema": "hosts", "source_system": "fleetdm"},
			},
			{
				Name:             familyPolicies,
				Path:             "/api/v1/fleet/policies",
				URNKind:          "fleetdm_policies",
			...
			},
			{
				...
				StaticAttributes: map[string]string{"record_class": "audit_event", "schema": "audit_activities", "source_system": "fleetdm"},
			},
		},
	})
	if err != nil {
