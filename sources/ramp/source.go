...
const (
	...
	defaultBaseURLTemplate     = "https://api.ramp.com/developer/v1"
	tokenHeader                = ""
	tokenScheme                = "Bearer"
	oauthTokenURLTemplate      = "https://api.ramp.com/developer/v1/token" // #nosec G101 -- token endpoint URL template, not credential material.
	oauthScopeSeparator        = " "
...
var oauthScopes = []string{"cards:read", "transactions:read", "users:read"}
...
func New() (*Source, error) {
	...
	inner, err := jsonapi.New(spec, jsonapi.Options{
		...
		OAuthTokenURL:   "https://api.ramp.com/developer/v1/token",
		OAuthScopes:     []string{"cards:read", "transactions:read", "users:read"},
		Families: []jsonapi.Family{
			{
				...
				StaticAttributes: map[string]string{"record_class": "identity_user", "schema": "users", "source_system": "ramp"},
			},
			{
				Name:             familyCards,
				Path:             "/cards",
				...
				StaticAttributes: map[string]string{"record_class": "asset", "schema": "cards", "source_system": "ramp"},
			},
			{
				Name:             familyTransactions,
				Path:             "/transactions",
				URNKind:          "ramp_transactions",
				...
				StaticAttributes: map[string]string{"record_class": "audit_event", "schema": "transactions", "source_system": "ramp"},
			},
		},
	})
	if err != nil {
