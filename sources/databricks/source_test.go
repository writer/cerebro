...
func TestSourceCheckAndRead(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.RequestURI() == "/api/2.0/preview/scim/v2/Me" {
	...
	defer server.Close()
	cfgValues := map[string]string{"tenant_id": "tenant", "base_url": server.URL, "family": defaultFamily, "token": "test-token", "workspace_url": "test-workspace_url"}
	...
	event := pull.Events[0]
	if event.Kind != "databricks.assets" {
