...
// NewFixture constructs the deterministic Duo Security source used by tests.
...
func resolveFixtureFamily(cfg sourcecdk.Config) (string, error) {
	...
	if family == familyAuditEvents || family == familyAuthenticationLogs {
		if strings.TrimSpace(sourcecdk.ConfigValue(cfg, "mintime")) == "" || strings.TrimSpace(sourcecdk.ConfigValue(cfg, "maxtime")) == "" {
			return "", fmt.Errorf("mintime and maxtime are required for %s", family)
		}
	}
	return family, nil
}
