...
func parseSettings(cfg sourcecdk.Config) (settings, error) {
	...
	default:
		return settings, fmt.Errorf("aws family must be one of access_key, asset_metadata, backup_vault, backup_plan, backup_protected_resource, backup_recovery_point, cloudtrail, datasync_location, datasync_task, ebs_snapshot, ebs_volume, ec2_instance, ecr_repository, ecs_service, ecs_task, ecs_task_definition, eks_cluster, eks_nodegroup, eks_fargate_profile, eks_pod_identity_association, effective_permission, iam_group, iam_group_membership, iam_role, iam_role_assignment, iam_role_trust, iam_user, kms_key, lambda_function, public_endpoint, rds_instance, resource_exposure, s3_access_point, s3_bucket, s3_multi_region_access_point, secret, sns_topic, or sqs_queue")
	}
	return settings, nil
}
