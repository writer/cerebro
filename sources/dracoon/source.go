...
const (
	...
	defaultHealthPath      = "/v4/provisioning/webhooks/event_types"
	defaultBaseURLTemplate = "/api"
	tokenHeader            = ""
	tokenScheme            = "Bearer"
	familyEventType        = "event_type"
	familyGroup            = "group"
	familyUser             = "user"
	familyChannel          = "channel"
)

var templateKeys = []string{"customer_id", "oauth_client_reference"}
...
func New() (*Source, error) {
	...
	inner, err := jsonapi.New(spec, jsonapi.Options{
		...
		OAuthScopes:                 []string{"all"},
		OAuthTokenRequestAuthMethod: "client_secret_basic",
		Families: []jsonapi.Family{
			{
				Name:             familyEventType,
				Path:             "/v4/provisioning/webhooks/event_types",
				URNKind:          "dracoon_event_type",
				...
				StaticAttributes: map[string]string{"record_class": "audit_event", "schema": "event_type", "source_system": "dracoon"},
			},
			{
				Name:             familyGroup,
				Path:             "/v4/groups",
				URNKind:          "dracoon_group",
				...
				StaticAttributes: map[string]string{"record_class": "identity_group", "schema": "group", "source_system": "dracoon"},
			},
			{
				Name:             familyUser,
				Path:             "/v4/provisioning/customers/${config.customer_id}/users",
				URNKind:          "dracoon_user",
				IDKeys:           []string{"id", "title", "user_id", "email", "primary_email", "login"},
				PageSizeParams:   []string{"limit"},
				ListKeys:         []string{"items"},
				TimestampKeys:    []string{"observed_at", "updated_at", "last_seen_at", "created_at"},
