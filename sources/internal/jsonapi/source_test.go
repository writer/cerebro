...
func TestReadUsesPageCursorFromOffsetTotals(t *testing.T) {
	...
	source := newCustomTestSource(t, server.URL, Family{
		...
		Config: FamilyConfig{
			...
			OffsetKeys:  []string{"start"},
			StaticQuery: map[string]string{"include_totals": "true"},
			TotalKeys:   []string{"total"},
		},
	})
	first, err := source.Read(context.Background(), sourcecdk.NewConfig(map[string]string{
		"tenant_id": "writer",
...
func TestReadIgnoresPageTotalsWithoutConfiguredResponseKeys(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got := r.URL.Query().Get("page"); got != "0" {
			t.Fatalf("page = %q, want 0", got)
		}
	...
	if pull.NextCursor.GetOpaque() != "1" {
		t.Fatalf("NextCursor = %q, want synthesized page 1", pull.NextCursor.GetOpaque())
	}
}
...
func TestReadStopsWhenHasMoreKeyMissing(t *testing.T) {
	requests := make([]*http.Request, 0, 1)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests = append(requests, r.Clone(r.Context()))
		if got := r.URL.Query().Get("limit"); got != "1" {
