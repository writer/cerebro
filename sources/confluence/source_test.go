...
func TestSourceCheckAndRead(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.RequestURI() == "/wiki/rest/api/user/current" {
	...
	defer server.Close()
	cfgValues := map[string]string{"tenant_id": "tenant", "base_url": server.URL, "family": defaultFamily, "token": "test-token", "site_url": "test-site_url"}
	...
	event := pull.Events[0]
	if event.Kind != "confluence.users" {
