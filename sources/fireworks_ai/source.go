...
const (
	sourceID               = "fireworks_ai"
	defaultFamily          = familyModelDeployments
	defaultHealthPath      = "/v1/models"
	defaultBaseURLTemplate = "https://api.fireworks.ai"
	tokenHeader            = ""
	tokenScheme            = "Bearer"
	familyModelDeployments = "model_deployments"
	familyServiceAccounts  = "service_accounts"
	familyAuditLogs        = "audit_logs"
	familyBillingMetrics   = "billing_metrics"
)
...
func New() (*Source, error) {
	...
	inner, err := jsonapi.New(spec, jsonapi.Options{
		...
		Families: []jsonapi.Family{
			{
				Name:             familyModelDeployments,
				Path:             "/v1/accounts/${config.account_id}/deployments",
				URNKind:          "fireworks_ai_model_deployments",
				...
				StaticAttributes: map[string]string{"record_class": "deployment", "schema": "model_deployments", "source_system": "fireworks_ai"},
			},
			{
				Name:             familyServiceAccounts,
				Path:             "/v1/accounts/${config.account_id}/serviceAccounts",
				URNKind:          "fireworks_ai_service_accounts",
				IDKeys:           []string{"id", "name", "user_id", "email", "primary_email", "login"},
				DisablePageSize:  true,
				ListKeys:         []string{"serviceAccounts"},
				TimestampKeys:    []string{"observed_at", "updated_at", "last_seen_at", "created_at"},
				...
				StaticAttributes: map[string]string{"record_class": "identity_user", "schema": "service_accounts", "source_system": "fireworks_ai"},
			},
			{
				Name:             familyAuditLogs,
				Path:             "/v1/accounts/${config.account_id}/auditLogs",
				URNKind:          "fireworks_ai_audit_logs",
				...
				StaticAttributes: map[string]string{"record_class": "audit_event", "schema": "audit_logs", "source_system": "fireworks_ai"},
			},
			{
				Name:             familyBillingMetrics,
				Path:             "/v1/accounts/${config.account_id}/billing/metrics",
				URNKind:          "fireworks_ai_billing_metrics",
				...
				StaticAttributes: map[string]string{"record_class": "asset", "schema": "billing_metrics", "source_system": "fireworks_ai"},
			},
		},
	})
	if err != nil {
