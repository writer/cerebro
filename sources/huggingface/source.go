...
const (
	...
	defaultHealthPath         = "/whoami-v2"
	defaultBaseURLTemplate    = "https://huggingface.co/api"
	tokenHeader               = ""
	tokenScheme               = "Bearer"
	familyOrganizationMembers = "organization_members"
	familyResourceGroups      = "resource_groups"
	...
	inner, err := jsonapi.New(spec, jsonapi.Options{
		...
		Families: []jsonapi.Family{
			...
			},
			{
				...
				StaticAttributes: map[string]string{"record_class": "identity_group", "schema": "resource_groups", "source_system": "huggingface"},
			},
			{
				Name:             familyAuditLogs,
				Path:             "/organizations/${config.organization}/audit-logs",
				URNKind:          "huggingface_audit_logs",
				IDKeys:           []string{"id", "event_id", "uuid", "request_id"},
				DisablePageSize:  true,
				ListKeys:         []string{"audit_logs"},
				TimestampKeys:    []string{"observed_at", "updated_at", "last_seen_at", "created_at"},
				...
				StaticAttributes: map[string]string{"record_class": "audit_event", "schema": "audit_logs", "source_system": "huggingface"},
			},
			{
				Name:             familyRepositories,
				Path:             "/models",
				URNKind:          "huggingface_repositories",
				IDKeys:           []string{"id", "modelId", "_id"},
				CursorParam:      "cursor",
				LinkHeader:       "Link",
				PageSizeParams:   []string{"limit"},
				TimestampKeys:    []string{"lastModified", "createdAt"},
				Attributes:       map[string]string{"resource_id": "id|modelId", "resource_name": "id|modelId", "source_event_id": "id|modelId"},
				StaticAttributes: map[string]string{"record_class": "asset", "resource_type": "model_repository", "schema": "repositories", "source_system": "huggingface"},
				Config: jsonapi.FamilyConfig{
					...
					EncodeURNID:      true,
					ResourceURNKind:  "huggingface_repositories",
				},
			},
		},
	})
