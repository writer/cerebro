...
export default function SecurityLifecyclePage() {
  ...
  const policyStateCounts = aggregates?.policy_state_counts;
  const expiredCount = lifecyclePolicyStateCount(policyStateCounts, "expired") ?? summary.expired;
  const expiringCount = lifecyclePolicyStateCount(policyStateCounts, "expiring") ?? summary.expiring;
  ...
  const aggregateDetail = aggregates
    ...
    : "On this page";
  const policyAggregateDetail = policyStateCounts
    ? aggregates?.counts_are_exact === false ? "Filtered population estimate" : "Filtered population"
    : "On this page";
  ...
  const coverageReason = lifecycleCoverageReason(coverage);
  const graphChanged = coverageReason === "graph_changed";
  const coveragePresentation = lifecycleCoveragePresentation(coverage);
  const nextPageToken = graphChanged ? "" : lifecycleNextPageToken(query.data);
  const serverPreviousPageToken = graphChanged ? "" : lifecyclePreviousPageToken(query.data);
  const fallbackPreviousToken = graphChanged ? undefined : fallbackPreviousTokens.at(-1);
  const otherPagesAvailable = Boolean(nextPageToken || serverPreviousPageToken || fallbackPreviousToken !== undefined);
  const pageAvailabilityLabel = graphChanged
    ? "Page navigation unavailable"
    : otherPagesAvailable ? "Other pages available" : "Only page";
  ...
  const goBack = () => {
    const previousToken = serverPreviousPageToken || fallbackPreviousToken;
  ...
  return (
    <div>
      <PageHeader
        ...
        action={(
          <button type="button" className="secondary-button px-3 py-2 text-[12px]" onClick={refreshLifecycle} disabled={query.loading}>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Expired" value={expiredCount} detail={policyAggregateDetail} intent={expiredCount > 0 ? "danger" : "success"} state={query.state} />
        <MetricCard
          label="Expiring"
          value={expiringCount}
          detail={policyStateCounts
      ...
      <div className="mt-4 grid gap-px overflow-hidden rounded-lg border border-[color:var(--border)] bg-[color:var(--border)] sm:grid-cols-3">
        ...
        <div className="bg-[var(--surface)] px-4 py-3">
          ...
          {coveragePresentation.detail ? <div className="mt-1 text-[11px] text-[var(--text-muted)]">{coveragePresentation.detail}</div> : null}
      ...
      <div className="mt-4">
        ...
        {records.length > 0 ? (
          <Panel
            ...
            action={<span className="text-[11px] text-[var(--text-muted)]">{pageAvailabilityLabel} · observed {lifecycleExpiryLabel(query.data?.as_of)}</span>}
          >
            <div className="space-y-3 md:hidden" data-lifecycle-mobile-records>
              {records.map((record) => {
                ...
                return (
                  <article
                    key={`${observation.subject_ref.id}:${observation.subject_ref.revision ?? observation.observed_at}:mobile`}
                  ...
                  >
                    ...
                    <div className={`mt-3 ${monoClass}`}>{observation.subject_ref.id}</div>
                    <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 text-[12px]">
                      <div>
                        <dt className={labelClass}>Expires</dt>
                        <dd className="mt-1 text-[var(--text-secondary)]">{lifecycleExpiryLabel(observation.expires_at)}</dd>
                      </div>
                      <div>
                        <dt className={labelClass}>Owner</dt>
                        <dd className="mt-1 text-[var(--text-secondary)]">{lifecycleOwnerLabel(observation.owner_urn)}</dd>
                      </div>
                      <div>
                      ...
                      <div>
                        <dt className={labelClass}>Route</dt>
                    ...
                    <div className="mt-4 border-t border-[color:var(--border)] pt-3">
                      ...
                      <div className={`mt-1 ${monoClass}`}>{observation.subject_ref.revision || "Not reported"}</div>
                      <button type="button" className="secondary-button mt-3 w-full px-3 py-2 text-[12px]" onClick={() => setSelectedRef(observation.subject_ref.id)}>
            ...
            <div className="hidden overflow-auto md:block" data-lifecycle-desktop-table>
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-[color:var(--border)] pt-4">
              ...
              <div className="flex gap-2">
                <button type="button" className="secondary-button px-3 py-2 text-[12px]" onClick={goBack} disabled={!serverPreviousPageToken && fallbackPreviousToken === undefined}>
                  Previous page
                </button>
