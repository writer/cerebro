...
import { createScheduledOccurrence } from "./schedules.js";
...
/**
 * Portable durable definition for one scheduled unit of work.
 *
 * The work reference and digest are opaque. Hosts own payload storage,
 * destination binding, persistence, and deployment-specific scheduling.
 */
...
export interface ScheduleOccurrencePlanRequest {
  created_at: string;
...
export interface ScheduleOccurrencePlan {
  due_at: string[];
  occurrences: ScheduledOccurrenceV1[];
  truncated: boolean;
}
...
/** Validate and normalize a definition before a durable store accepts it. */
export function normalizeScheduleDefinition(
  ...
  requirePositiveInteger(definition.generation, "generation");
  if (
  ...
  if (
    definition.misfire_policy !== "skip" &&
    definition.misfire_policy !== "coalesce_once" &&
    definition.misfire_policy !== "run_all_bounded"
  ) {
    throw new ScheduleDefinitionInputError("misfire_policy is invalid");
  }
...
/**
 * Materialize deterministic occurrences for a bounded planning window.
 * The interval cadence is anchored to durable state, so restarts and deploys
 * cannot move its future due times.
 */
export function planScheduleOccurrences(
  request: ScheduleOccurrencePlanRequest,
): ScheduleOccurrencePlan {
  const definition = normalizeScheduleDefinition(request.definition);
  const createdAt = normalizeTime(request.created_at, "created_at");
  ...
  if (Date.parse(endInclusive) - Date.parse(startExclusive) > MAX_WINDOW_MS) {
    throw new ScheduleDefinitionInputError(
      "schedule planning window exceeds 366 days",
    );
  }

  const limit = request.max_occurrences ?? DEFAULT_MAX_OCCURRENCES;
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > MAX_OCCURRENCES) {
    throw new ScheduleDefinitionInputError(
      `max_occurrences must be between 1 and ${MAX_OCCURRENCES}`,
    );
  }
  if (definition.state !== "active") {
    return { due_at: [], occurrences: [], truncated: false };
  }
  ...
  const dueAt = candidates.slice(0, limit);
  return {
    due_at: dueAt,
    occurrences: dueAt.map((due) =>
...
function normalizeCadence(cadence: ScheduleCadenceV1): ScheduleCadenceV1 {
  switch (cadence.kind) {
    ...
    default:
      throw new ScheduleDefinitionInputError("cadence kind is invalid");
  }
}
