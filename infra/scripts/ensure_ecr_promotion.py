...
def _ecr_image_uri(registry_id: str, region: str, repository_name: str, tag: str) -> str:
...
def _describe_ecr_image(image: RequiredImage, *, repository_name: str, region: str) -> EcrImage:
    completed = _run(
        [
            "aws",
            "ecr",
    ...
    if completed.returncode != 0:
        ...
        raise RuntimeError(f"failed to describe ECR image {image.tag}: {completed.stderr.strip()}")
    ...
    if not digest or not registry_id:
        raise RuntimeError(f"describe-images response for {image.tag} did not include registryId and imageDigest")
    return EcrImage(
        label=image.label,
        tag=image.tag,
        digest=digest,
...
def _find_missing_images(images: list[RequiredImage], *, repository_name: str, region: str) -> tuple[list[EcrImage], list[RequiredImage]]:
    found: list[EcrImage] = []
    missing: list[RequiredImage] = []
    for image in images:
        try:
            found.append(_describe_ecr_image(image, repository_name=repository_name, region=region))
...
def _dispatch_workflow(args: argparse.Namespace) -> datetime:
    if not args.github_repository:
        raise RuntimeError("cannot dispatch promotion recovery without GITHUB_REPOSITORY or --github-repository")
...
def _wait_for_dispatched_workflow(args: argparse.Namespace, started_at: datetime) -> str:
    ...
    if matched_run is None:
        raise RuntimeError(f"timed out waiting for {args.workflow} workflow_dispatch run to appear")
    ...
    while time.monotonic() < deadline:
        ...
        if status == "completed":
            if conclusion == "success":
                print(f"::notice::Promotion recovery workflow completed successfully: {run_url}")
                return run_url
            raise RuntimeError(f"promotion recovery workflow completed with conclusion {conclusion}: {run_url}")
        time.sleep(args.poll_seconds)
    raise RuntimeError(f"timed out waiting for promotion recovery workflow to complete: {run_url}")
    ...
    *,
    ...
    payload = {
        ...
        "workflowRun": {
            ...
            "url": f"{server_url}/{repository}/actions/runs/{run_id}" if repository and run_id else "",
        },
...
def _write_github_output(path: Path | None, *, receipt: Path | None, recovered: bool, images: list[EcrImage]) -> None:
...
def _missing_image_message(missing: list[RequiredImage], args: argparse.Namespace) -> str:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Verify ECR image promotion and optionally dispatch CI recovery before deploy.")
    parser.add_argument("--stack-file", type=Path, required=True)
    parser.add_argument("--repository-name", default=os.environ.get("ECR_REPOSITORY", DEFAULT_REPOSITORY))
    parser.add_argument("--region", default=os.environ.get("AWS_REGION", DEFAULT_REGION))
    parser.add_argument("--receipt-output", type=Path, help="Write a JSON promotion receipt for the verified ECR image tags.")
    parser.add_argument("--github-output", type=Path, help="Write GitHub Actions step outputs.")
    parser.add_argument("--dispatch-if-missing", action="store_true", help="Dispatch CI and wait when required ECR image tags are missing.")
    parser.add_argument("--workflow", default="ci.yml", help="Promotion workflow file to dispatch for missing-image recovery.")
    parser.add_argument("--workflow-ref", default=os.environ.get("PROMOTION_WORKFLOW_REF", "main"))
    ...
    found, missing = _find_missing_images(required_images, repository_name=args.repository_name, region=args.region)
    recovered = False
    ...
    _write_github_output(args.github_output, receipt=args.receipt_output, recovered=recovered, images=found)
    print(
        f"Verified ECR promotion for {_stack_name(args.stack_file)}: "
        + ", ".join(f"{image.label} {image.image_uri}@{image.digest}" for image in found)
    )
    return 0
