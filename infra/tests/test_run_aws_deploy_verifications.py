...
class RunAwsDeployVerificationsTest(unittest.TestCase):
    ...
    def test_graph_command_exit_degrades_when_allowed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            output_path = Path(temp_dir) / "outputs.txt"
            diagnostics = "ERROR: graph health command exited with 1"
            with patch(
                "scripts.run_aws_deploy_verifications._stream_graph_health",
                return_value=run_aws_deploy_verifications.GraphHealthResult(1, diagnostics),
            ):
                status = run_aws_deploy_verifications.main(
                    [
                        "--stack-file",
