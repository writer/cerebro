...
class NatsContainerDefinitionTest(unittest.TestCase):
    def test_nats_containers_are_hardened(self) -> None:
        ...
        self.assertIn("nats_js stream add \"$STREAM_NAME\" \"$@\" --storage file --retention limits --defaults", bootstrap_command)
        self.assertIn("nats_js stream add \"$STREAM_NAME\" \"$@\" --storage file --retention limits --defaults || nats_js", bootstrap_command)
    ...
    def test_extra_client_security_group_ids_open_nats_client_port(self) -> None:
        ingress_args: list[dict] = []
        ...
        def fake_ingress_args(**kwargs):
            ingress_args.append(kwargs)
            return fake_args(**kwargs)
        ...
        with ExitStack() as stack:
            for patcher in [
                ...
                patch.object(nats.aws.servicediscovery, "ServiceDnsConfigArgs", side_effect=fake_args),
                patch.object(nats.aws.servicediscovery, "ServiceDnsConfigDnsRecordArgs", side_effect=fake_args),
                patch.object(nats.aws.servicediscovery, "ServiceHealthCheckCustomConfigArgs", side_effect=fake_args),
                patch.object(nats.aws.ec2, "SecurityGroup", side_effect=fake_resource),
                patch.object(nats.aws.ec2, "SecurityGroupIngressArgs", side_effect=fake_ingress_args),
                patch.object(nats.aws.ec2, "SecurityGroupEgressArgs", side_effect=fake_args),
                patch.object(nats.storage, "create_efs_volume", side_effect=fake_storage),
                patch.object(nats.aws.ecs, "Cluster", side_effect=fake_resource),
                patch.object(nats.aws.ecs, "ClusterSettingArgs", side_effect=fake_args),
            ...
            nats.create_nats_service(
                ...
                kms_key_arn="kms-key",
                efs_throughput_mode="elastic",
                extra_client_security_group_ids=["sg-mender"],
            )

        client_ingress = [args for args in ingress_args if args.get("from_port") == 4222]
        monitor_ingress = [args for args in ingress_args if args.get("from_port") == 8222]
        self.assertEqual(len(client_ingress), 1)
        self.assertEqual(client_ingress[0]["security_groups"], ["sg-app", "sg-mender"])
        self.assertEqual(len(monitor_ingress), 1)
        self.assertEqual(monitor_ingress[0]["security_groups"], ["sg-app"])
