...
class WafOAuthLoopbackAllowRuleTest(unittest.TestCase):
    def test_loopback_oauth_allow_rules_precede_managed_rules(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                "rate-limit",
                "allow-mcp-oauth-loopback-authorize",
                "allow-mcp-oauth-loopback-register",
                "aws-common-rules",
                "aws-bad-inputs",
                "aws-sqli",
            ],
        )
        self.assertLess(rules[1].priority, rules[3].priority)
        self.assertLess(rules[2].priority, rules[3].priority)
