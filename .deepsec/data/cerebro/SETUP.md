# Agent setup for `cerebro`

This is a deepsec scanning workspace. Project `cerebro` was just registered
(target: `..`). Setup context has been bootstrapped in `data/cerebro/INFO.md`.
...
2. **Fill in `data/cerebro/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id cerebro
pnpm deepsec process --project-id cerebro
```
