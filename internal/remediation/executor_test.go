...
func TestExecutor_ApproveUsesSnapshottedRuleWhenRuleMutates(t *testing.T) {
	engine := NewEngine(testutil.Logger())
	rule := Rule{
		ID:      "enable-bucket-default-encryption-snapshot-approval",
		Name:    "Enable Bucket Default Encryption Snapshot Approval",
		Enabled: true,
		Trigger: Trigger{Type: TriggerManual},
		Actions: []Action{{
			Type: ActionEnableBucketDefaultEncryption,
	...
	snapshot, ok := remediationRuleFromTriggerData(execution.TriggerData)
	if !ok {
		t.Fatal("expected rule snapshot in trigger data")
	}
	if got := snapshot.Actions[0].Config["delivery_mode"]; got != "remote_apply" {
		t.Fatalf("snapshot delivery_mode = %q, want remote_apply", got)
	}

	liveRule, ok := engine.GetRule(rule.ID)
	if !ok {
		t.Fatalf("expected live rule %s", rule.ID)
	}
	liveRule.Actions = []Action{{
		Type: ActionEnableBucketDefaultEncryption,
		Config: map[string]string{
			"approval_mode": "required",
			"delivery_mode": "terraform",
	...
	if artifact, _ := execution.Actions[0].Metadata["artifact"].(map[string]any); len(artifact) > 0 {
		t.Fatalf("expected remote approval to avoid terraform artifact, got %#v", artifact)
	}
}
