...
func TaskToolDefinitions(limits TaskToolLimits) []TaskToolDefinition {
	...
	return []TaskToolDefinition{
		...
		{Name: "cerebro.evidence.packet", Title: "Check Agent Run Readiness", Description: "Build caller-scoped readiness and guardrail metadata from current coverage and verifier contracts. This artifact does not resolve or persist authoritative evidence; use cerebro.decision.packet for canonical resolved decision context. The tool never approves or executes an action and reports blocked or partial task states." + readScope, InputSchema: objectSchema(map[string]any{
		...
		{Name: "cerebro.sources.health", Title: "Report Source Health", Description: "Identify broken integrations, connector blind spots, source coverage gaps, stale data, failed runtimes, and unsupported dimensions. Missing runtime state returns a partial task state." + readScope, InputSchema: objectSchema(nil, nil)},
		{Name: "cerebro.action.plan", Title: "Plan Remediation Actions", Description: "Decide what to fix next by ranking non-mutating remediation candidates from authorized findings. Graph context is optional and its absence returns a partial task state; execution remains a separate approval-gated operation." + readScope, InputSchema: objectSchema(map[string]any{
