...
func TaskToolDefinitions(limits TaskToolLimits) []TaskToolDefinition {
	...
	return []TaskToolDefinition{
		...
		{Name: "cerebro.sources.health", Title: "Report Source Health", Description: "Identify connector blind spots, source coverage gaps, stale data, failed runtimes, and unsupported dimensions. Missing runtime state returns a partial task state." + readScope, InputSchema: objectSchema(nil, nil)},
