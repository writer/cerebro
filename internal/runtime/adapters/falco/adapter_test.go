...
func TestAdapterNormalizeNetworkAlert(t *testing.T) {
	raw := []byte(`{
		...
		"output_fields": {
			...
			"container.image.repository": "ghcr.io/upstream/payments",
	...
	if observation.ImageRef != "ghcr.io/upstream/payments:1.2.3" {
		t.Fatalf("ImageRef = %q, want ghcr.io/upstream/payments:1.2.3", observation.ImageRef)
	}
