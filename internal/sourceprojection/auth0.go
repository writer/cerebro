...
func auth0RolesProjections(event *cerebrov1.EventEnvelope) ([]*ports.ProjectedEntity, []*ports.ProjectedLink, error) {
	...
	legacyEvent := *event
	legacyEvent.Attributes = auth0LegacyRoleGroupAttributes(attributes, roleID)
	...
	legacyGroupURN := identityGroupURN(tenantID, auth0IdentityProfile.Provider, legacyEvent.Attributes["group_id"], legacyEvent.Attributes["group_email"])
	if legacyGroupURN != "" {
