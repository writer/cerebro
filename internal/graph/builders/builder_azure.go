			...
			parse: func(rows []map[string]any) []*Node {
				...
				for _, kv := range rows {
					...
					nodes = append(nodes, &Node{
						...
						TenantID: queryRowString(kv, "tenant_id"),
						Risk:     RiskHigh,
						Properties: map[string]any{
	...
	b.loadAzurePreferredIdentityNodes(ctx, []nodeQuery{
		{
			...
			query: `SELECT id, display_name, app_id, service_principal_type, account_enabled, app_owner_organization_id, app_role_assignment_required, publisher_name, verified_publisher_display_name, verified_publisher_id, verified_publisher_added_datetime, created_date_time, tags, subscription_id FROM azure_graph_service_principals`,
			parse: parseAzureServicePrincipalNodes,
		},
		{
			table: "entra_service_principals",
			query: `SELECT id, display_name, app_id, service_principal_type, account_enabled, app_owner_organization_id, app_role_assignment_required, publisher_name, verified_publisher_display_name, verified_publisher_id, verified_publisher_added_datetime, created_datetime, tags FROM entra_service_principals`,
			parse: parseAzureServicePrincipalNodes,
		},
		{
...
func parseAzureServicePrincipalNodes(rows []map[string]any) []*Node {
	...
	for _, sp := range rows {
		...
		properties := map[string]any{
			...
			"verified_publisher_display_name":   queryRow(sp, "verified_publisher_display_name"),
			"verified_publisher_id":             queryRow(sp, "verified_publisher_id"),
			"verified_publisher_added_datetime": queryRow(sp, "verified_publisher_added_datetime"),
