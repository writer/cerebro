...
func (g *Graph) addNodeLocked(node *Node) bool {
	...
	wasActive := false
	var existing *Node
	removedProperties := make(map[string]struct{})
	if current, ok := g.nodes[node.ID]; ok && current != nil {
		...
		for property := range existing.PropertyMap() {
			if _, stillPresent := node.PropertyMap()[property]; !stillPresent {
				removedProperties[property] = struct{}{}
			}
		}
	...
	for property := range removedProperties {
		g.appendNodePropertyTombstoneLocked(node, property, node.UpdatedAt)
	}
