...
func TestGenerateWritesSourceRuntimeSDKScaffold(t *testing.T) {
	...
	for _, path := range []string{
		...
		if _, err := os.Stat(filepath.Join(outputDir, path)); !errors.Is(err, os.ErrNotExist) {
			t.Fatalf("deprecated artifact %s exists or returned an unexpected error: %v", path, err)
		}
	}
...
func TestGenerateDefinitionWritesIdentitySource(t *testing.T) {
	...
	if !strings.Contains(sourceTest, `sourcefixture.NewCatalogSource(".", defaultFamily)`) {
		t.Fatalf("definition generated source test does not use the shared catalog fixture loader:\n%s", sourceTest)
	}
