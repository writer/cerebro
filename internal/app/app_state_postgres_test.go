...
func TestAppStateDatabaseURLUsesDatabaseURLOnly(t *testing.T) {
	testCases := []struct {
		name string
		cfg  *Config
		want string
		...
		{
			name: "snowflake backend still uses database url for app state",
			cfg: &Config{
				WarehouseBackend: "snowflake",
				DatabaseURL:      "postgres://app-state",
			},
		...
		{
			name: "database url wins over job database url",
			cfg: &Config{
				WarehouseBackend: "postgres",
				DatabaseURL:      "postgres://app-state",
				JobDatabaseURL:   "postgres://jobs",
