...
func (a *App) ReloadSecrets(ctx context.Context) error {
	...
	if !apiCredentialsChanged && !postgresChanged && !snowflakeChanged && !providersChanged && !aiAgentsChanged && !agentSCMChanged && !agentToolingChanged {
		return nil
	}
	...
	if providersChanged || postgresChanged || snowflakeChanged {
...
func (a *App) rebindAgentSessionStore(ctx context.Context, activeSnowflake *snowflake.Client) {
	...
	case a.PostgresDB != nil:
		store := agents.NewPostgresSessionStore(a.PostgresDB)
...
func (a *App) rotatePostgresClient(ctx context.Context, cfg *Config) error {
	...
	newDB, err := sql.Open("postgres", preparedDSN)
	if err != nil {
...
func (a *App) rotateSnowflakeClient(ctx context.Context, cfg *Config) error {
	...
	if !hasSnowflakeCredentials(cfg) {
		if !strings.EqualFold(strings.TrimSpace(cfg.WarehouseBackend), "snowflake") {
			...
			if warehouseClient, ok := a.Warehouse.(*snowflake.Client); ok && (warehouseClient == oldClient || warehouseClient == oldLegacyClient) {
				a.Warehouse = nil
			}
			a.Snowflake = nil
