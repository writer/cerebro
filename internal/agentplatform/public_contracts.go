...
func EventSubscriptions() EventSubscriptionContract {
	return EventSubscriptionContract{
		...
		EventTypes: []EventSubscriptionType{
			...
			{Name: EventTypeComplianceAssuranceDecisionRecorded, Family: "compliance", Description: "An immutable assurance decision was recorded for one assessment result.", PayloadFields: []string{"event_id", "tenant_id", "decision_id", "assessment_run_id", "objective_result_id", "qualified", "decision_digest", "record_digest", "occurred_at"}},
			{Name: EventTypeComplianceAssessmentSnapshotRecorded, Family: "compliance", Description: "An immutable assessment snapshot bound a completed result set, decision cutoff, and evidence set.", PayloadFields: []string{"event_id", "tenant_id", "snapshot_id", "assessment_run_id", "result_set_digest", "decision_set_digest", "evidence_set_digest", "occurred_at"}},
			{Name: EventTypeComplianceWorkItemUpdated, Family: "compliance", Description: "A canonical compliance work item changed state or received another assessment occurrence.", PayloadFields: []string{"event_id", "tenant_id", "work_item_id", "state", "version", "occurrence_count", "occurred_at"}},
			{Name: EventTypeComplianceWorkItemVerified, Family: "compliance", Description: "A canonical compliance work item was verified against a qualified post-remediation assurance decision.", PayloadFields: []string{"event_id", "tenant_id", "work_item_id", "assurance_decision_id", "assessment_run_id", "objective_result_id", "decision_digest", "occurred_at"}},
