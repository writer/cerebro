...
var Capabilities = []Capability{
	{
		...
		Eval: EvalStatus{
			Required:      true,
			Status:        "required",
			LocalCommands: []string{"go test ./internal/graphagent -run 'TestAskTrajectoryGoldenEvals|TestScoreAskEvents' -count=1"},
			ScenarioSets:  []string{"ask-golden", "ask-adversarial", "graph-ask-grounded-regression"},
			Rubrics:       []string{"groundedness", "source use", "safe refusal", "answer completeness", "trajectory quality score"},
		},
	...
	},
	{
		...
		Eval: EvalStatus{
			Required:      true,
			Status:        "required",
			LocalCommands: []string{"go test ./internal/graphactions ./internal/graphactionworkflow ./internal/observability ./internal/bootstrap -run 'Test.*GraphAction|TestRecord.*GraphAction'"},
			ScenarioSets:  []string{"graph-action-execution-safety"},
			Rubrics:       []string{"finding-scoped target", "allowed action policy", "provider action ref", "reconciliation status", "provider-neutral observability"},
		},
		RuntimeEvents: []string{"adapter.execution.started", "adapter.execution.completed", "adapter.execution.failed", "graph_action.recorded"},
		Provenance:    []string{"runtime-response-action", "graph-action-record"},
		Review: ReviewStatus{
			State:             "required",
	...
	},
	{
		...
		Risk:            "medium",
		DefaultOn:       true,
		Summary:         "Turns OpenAI, Anthropic, Bedrock, SageMaker, and Azure AI usage, tool permissions, model permissions, and deployment posture into graph-grounded governance evidence.",
		ConsoleSurfaces: []string{"Source coverage", "findings", "Ask console", "Agent platform API"},
		RequiredScopes:  []string{ScopeCosmoSecurityRead},
		Eval: EvalStatus{
			Required:      true,
			Status:        "required",
			LocalCommands: []string{"go test ./internal/agentplatform -run TestRunSecurityAgentEvalSuiteFixture"},
			ScenarioSets:  []string{"ai-governance-posture"},
			Rubrics:       []string{"AI source coverage", "model permission risk", "hosted tool exposure", "deployment posture"},
		},
		RuntimeEvents: []string{"capability.selected", "knowledge.retrieval.completed", "verifier.completed", "agent.run.completed"},
		Provenance:    []string{"ai-governance-posture", "source-coverage", "graph-provenance"},
		Review: ReviewStatus{
			State:             "required",
			Cadence:           "before AI provider source, posture, or finding rule changes",
			RequiredApprovers: []string{"platform", "security"},
		},
	},
	{
		...
		Risk:            "high",
		DefaultOn:       true,
		Summary:         "Verifies each agent conclusion as a typed claim with supporting evidence, counterevidence, missing evidence, freshness, coverage caveats, and an allowed next action stage.",
		ConsoleSurfaces: []string{"Agent platform API", "MCP", "trace drawer", "findings"},
		RequiredScopes:  []string{ScopeCosmoSecurityRead},
		Eval: EvalStatus{
			Required:      true,
			Status:        "required",
			LocalCommands: []string{"go test ./internal/agentplatform -run TestBuildClaimVerification"},
			ScenarioSets:  []string{"claim-verification", "stale-data-refusal", "false-positive-suppression"},
			Rubrics:       []string{"supporting evidence", "counterevidence handling", "freshness caveats", "coverage caveats", "action stage gating"},
		},
		RuntimeEvents: []string{"claim.verification.completed", "verifier.completed", "agent.run.completed", "agent.run.failed"},
		Provenance:    []string{"claim-verification", "verifier-result", "evidence-record", "source-coverage"},
		Review: ReviewStatus{
			State:             "required",
			Cadence:           "before claim verdict, promotion, or action-stage gate changes",
			RequiredApprovers: []string{"platform", "security"},
		},
	},
	{
		...
		DomainID:        "streaming-replay",
		Kind:            "work-ledger-contract",
		Version:         "1.0.0",
		Owner:           "cerebro-platform",
		Risk:            "medium",
		DefaultOn:       true,
		Summary:         "Defines durable agent investigation work objects that carry objectives, hypotheses, claims, tool calls, verifier results, action proposals, approvals, verification checks, and closure.",
		ConsoleSurfaces: []string{"Agent platform API", "MCP", "trace drawer", "workflow replay"},
		RequiredScopes:  []string{ScopeCosmoSecurityRead},
		Eval: EvalStatus{
			Required:      true,
			Status:        "required",
			LocalCommands: []string{"go test ./internal/agentplatform -run TestSecurityControlPlaneSnapshot"},
			ScenarioSets:  []string{"agent-work-ledger", "trace-replay", "security-memory"},
			Rubrics:       []string{"state continuity", "claim linkage", "verifier linkage", "closure conditions", "memory scope"},
		},
		RuntimeEvents: []string{"agent.work.created", "agent.work.updated", "claim.verification.completed", "agent.work.closed", "agent.run.completed", "agent.run.failed"},
		Provenance:    []string{"agent-work", "claim-verification", "replay-record", "security-memory"},
		Review: ReviewStatus{
			State:             "required",
			Cadence:           "before agent work state, closure, or replay semantics change",
			RequiredApprovers: []string{"platform", "security"},
		},
	},
}
...
var RuntimeEvents = []RuntimeEvent{
	...
	{
		Name:             "claim.verification.completed",
		DomainID:         "capabilities",
		Stage:            "verification",
		SequenceRequired: true,
		Replayable:       true,
		PayloadFields:    []string{"claim", "verdict", "allowed_next_stage", "supporting_evidence", "counter_evidence", "missing_evidence"},
		ProvenanceFields: []string{"tenant_id", "scope", "evidence_urn", "citation_status", "verifier_id"},
	},
	{
		Name:             "agent.work.created",
		DomainID:         "streaming-replay",
		Stage:            "work_start",
		SequenceRequired: true,
		Replayable:       true,
		PayloadFields:    []string{"work_id", "tenant_id", "objective", "current_hypothesis", "trace_ids"},
		ProvenanceFields: []string{"work_id", "tenant_id", "trace_id"},
	},
	{
		...
		SequenceRequired: true,
		Replayable:       true,
		PayloadFields:    []string{"work_id", "status", "claim_ids", "evidence_urns", "verifier_results", "action_proposals"},
		ProvenanceFields: []string{"work_id", "claim_id", "evidence_urn", "verifier_id"},
	},
	{
		Name:             "agent.work.closed",
		DomainID:         "streaming-replay",
		Stage:            "terminal",
		SequenceRequired: true,
		Terminal:         true,
		Replayable:       true,
		PayloadFields:    []string{"work_id", "terminal_outcome", "closure_reason", "verified_claims", "memory_writes"},
		ProvenanceFields: []string{"work_id", "terminal_outcome", "claim_id", "memory_id"},
	},
	{
	...
	{
		Name:             "graph_action.recorded",
		DomainID:         "execution",
		Stage:            "workflow_link",
		SequenceRequired: true,
		Replayable:       true,
		PayloadFields:    []string{"finding_id", "action_id", "provider", "action", "target_urn", "external_status", "dry_run"},
		ProvenanceFields: []string{"scope", "action_id", "provider", "action", "reconciliation_status"},
	},
	{
...
var ProvenanceRequirements = []ProvenanceRequirement{
	...
	{
		Surface:          "graph-action-record",
		DomainID:         "execution",
		RequiredFields:   []string{"finding_id", "action_id", "provider", "action", "target_urn", "external_status", "reconciliation_status"},
		CitationRequired: false,
		BudgetRequired:   false,
		FallbackRequired: false,
	},
	...
	{
		Surface:          "ai-governance-posture",
		DomainID:         "capabilities",
		RequiredFields:   []string{"source_id", "tenant_id", "model_permission", "tool_permission", "usage_signal", "deployment_posture"},
		CitationRequired: true,
		BudgetRequired:   true,
		FallbackRequired: true,
	},
	...
	},
	{
		...
		DomainID:         "capabilities",
		RequiredFields:   []string{"tenant_id", "claim", "verdict", "allowed_next_stage", "evidence_urn", "citation_status", "verifier_result"},
		CitationRequired: true,
		BudgetRequired:   true,
		FallbackRequired: true,
	},
	{
		Surface:          "agent-work",
		DomainID:         "streaming-replay",
		RequiredFields:   []string{"work_id", "tenant_id", "objective", "status", "claim_id", "trace_id", "closure_reason"},
		CitationRequired: false,
		BudgetRequired:   true,
		FallbackRequired: true,
	},
