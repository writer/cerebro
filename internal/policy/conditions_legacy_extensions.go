...
func parseNullCondition(condition string) (string, bool, bool) {
	if parts := splitTopLevelFold(condition, " IS NOT NULL"); len(parts) == 2 && strings.TrimSpace(parts[1]) == "" {
		field := strings.TrimSpace(parts[0])
		return field, true, field != ""
	}
	if parts := splitTopLevelFold(condition, " IS NULL"); len(parts) == 2 && strings.TrimSpace(parts[1]) == "" {
		field := strings.TrimSpace(parts[0])
...
func parseEndsWithCondition(condition string) (string, string, bool, bool) {
	if parts := splitTopLevelFold(condition, " NOT ENDS_WITH "); len(parts) == 2 {
		field := strings.TrimSpace(parts[0])
		suffix := strings.TrimSpace(parts[1])
		return field, suffix, true, field != "" && suffix != ""
	}
	if parts := splitTopLevelFold(condition, " ENDS_WITH "); len(parts) == 2 {
		field := strings.TrimSpace(parts[0])
		suffix := strings.TrimSpace(parts[1])
		return field, suffix, false, field != "" && suffix != ""
	}
	return "", "", false, false
}

func parseReferenceBucketWithPublicAccessCondition(condition string) (string, bool) {
	if parts := splitTopLevelFold(condition, " REFERENCES BUCKET WITH PUBLIC ACCESS"); len(parts) == 2 && strings.TrimSpace(parts[1]) == "" {
		field := strings.TrimSpace(parts[0])
		return field, field != ""
	}
	return "", false
...
func resolveComparisonExpectedValue(asset map[string]interface{}, expected string) interface{} {
	trimmed := strings.TrimSpace(expected)
	if trimmed == "" {
		return trimmed
	}
	if looksLikeFieldReference(trimmed) {
		return getNestedValue(asset, trimmed)
	}
	return trimmed
}

func looksLikeFieldReference(value string) bool {
	...
	if _, ok := parseRelativeTimeExpression(value); ok {
		return false
	}
...
func shouldTreatContainsInnerAsCondition(condition string) bool {
	condition = strings.TrimSpace(condition)
	if condition == "" {
		return false
	}
	if _, _, _, ok := parseAnyCondition(condition); ok {
		return true
	}
	if _, _, _, ok := parseInCondition(condition); ok {
		return true
	}
	if _, _, _, ok := parseMatchesCondition(condition); ok {
		return true
	}
	if _, _, _, ok := parseContainsCondition(condition); ok {
		return true
	}
	if _, _, _, ok := parseComparisonCondition(condition); ok {
		return true
	}
	if _, _, ok := parseNullCondition(condition); ok {
		return true
	}
	if _, _, _, ok := parseEndsWithCondition(condition); ok {
		return true
	}
	if _, ok := parseReferenceBucketWithPublicAccessCondition(condition); ok {
		return true
	}
	...
	return strings.HasSuffix(lower, " not exists") || strings.HasSuffix(lower, " exists")
}

func parseLegacyObjectLiteral(value string) (map[string]interface{}, bool) {
	value = strings.TrimSpace(value)
	...
	entries := splitObjectLiteralEntries(body)
	if len(entries) == 0 {
		return nil, false
	}
	...
	for _, entry := range entries {
		key, rawValue, ok := splitObjectLiteralEntry(entry)
		if !ok {
			return nil, false
		}
		parsedValue, ok := parseLegacyLiteralValue(rawValue)
		if !ok {
			return nil, false
		}
...
func splitObjectLiteralEntries(body string) []string {
	var entries []string
	start := 0
	depth := 0
	inSingle := false
...
func splitObjectLiteralEntry(entry string) (string, string, bool) {
	inSingle := false
	inDouble := false
	depth := 0
...
func parseLegacyLiteralValue(value string) (interface{}, bool) {
	value = strings.TrimSpace(value)
	if value == "" {
		return nil, false
	}

	if mapped, ok := parseLegacyObjectLiteral(value); ok {
		return mapped, true
	}
	if isQuoted(value) {
