...
func TestWizPublicVMPolicies(t *testing.T) {
	engine := NewEngine()

	// Load a subset of wiz policies for testing
	engine.AddPolicy(&Policy{
		ID:          "wiz-vm-public-admin",
...
func TestWizSecretsDetection(t *testing.T) {
	engine := NewEngine()

	engine.AddPolicy(&Policy{
		ID:          "wiz-resource-cleartext-keys-privileged",
...
func TestWizIdentityPolicies(t *testing.T) {
	engine := NewEngine()

	engine.AddPolicy(&Policy{
		ID:          "wiz-aws-user-inactive-admin-no-mfa",
...
func TestWizVulnerabilityPolicies(t *testing.T) {
	engine := NewEngine()

	engine.AddPolicy(&Policy{
		ID:          "wiz-log4shell-public",
...
func TestWizStoragePolicies(t *testing.T) {
	engine := NewEngine()

	engine.AddPolicy(&Policy{
		ID:          "wiz-bucket-public-sensitive",
...
func TestWizKubernetesPolicies(t *testing.T) {
	engine := NewEngine()

	engine.AddPolicy(&Policy{
		ID:          "wiz-k8s-wildcard-permissions",
...
func TestLoadWizPoliciesFromDisk(t *testing.T) {
	...
	if len(policies) == 0 {
		t.Error("expected wiz policies to be loaded")
	}

	// Verify specific policies exist
	testCases := []string{
		"wiz-vm-public-admin",
		"wiz-bucket-public-sensitive",
		"wiz-log4shell-public",
		"wiz-aws-user-inactive-admin-no-mfa",
	}
