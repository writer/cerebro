...
func TestVerifyRemediationMilestoneRejectsClosedPlanWithoutMutation(t *testing.T) {
	now := time.Date(2026, 7, 11, 12, 0, 0, 0, time.UTC)
	plan, err := NewRemediationPlan(RemediationPlanInput{
		ID: "plan-a", TenantID: "tenant-a", ProgramID: "program-a", ScopeRevisionID: "scope-r1", RiskID: "risk-a", WorkItemID: "work-a",
		Treatment: RiskTreatmentMitigate, OwnerID: "plan-owner", TargetAt: now.Add(30 * 24 * time.Hour), VerificationRequired: false,
		Milestones: []RemediationMilestoneInput{
			{ID: "milestone-a", Title: "Deploy change", OwnerID: "implementer-a", TargetAt: now.Add(7 * 24 * time.Hour), PlannedAction: "Deploy the control change."},
		},
		CreatedAt: now,
	...
	_, err = VerifyRemediationMilestone(closed, closed.Version, "milestone-a", []string{"verify-a"}, "reviewer-a", now.Add(5*time.Hour))
	...
	if !reflect.DeepEqual(closed, before) {
		t.Fatalf("VerifyRemediationMilestone(closed) mutated input:\n got  %+v\n want %+v", closed, before)
	}
}
