...
func (r *coordinationGraphRule) EvaluateRows(_ context.Context, runtime *cerebrov1.SourceRuntime, rows []ports.CypherRow) ([]*ports.FindingRecord, error) {
	...
	for _, row := range rows {
		...
		findings = append(findings, &ports.FindingRecord{
			...
			FindingPersistenceEnvelope: ports.FindingPersistenceEnvelope{GraphEvidenceRows: coordinationGraphEvidenceRows(primaryURN, cypherRowString(row, "primary_label"), cypherRowString(row, "primary_type"), row)},
