...
var kolideHostFailingComplianceChecksDefinition = RuleDefinition{
	...
	SourceID:           "kolide",
	EventKinds:         []string{"kolide.device"},
	...
	FingerprintFields:  []string{"kolide_device_urn"},
...
// CloseOnEvent resolves an open finding when a later host snapshot shows all
// compliance checks passing (remediation) or when the host is no longer managed
// by Kolide (deprovisioned/offboarded), so removed hosts do not leave stale open
// findings.
func (r *kolideHostFailingComplianceChecksRule) CloseOnEvent(event Event) (string, bool) {
	if !kolideHostFailingComplianceChecksKindMatcher(event) || !hasRequiredAttributes(event, kolideHostFailingComplianceChecksDefinition.RequiredAttributes...) {
		return "", false
	}
...
func matchesKolideHostFailingComplianceChecks(event *cerebrov1.EventEnvelope) bool {
	...
	return kolideHostChecksFailing(eventAttributes(event))
}
