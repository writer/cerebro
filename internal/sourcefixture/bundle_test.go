...
func TestValidateHTTPReplayRequestBindsMethodPathAndCanonicalQuery(t *testing.T) {
	request := httptest.NewRequest(http.MethodGet, "https://api.example.test/v1/items?offset=0&limit=1", nil)
	if err := ValidateHTTPReplayRequest(request, http.MethodGet, "/v1/items", "limit=1&offset=0"); err != nil {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			if err := ValidateHTTPReplayRequest(request, test.method, test.path, test.rawQuery); err == nil {
				t.Fatal("ValidateHTTPReplayRequest() error = nil, want mismatch rejection")
			}
		})
	}
	for _, test := range []struct {
		...
		t.Run("actual_"+test.name, func(t *testing.T) {
			...
			if err := ValidateHTTPReplayRequest(request, http.MethodGet, "/v1/items", "kept=1"); err == nil || !strings.Contains(err.Error(), "replay HTTP query is invalid") {
				t.Fatalf("ValidateHTTPReplayRequest(actual malformed query) error = %v", err)
			}
		})
	}
	...
	for _, test := range []struct {
		...
		t.Run("expected_"+test.name, func(t *testing.T) {
			if err := ValidateHTTPReplayRequest(request, http.MethodGet, "/v1/items", test.rawQuery); err == nil || !strings.Contains(err.Error(), "expected replay HTTP query is invalid") {
				t.Fatalf("ValidateHTTPReplayRequest(expected malformed query) error = %v", err)
			}
		})
	}
}
