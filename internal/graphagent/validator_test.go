...
func TestValidatorStaticContract(t *testing.T) {
	tests := []struct {
		...
		{name: "missing final union branch limit", cypher: `MATCH (e:Entity {tenant_id:$tenant_id}) RETURN e LIMIT 1 UNION MATCH (e:Entity {tenant_id:$tenant_id}) RETURN e`, wantResult: validatorRefusal("limit_required", "read Cypher must include a numeric LIMIT clause")},
		{name: "missing first union branch limit", cypher: `MATCH (e:Entity {tenant_id:$tenant_id}) RETURN e UNION MATCH (e:Entity {tenant_id:$tenant_id}) RETURN e LIMIT 1`, wantResult: validatorRefusal("limit_required", "read Cypher must include a numeric LIMIT clause")},
		...
		{name: "function pattern variable cannot scope later match", cypher: `MATCH (e:Entity {tenant_id:$tenant_id}) WITH e, size((e)-[:R]->(b:Entity {tenant_id:$tenant_id})) AS cnt MATCH (b) RETURN b LIMIT 25`, wantResult: validatorRefusal("tenant_scope_required", "every node pattern must use Entity label and inline tenant_id")},
