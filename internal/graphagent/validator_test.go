...
func TestValidatorStaticContract(t *testing.T) {
	tests := []struct {
		...
		{name: "exists with alias cannot leak expression scope", cypher: "MATCH (e:Entity {tenant_id:$tenant_id}) WITH e, EXISTS { WITH e AS x MATCH (b:Entity {tenant_id:$tenant_id}) RETURN b } AS found MATCH (x) RETURN x LIMIT 25", wantResult: validatorRefusal("tenant_scope_required", "every node pattern must use Entity label and inline tenant_id")},
		{name: "count with alias cannot leak expression scope", cypher: "MATCH (e:Entity {tenant_id:$tenant_id}) WITH e, COUNT { WITH e AS x MATCH (b:Entity {tenant_id:$tenant_id}) RETURN b } AS n MATCH (x) RETURN x LIMIT 25", wantResult: validatorRefusal("tenant_scope_required", "every node pattern must use Entity label and inline tenant_id")},
		{name: "collect with alias cannot leak expression scope", cypher: "MATCH (e:Entity {tenant_id:$tenant_id}) WITH e, COLLECT { WITH e AS x MATCH (b:Entity {tenant_id:$tenant_id}) RETURN b } AS xs MATCH (x) RETURN x LIMIT 25", wantResult: validatorRefusal("tenant_scope_required", "every node pattern must use Entity label and inline tenant_id")},
