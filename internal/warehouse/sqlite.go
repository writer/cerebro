...
func (w *SQLiteWarehouse) queryInformationSchemaColumns(ctx context.Context, args ...any) (*snowflake.QueryResult, error) {
	if w == nil || w.db == nil {
		return nil, fmt.Errorf("sqlite warehouse is not initialized")
	}
	if len(args) == 0 {
		return &snowflake.QueryResult{Columns: []string{"column_name"}}, nil
	}
