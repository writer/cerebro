...
func (s *QueryStore) ListVendorRegister(ctx context.Context, filter ports.VendorRegisterFilter) (*ports.VendorRegisterPage, error) {
	...
	for _, vendor := range message.GetVendors() {
		...
		row := ports.VendorRegisterRow{
			VendorRegisterIdentity:   ports.VendorRegisterIdentity{URN: vendor.GetUrn(), VendorID: vendor.GetVendorId(), Name: vendor.GetName(), SourceID: vendor.GetSourceId(), RuntimeID: vendor.GetRuntimeId(), Provider: vendor.GetProvider(), Status: vendor.GetStatus(), Category: vendor.GetCategory(), WebsiteURL: vendor.GetWebsiteUrl(), ServicesProvided: vendor.GetServicesProvided()},
			VendorRegisterGovernance: ports.VendorRegisterGovernance{LifecycleState: vendor.GetLifecycleState(), Owner: vendor.GetOwner(), OwnerState: vendor.GetOwnerState(), RiskLevel: vendor.GetRiskLevel(), RiskScore: vendor.GetRiskScore(), RiskScoreLevel: vendor.GetRiskScoreLevel(), ReviewState: vendor.GetReviewState(), ReviewDueAt: vendor.GetReviewDueAt(), EvidenceFreshnessState: vendor.GetEvidenceFreshnessState(), PacketState: vendor.GetPacketState()},
			VendorRegisterEvidence:   ports.VendorRegisterEvidence{ContractCount: vendor.GetContractCount(), SecurityReviewCount: vendor.GetSecurityReviewCount(), QuestionnaireCount: vendor.GetQuestionnaireCount(), AssuranceDocumentCount: vendor.GetAssuranceDocumentCount(), OpenFindings: vendor.GetOpenFindings(), CriticalFindings: vendor.GetCriticalFindings(), HighFindings: vendor.GetHighFindings(), EvidenceItems: vendor.GetEvidenceItems()},
			VendorRegisterQueue:      ports.VendorRegisterQueue{RiskQueueRank: vendor.GetRiskQueueRank(), QueueReasons: append([]string{}, vendor.GetQueueReasons()...)},
