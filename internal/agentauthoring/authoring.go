...
func (s Service) DraftPolicyBundle(ctx context.Context, request PolicyBundleDraftRequest) (*PolicyBundleDraftResult, error) {
	...
	contextValues["test_author_contract"] = map[string]any{
		...
		"graph_query": map[string]any{
			...
			"bounded_path":  "use fixed directed relationships or the allowlisted APOC path procedures with literal tenant-scoped configuration capped at six hops; raw variable-length traversal, unallowlisted APOC, unallowlisted procedures, UNWIND, range, and collect are forbidden",
		},
