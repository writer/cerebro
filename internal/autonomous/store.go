...
const executionNamespace = executionstore.NamespaceAutonomousWorkflow
...
func (s *SQLiteRunStore) SaveRun(ctx context.Context, run *RunRecord) error {
	...
	if err != nil {
		return fmt.Errorf("encode autonomous workflow run: %w", err)
	}
	return s.store.UpsertRun(ctx, executionstore.RunEnvelope{
		Namespace:   executionNamespace,
		RunID:       run.ID,
		Kind:        string(run.WorkflowID),
...
func (s *SQLiteRunStore) LoadRun(ctx context.Context, runID string) (*RunRecord, error) {
	...
	if err := json.Unmarshal(env.Payload, &run); err != nil {
		return nil, fmt.Errorf("decode autonomous workflow run: %w", err)
	}
	return &run, nil
}
...
func (s *SQLiteRunStore) ListRuns(ctx context.Context, opts RunListOptions) ([]RunRecord, error) {
	...
	envs, err := s.store.ListRuns(ctx, executionNamespace, executionstore.RunListOptions{
	...
	for _, env := range envs {
		var run RunRecord
		if err := json.Unmarshal(env.Payload, &run); err != nil {
			return nil, fmt.Errorf("decode autonomous workflow run payload: %w", err)
		}
...
func (s *SQLiteRunStore) AppendEvent(ctx context.Context, runID string, event RunEvent) (RunEvent, error) {
	...
	if err != nil {
		return event, fmt.Errorf("encode autonomous workflow event: %w", err)
	}
...
func (s *SQLiteRunStore) LoadEvents(ctx context.Context, runID string) ([]RunEvent, error) {
	...
	for _, env := range envs {
		...
		if err := json.Unmarshal(env.Payload, &event); err != nil {
			return nil, fmt.Errorf("decode autonomous workflow event payload: %w", err)
		}
