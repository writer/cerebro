...
func (s *Server) currentGraphIntelligenceStore(ctx context.Context) graph.GraphStore {
	if s == nil || s.graphIntelligence == nil {
		return nil
	}
	store, err := s.graphIntelligence.CurrentGraphStore(ctx)
	if err != nil {
		return nil
	}
	return store
...
func (s *Server) graphQuery(w http.ResponseWriter, r *http.Request) {
	...
	if asOfRaw == "" && fromRaw == "" && toRaw == "" {
		if store := s.currentGraphIntelligenceStore(r.Context()); store != nil {
			switch mode {
			case "neighbors":
				s.graphQueryNeighborsStore(w, r, store, nodeID, temporalScope)
			case "paths", "path":
				s.graphQueryPathsStore(w, r, store, nodeID, temporalScope)
			default:
...
func (s *Server) graphQueryNeighborsStore(w http.ResponseWriter, r *http.Request, store graph.GraphStore, nodeID string, temporalScope map[string]any) {
	direction := strings.ToLower(strings.TrimSpace(r.URL.Query().Get("direction")))
	if direction == "" {
		direction = "both"
	}
	...
	if _, ok, err := store.LookupNode(r.Context(), nodeID); err != nil {
		s.error(w, http.StatusServiceUnavailable, "graph platform not initialized")
		return
	} else if !ok {
		s.error(w, http.StatusNotFound, fmt.Sprintf("node not found in selected scope: %s", nodeID))
	...
	edgesScanned := 0
	if direction == "out" || direction == "both" {
		edges, err := store.LookupOutEdges(r.Context(), nodeID)
		if err != nil {
			s.error(w, http.StatusServiceUnavailable, "graph platform not initialized")
			return
		}
		for _, edge := range edges {
			edgesScanned++
			targetNode, _, err := store.LookupNode(r.Context(), edge.Target)
			if err != nil {
				s.error(w, http.StatusServiceUnavailable, "graph platform not initialized")
				return
			}
	...
	if direction == "in" || direction == "both" {
		edges, err := store.LookupInEdges(r.Context(), nodeID)
		if err != nil {
			s.error(w, http.StatusServiceUnavailable, "graph platform not initialized")
			return
		}
		for _, edge := range edges {
			edgesScanned++
			sourceNode, _, err := store.LookupNode(r.Context(), edge.Source)
			if err != nil {
				s.error(w, http.StatusServiceUnavailable, "graph platform not initialized")
				return
			}
...
func (s *Server) graphQueryPathsStore(w http.ResponseWriter, r *http.Request, store graph.GraphStore, nodeID string, temporalScope map[string]any) {
	targetID := strings.TrimSpace(r.URL.Query().Get("target_id"))
	if targetID == "" {
		s.error(w, http.StatusBadRequest, "target_id is required for paths mode")
		return
	...
	if _, ok, err := store.LookupNode(r.Context(), targetID); err != nil {
		s.error(w, http.StatusServiceUnavailable, "graph platform not initialized")
		return
	} else if !ok {
		s.error(w, http.StatusNotFound, fmt.Sprintf("target node not found: %s", targetID))
	...
	queryGraph, err := store.ExtractSubgraph(r.Context(), nodeID, graph.ExtractSubgraphOptions{
		MaxDepth:  maxDepth,
		Direction: graph.ExtractSubgraphDirectionOutgoing,
	})
	if err != nil || queryGraph == nil {
		s.error(w, http.StatusServiceUnavailable, "graph platform not initialized")
		return
	}
