...
func TestListProviders_HidesIncompleteByDefault(t *testing.T) {
	...
	a.Providers.Register(&staticProvider{name: "semgrep", provider: providers.ProviderTypeSaaS})
...
func TestListProviders_IncludeIncomplete(t *testing.T) {
	...
	for _, item := range items {
		...
		if provider["name"] == "semgrep" {
			foundSemgrep = true
			if provider["maturity"] != string(providers.ProviderMaturityStub) {
				t.Fatalf("expected semgrep maturity %q, got %v", providers.ProviderMaturityStub, provider["maturity"])
			}
		}
	}
	if !foundSemgrep {
		t.Fatal("expected semgrep in provider list when include_incomplete=true")
	}
}
...
func TestGetProvider_IncompleteHiddenByDefault(t *testing.T) {
	...
	w := do(t, s, "GET", "/api/v1/providers/semgrep", nil)
	if w.Code != http.StatusNotFound {
		t.Fatalf("expected 404, got %d", w.Code)
	}

	w = do(t, s, "GET", "/api/v1/providers/semgrep?include_incomplete=true", nil)
	if w.Code != http.StatusOK {
