...
func evidenceSchema() map[string]any {
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"requirementRefs":   uniqueStringArraySchema("Evidence requirement references using profile_id/source_id/entity_type."),
...
func auditSchema() map[string]any {
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"claimStrength":            stringSchema("Evidence claim strength for policy evidence packets."),
			"sufficiencyRule":          stringSchema("Rule describing when the evidence is sufficient."),
			"coverageClaim":            stringSchema("Control coverage claim supported by this policy."),
			"overclaimGuard":           stringSchema("Boundary that prevents broader unsupported control claims."),
			"adjacentControlRationale": stringSchema("How adjacent controls should be interpreted for review."),
