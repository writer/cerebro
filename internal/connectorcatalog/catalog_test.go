...
func TestBuiltinCatalogSeedSummary(t *testing.T) {
	...
	if analysis.Summary.Total != 114 {
		t.Fatalf("summary total = %d, want 114", analysis.Summary.Total)
	}
	if len(analysis.Entries) != 114 {
		t.Fatalf("entries len = %d, want 114", len(analysis.Entries))
	}
	if analysis.Summary.Generateable != 114 {
