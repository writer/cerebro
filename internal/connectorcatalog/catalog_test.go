...
func TestBuiltinCatalogSeedSummary(t *testing.T) {
	...
	if analysis.Summary.Total != 202 {
		t.Fatalf("summary total = %d, want 202", analysis.Summary.Total)
	}
	if len(analysis.Entries) != 202 {
		t.Fatalf("entries len = %d, want 202", len(analysis.Entries))
	}
	if analysis.Summary.Generateable != 202 {
