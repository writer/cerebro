...
func TestHostKeepsCredentialInGoAndReturnsSafeReceipt(t *testing.T) {
	...
	event, err := AuthorizationPolicyEvent(plan, scope, output.Result, now)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !bytes.Contains(event.GetPayload(), []byte(`"tenant_id":"tenant-1"`)) {
...
func TestPublicDNSValidationRejectsPrivateAndMixedAnswers(t *testing.T) {
	for name, addresses := range map[string][]net.IPAddr{
		...
		t.Run(name, func(t *testing.T) {
			if err := validatePublicAddresses(addresses); !errorsIsInvalid(err) {
