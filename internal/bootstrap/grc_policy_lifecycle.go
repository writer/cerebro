...
type grcPolicyLifecycleResponse struct {
...
func (a *App) handleGRCPolicyLifecycle(w http.ResponseWriter, r *http.Request) {
	...
	response, err := buildGRCPolicyLifecycle(r.Context(), store, scope)
	if err != nil {
		writeGRCError(w, err)
		return
	}
...
func buildGRCPolicyLifecycle(ctx context.Context, store ports.GraphQueryStore, scope grcScope) (grcPolicyLifecycleResponse, error) {
	limit := int(scope.Limit)
	if limit <= 0 {
		limit = int(grcDefaultLimit)
	}
	...
	if err != nil {
		return grcPolicyLifecycleResponse{}, err
	}
...
func grcPolicyLifecycleFromGraph(entityRows []ports.CypherRow, relationRows []ports.CypherRow, generatedAt time.Time) grcPolicyLifecycleResponse {
	...
	return grcPolicyLifecycleResponse{
