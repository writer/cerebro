...
from cerebro.compliance.evidence_data_fabric import (
    create_evidence_data_fabric, EvidenceEntityType, EvidenceSourceType
)
...
from cerebro.compliance.risk_management import (
    create_risk_management_system, RiskCategory, RiskImpactLevel, 
    RiskProbability, VendorRiskTier, TreatmentType
)
...
async def demo_integrated_risk_management():
    ...
    print("=" * 50)
    print("Problem: 'We still manage risks in Excel because Vanta/Drata have limited functionality'")
    ...
    print(f"   🏢 Added vendor: {critical_vendor.name} (Risk Tier: {critical_vendor.risk_tier.value})")
    print(f"   🏢 Added vendor: {medium_vendor.name} (Risk Tier: {medium_vendor.risk_tier.value})")
    ...
    print(f"      Next review due: {critical_vendor.next_assessment_due.strftime('%Y-%m-%d')}")
    ...
    print(f"      Target completion: {treatment.target_completion_date.strftime('%Y-%m-%d')}")
    ...
    print(f"      Assessments due: {len(dashboard['vendor_summary']['assessments_due'])}")
...
async def demo_flexible_controls_framework():
    ...
    for framework, summary in cross_mapping["mapping_summary"].items():
        ...
        print(f"      Evidence reuse: {cross_mapping['evidence_reuse_analysis'][framework]['potential_savings']} opportunities")
...
async def demo_rich_reporting_analytics():
    ...
    print(f"      Overall compliance score: {executive_dashboard['executive_summary']['overall_compliance_score']:.1f}%")
    print(f"      Controls monitored: {executive_dashboard['executive_summary']['total_controls_monitored']}")
    print(f"      Automation level: {executive_dashboard['executive_summary']['automated_controls_percentage']:.1f}%")
    print(f"      High-risk items: {executive_dashboard['executive_summary']['high_risk_count']}")
    print(f"      Evidence freshness: {executive_dashboard['executive_summary']['evidence_freshness_score']:.1f}%")
    ...
    print(f"      Key accomplishments: {len(board_presentation['executive_summary']['key_accomplishments'])}")
    print(f"      Areas of concern: {len(board_presentation['executive_summary']['areas_of_concern'])}")
    ...
    for kpi_id, result in kpi_results.items():
        ...
        print(f"      {status_icon} {result['name']}: {result['value']:.1f}{result['unit']}")
    ...
    print(f"      Evidence items ready: {audit_report['auditor_package_status']['evidence_items_ready']}")
    print(f"      Missing evidence: {audit_report['auditor_package_status']['missing_evidence']}")
    print(f"      Package completeness: {audit_report['auditor_package_status']['estimated_package_completeness']:.1f}%")
...
async def demo_trust_transparency():
    ...
    print(f"      Security certifications: {len(trust_center['public_information']['security_certifications'])}")
    print(f"      Customer controls: {len(trust_center['customer_controls'])} available")
    print(f"      Transparency reports: Real-time availability tracking")
    print(f"      Data processing: {len(trust_center['public_information']['data_processing_locations'])} regions")
