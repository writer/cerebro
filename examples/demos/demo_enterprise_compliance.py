...
async def demo_comprehensive_platform():
    ...
    print(f"Available Security Tables: {len(tables)} tables across {len(provider_counts)} providers")
    ...
    for i, query in enumerate(demo_queries, 1):
        ...
        try:
            ...
            print(f"      → {result.total_rows} rows in {result.execution_time_ms:.1f}ms")
    ...
    print("  1. Query evidence: SELECT * FROM access_review_evidence WHERE quarter = 'Q1 2024'")
    print("  2. Generate bundle: cerebro evidence export --control CC6.1 --bundle soc2_cc61.evb")
    ...
    print("  1. Graph analysis: cerebro graph path --from principal:compromised_user --to resource:prod_db")
    print("  2. Evidence collection: Query transparency log for all related actions")
    print("  3. Impact assessment: Blast radius analysis shows 23 reachable critical resources")
    print("  4. Result: Complete forensic timeline with mathematical proofs")
    print()
    
    print("Scenario 3: Vendor Risk Assessment")
    print("  1. Discovery: OAuth registry finds 47 vendors across all SaaS applications")
