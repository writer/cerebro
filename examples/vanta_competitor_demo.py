...
from cerebro.compliance.evidence_store import (
    EvidenceStore,
    EvidenceCategory,
    EvidenceMetadata,
)
...
async def demonstrate_vanta_competitor_features():
    ...
    print("=" * 50)
    print(
        "Demonstrating Vanta/Drata-class features that sell to CFOs and compliance managers\n"
    )
    ...
    evidence_store = EvidenceStore(
        storage_path="./demo_evidence_store", signing_key_path="./demo_signing_key.pem"
    )
    ...
    print(
        f"  🤖 Automated: {soc2_coverage['automated_controls']} ({soc2_coverage['automation_percentage']:.1f}%)"
    )
    print(
        f"  ✅ Testable: {soc2_coverage['controls_with_tests']} ({soc2_coverage['coverage_percentage']:.1f}%)"
    )
    print(
        f"  🎯 Rule Utilization: {soc2_coverage['used_rules']}/{soc2_coverage['framework_mapped_rules']} ({soc2_coverage['rule_utilization']:.1f}%)"
    )
    ...
    results = await test_runner.run_framework_tests(
        "soc2", sample_tests, period_start, period_end
    )
    ...
    coverage = await test_runner.calculate_coverage(
        "soc2", [], results
    )  # Mock empty controls list
    ...
    evidence_id = await evidence_store.store_evidence(
        content=sample_evidence, metadata=evidence_metadata, seal_immediately=True
    )
    ...
    print(
        f"  📅 Audit period: {audit_period_start.date()} to {audit_period_end.date()}"
    )
    ...
    employees = [
        ...
        Employee(
            "emp002", "Bob Smith", "bob@company.com", "Security", "Security Analyst"
        ),
    ...
    await policy_system.publish_policy(
        policy_id=security_policy.id, published_by="ciso@company.com"
    )
    ...
    print(
        f"  📊 Policy compliance rate: {compliance_report['summary']['compliance_rate']:.1f}%"
    )
    print()
    ...
    if bundle_id:
        ...
        export_path = await evidence_store.export_bundle(
            bundle_id=bundle_id, export_path="./audit_delivery", format="zip"
        )
        print(f"  📦 Exported to: {export_path}")

        # Get auditor portal data
        auditor_data = audit_manager.get_auditor_portal_data(
            request_list_id=request_list.id, auditor_email="auditor@big4firm.com"
        )
        if auditor_data:
            print(
                f"  👨‍💼 Auditor portal ready: {auditor_data['controls_tested']} controls tested"
            )
    print()
