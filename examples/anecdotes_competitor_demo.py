...
from cerebro.compliance.evidence_data_fabric import (
    EvidenceDataFabric, EvidenceEntityType, EvidenceSourceType,
    RequirementDefinition, create_evidence_data_fabric
)
from cerebro.compliance.rules_engine import (
    NoCodeRulesEngine, RuleType, RuleSeverity, PolicyStatementParser,
    create_rules_engine
)
from cerebro.compliance.requirement_mapper import (
    RequirementMappingService, MappingConfidence, EvidenceReusability,
...
async def demonstrate_data_fabric_capabilities():
    ...
    print("=" * 50)
    print("Building the normalized, queryable evidence substrate that Anecdotes.ai uses")
    print()
    ...
    for user in okta_users:
        evidence_id = evidence_fabric.ingest_evidence(
            ...
            tags={"evidence_type": "user_account", "mfa_status": "enabled" if user["factors"] else "disabled"}
        )
        print(f"   ✅ Ingested Okta user: {user['displayName']} (ID: {evidence_id[:8]}...)")
    ...
    aws_users = [
        {
            ...
            "MfaDevices": [{"SerialNumber": "arn:aws:iam::123456789:mfa/alice.johnson"}]
        }
    ]
    ...
    print(f"   ✅ Created derived evidence for identity correlation: {derived_evidence_id[:8]}...")
...
async def demonstrate_rules_engine():
    ...
    for result in rule_results[:3]:  # Show first 3 results
        ...
        if not result.passed and result.violations:
            ...
            print(f"      Example violation: {violation.get('entity_name', 'Unknown')} - {violation.get('actual', 'N/A')}")
...
async def demonstrate_requirement_mapping():
    ...
    print("=" * 50)
    print("Fine-grained evidence reuse - the key to beating Vanta's control-level approach")
    print()
    ...
    requirements_to_analyze = [
        "CC6.1.1", "CC6.1.2", "CC6.2.1",  # SOC 2 
        "A.9.2.1.1", "A.9.4.2.1"          # ISO 27001
    ]
    
    reuse_analysis = mapper.analyze_evidence_reuse(
        requirements=requirements_to_analyze,
    ...
    for opportunity in reuse_analysis["reuse_opportunities"][:2]:  # Show first 2
        print(f"   💡 {opportunity['source_framework']} {opportunity['source_requirement']} → {opportunity['target_framework']} {opportunity['target_requirement']}")
    ...
    coverage_results = mapper.validate_requirement_coverage(
        ...
        evidence_query_filter={"time_range": (datetime.now() - timedelta(days=30), datetime.now())}
    )
    ...
    print(f"   🎯 Total reuse opportunities: {overall_stats['total_reuse_opportunities']}")
...
async def demonstrate_comprehensive_compliance_program():
    ...
    for source, count in evidence_sources.items():
        ...
        for i in range(count):
            evidence_fabric.ingest_evidence(
                ...
                source_type=EvidenceSourceType.API,
                entity_type=EvidenceEntityType.IDENTITY if i % 2 == 0 else EvidenceEntityType.CONFIGURATION,
                entity_id=f"{source}_entity_{i}",
                raw_data={"simulated": True, "item": i, "timestamp": datetime.now().isoformat()},
    ...
    print(f"   🔗 Evidence reuse opportunities: {cross_analysis['overall_stats']['total_reuse_opportunities']}")
    print(f"   📈 Average mapping coverage: {cross_analysis['overall_stats']['avg_mapping_percentage']:.1f}%")
    ...
    for severity, stats in severity_breakdown.items():
        pass_pct = (stats["passing"] / stats["total"] * 100) if stats["total"] > 0 else 0
        print(f"   📋 {severity.upper()}: {pass_pct:.1f}% passing ({stats['passing']}/{stats['total']})")
    ...
    adequate_coverage = sum(1 for c in coverage_assessment.values() if c.get("coverage", 0) >= 0.8)
    total_requirements = len(coverage_assessment)
    readiness_pct = (adequate_coverage / total_requirements * 100) if total_requirements > 0 else 0
    
    print(f"   🎯 Audit readiness: {readiness_pct:.1f}%")
    print(f"   ✅ Requirements with adequate coverage: {adequate_coverage}/{total_requirements}")
    
    inadequate_reqs = [
        req_id for req_id, coverage in coverage_assessment.items()
...
async def main():
    ...
    try:
        ...
        print("   Key differentiators: Temporal queries + Security depth + Forensic replay")
        ...
        print("This is expected in a conceptual demo - real implementation would have proper DB setup")
