...
def upgrade() -> None:
    ...
    op.create_table('agent_sessions',
        ...
        sa.Column('agent_type', sa.Enum('security_analyst', 'incident_responder', 'identity_advisor', 'compliance_advisor', 'attack_path_analyst', name='agenttype'), nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, default=sa.text('now()')),
        sa.Column('created_by', sa.String(255), nullable=False),
        sa.Column('title', sa.String(500), nullable=True),
        sa.Column('context', postgresql.JSONB(), nullable=False, default=sa.text("'{}'::jsonb")),
    ...
    op.create_table('tool_invocations',
        ...
        sa.Column('status', sa.Enum('pending', 'running', 'success', 'error', 'dry_run', 'approval_required', name='toolinvocationstatus'), nullable=False, default='pending'),
        sa.Column('started_at', sa.DateTime(timezone=True), nullable=False, default=sa.text('now()')),
    ...
    op.create_index('ix_tool_invocations_celery_task_id', 'tool_invocations', ['celery_task_id'])
    ...
    op.create_table('tool_approvals',
        ...
        sa.Column('status', sa.Enum('pending', 'approved', 'rejected', 'expired', name='approvalstatus'), nullable=False, default='pending'),
        ...
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('tool_invocation_id', name='uq_tool_approvals_tool_invocation_id')
    )
    
    # Add indexes for tool approvals
    op.create_index('ix_tool_approvals_org_id', 'tool_approvals', ['org_id'])
    op.create_index('ix_tool_approvals_tool_invocation_id', 'tool_approvals', ['tool_invocation_id'])
    ...
    op.create_table('agent_recommendations',
        ...
        sa.Column('action_items', postgresql.JSONB(), nullable=False, default=sa.text("'[]'::jsonb")),
        sa.Column('estimated_effort', sa.String(50), nullable=True),
        sa.Column('implementation_timeline', sa.String(50), nullable=True),
        sa.Column('cis_controls', postgresql.JSONB(), nullable=False, default=sa.text("'[]'::jsonb")),
        sa.Column('nist_controls', postgresql.JSONB(), nullable=False, default=sa.text("'[]'::jsonb")),
        sa.Column('cwe_ids', postgresql.JSONB(), nullable=False, default=sa.text("'[]'::jsonb")),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, default=sa.text('now()')),
    ...
    op.create_index('ix_agent_recommendations_session_id', 'agent_recommendations', ['session_id'])
    op.create_index('ix_agent_recommendations_org_id', 'agent_recommendations', ['org_id'])
    op.create_index('ix_agent_recommendations_created_at', 'agent_recommendations', ['created_at'])
    op.create_index('ix_agent_recommendations_status', 'agent_recommendations', ['status'])
    op.create_index('ix_agent_recommendations_priority', 'agent_recommendations', ['priority'])
