...
def upgrade() -> None:
    ...
    if 'provider_credentials' not in inspector.get_table_names():
        logger.info("provider_credentials table doesn't exist yet - will be created in migration 005")
        return
    ...
    if existing_rows > 0:
        logger.info(f"Found {existing_rows} existing credential rows that need envelope encryption migration")
        ...
        if kms_provider == 'local':
            logger.warning("Using local KMS for migration - this should only be done in development")
    ...
    op.create_index('ix_provider_credentials_encrypted_dek', 'provider_credentials', ['encrypted_dek'])
...
def downgrade() -> None:
    ...
    logger.warning("Credentials will fall back to direct SECRET_KEY encryption (less secure)")
