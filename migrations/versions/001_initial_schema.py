...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "normalized_config", postgresql.JSONB(astext_type=sa.Text()), nullable=False
        ),
        sa.Column("collector_version", sa.String(), nullable=False),
        sa.ForeignKeyConstraint(
            ["resource_id"], ["resources.resource_id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("snapshot_id"),
        sa.UniqueConstraint("resource_id", "config_sha"),
    )
    op.create_index(
        "ix_config_snapshots_resource_captured",
        "config_snapshots",
        ["resource_id", "captured_at"],
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "expression_lang IN ('sql','rego','cel')",
            name="rules_expression_lang_check",
        ),
        sa.CheckConstraint(
        ...
        sa.ForeignKeyConstraint(
            ["policy_id"], ["policies.policy_id"], ondelete="SET NULL"
        ),
    ...
    op.create_index(
        "ix_rules_resource_types", "rules", ["resource_types"], postgresql_using="gin"
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('open','suppressed','accepted_risk','fixed')",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["finding_id"], ["findings.finding_id"], ondelete="CASCADE"
        ),
    ...
    op.create_index(
        "ix_audit_events_provider_action", "audit_events", ["provider", "action"]
    )
    op.create_index(
        "ix_audit_events_raw", "audit_events", ["raw"], postgresql_using="gin"
    )
