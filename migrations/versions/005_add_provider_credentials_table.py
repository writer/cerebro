...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "account_id", "provider", name="uq_provider_credentials_account_provider"
        ),
    )

    # Add indexes for performance
    op.create_index(
        "ix_provider_credentials_account_id", "provider_credentials", ["account_id"]
    )
    op.create_index(
        "ix_provider_credentials_provider", "provider_credentials", ["provider"]
    )
    op.create_index(
        "ix_provider_credentials_is_active", "provider_credentials", ["is_active"]
    )
    op.create_index(
        "ix_provider_credentials_expires_at", "provider_credentials", ["expires_at"]
    )
