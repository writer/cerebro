...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["cluster_id"], ["identity_clusters.cluster_id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["cluster_id"], ["identity_clusters.cluster_id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(["org_id"], ["orgs.org_id"], ondelete="CASCADE"),
    ...
    # Create indexes
    op.create_index(
        "ix_identity_clusters_org_confidence",
        "identity_clusters",
        ["org_id", "confidence_score"],
    )
    op.create_index(
        "ix_identity_cluster_members_cluster",
        "identity_cluster_members",
        ["cluster_id"],
    )
    op.create_index(
        "ix_identity_cluster_members_principal",
        "identity_cluster_members",
        ["principal_id"],
    )
    op.create_index(
        "ix_identity_stitching_logs_timestamp", "identity_stitching_logs", ["timestamp"]
    )


def downgrade() -> None:
    op.drop_index(
        "ix_identity_stitching_logs_timestamp", table_name="identity_stitching_logs"
    )
    op.drop_index(
        "ix_identity_cluster_members_principal", table_name="identity_cluster_members"
    )
    op.drop_index(
        "ix_identity_cluster_members_cluster", table_name="identity_cluster_members"
    )
