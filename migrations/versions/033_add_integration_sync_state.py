...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("state_id", postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text("gen_random_uuid()")),
        ...
        sa.Column("state_metadata", postgresql.JSONB(astext_type=sa.Text()), nullable=True, server_default=sa.text("'{}'::jsonb")),
...
def downgrade() -> None:
    op.drop_index("ix_integration_sync_state_scope", table_name="integration_sync_state")
    op.drop_index("ix_integration_sync_state_integration", table_name="integration_sync_state")
