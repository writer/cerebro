...
echo "Waiting for ${ecr_repository}:${image_tag} in ${aws_region} before deploying ${stack_name}..."
for attempt in $(seq 1 "${max_attempts}"); do
  err_file="$(mktemp)"
  if aws ecr describe-images --repository-name "${ecr_repository}" --image-ids imageTag="${image_tag}" >/dev/null 2>"${err_file}"; then
    rm -f "${err_file}"
    echo "Found ${ecr_repository}:${image_tag}; continuing with Pulumi deploy"
    exit 0
  fi
  ...
  echo "Image not available yet (attempt ${attempt}/${max_attempts}); retrying in ${sleep_seconds} seconds"
  sleep "${sleep_seconds}"
done

echo "ERROR: ${ecr_repository}:${image_tag} was not found in ECR after ${max_attempts} attempts"
exit 1
