...
- Release promotion workflows prefer a short-lived, preflighted GitHub App installation token for automation branches and PRs, and fall back to the existing `CEREBRO_AUTORELEASE_TOKEN` while the App credentials are not configured. Trusted `sec-dev` release dispatches auto-merge their PRs only after required checks pass; the resulting normal `main` push runs deploys without direct branch writes or explicit deploy dispatches. To use the App path, configure `CEREBRO_DEPLOY_APP_CLIENT_ID` as a repository variable and `CEREBRO_DEPLOY_APP_PRIVATE_KEY` as a repository secret; the App installation needs Contents write, Pull requests write, Issues write, Commit statuses read, and the implicit Metadata read permission for this repository.
