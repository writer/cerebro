...
class AgentSessionManager:
    ...
    def create_session(self, scenario: str, org_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        with SessionLocal() as db:
            if org_name:
                ...
                if not org:
                    raise ValueError("No organizations found. Run generate_test_data.py first")
            
            # Get context data
            accounts = db.query(models.Account).filter(
                models.Account.org_id == org.org_id
            ).all()
            
            findings = db.query(models.Finding).filter(
                models.Finding.org_id == org.org_id
            ).order_by(models.Finding.severity == 'critical').limit(10).all()
        ...
        print(f"🚨 Critical findings: {session_context['context']['critical_findings']}")
        print()
...
class MockDataGenerator:
    ...
    def generate_findings(self, count: int = 10, org_id: Optional[str] = None) -> List[models.Finding]:
        ...
        finding_templates = [
            ...
            ("Overprivileged IAM Role", "IAM role {resource} has excessive permissions"),
            ("Public Database Access", "Database {resource} allows public network access"),
            ("Weak Password Policy", "Account {resource} has insufficient password requirements"),
            ("Missing MFA", "User {resource} lacks multi-factor authentication"),
            ("Expired Certificate", "SSL certificate for {resource} has expired"),
            ("Vulnerable Software", "Instance {resource} running vulnerable software version"),
            ("Excessive Data Retention", "Dataset {resource} retained beyond policy limits"),
            ("Cross-Border Data Transfer", "Data transfer to {resource} lacks consent"),
            ("Missing Audit Logging", "Service {resource} has insufficient audit logging")
        ]
    ...
    def generate_audit_events(self, count: int = 50, account_id: Optional[str] = None) -> List[models.AuditEvent]:
        ...
        account = self.session.query(models.Account).filter(
        ...
        event_templates = [
            ...
            ("AssumeRole", {"eventName": "AssumeRole", "roleArn": "arn:aws:iam::123456789012:role/TestRole"}),
            ("PutBucketPolicy", {"eventName": "PutBucketPolicy", "bucketName": "test-bucket"}),
            ("CreateRole", {"eventName": "CreateRole", "roleName": "test-role"}),
            ("DeleteUser", {"eventName": "DeleteUser", "userName": "old-user"}),
            ("ModifyDBInstance", {"eventName": "ModifyDBInstance", "dbInstanceId": "prod-db"})
        ]
        ...
        for i in range(count):
            ...
            event = models.AuditEvent(
                ...
                provider=account.provider,
                occurred_at=datetime.now() - timedelta(minutes=random.randint(1, 10080)),  # Last week
...
def main():
    ...
    mock_parser.add_argument("--type", choices=["findings", "events", "all"], default="all")
    mock_parser.add_argument("--count", type=int, default=10, help="Number of items to generate")
    ...
    if args.command == "session":
        ...
        if args.list:
            ...
            for session in sessions:
                print(f"  {session['session_id']} - {session['scenario']} - {session['organization']}")
