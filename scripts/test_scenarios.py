...
def test_scenario_generation():
    ...
    scenarios = {
        ...
        "Compliance Violation": (
            ...
            "generate_compliance_violation_scenario",
        ),
        "Multi-Cloud Review": (
            "multi_cloud_review",
            "generate_multi_cloud_review_scenario",
        ),
        "Attack Path Analysis": (
            "attack_path_scenario",
            "generate_attack_path_scenario",
        ),
    }
    ...
    for name, result in results.items():
        ...
        if result["status"] == "PASS":
            print(
                f"    - {result['findings']} findings, {result['accounts']} accounts, {result['principals']} principals"
            )
