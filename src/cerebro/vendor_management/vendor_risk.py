...
class VendorRiskAssessment(Base):
    ...
    data_security_score: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0
    )
    access_control_score: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0
    )
    compliance_score: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    business_continuity_score: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0
    )
...
class VendorRiskManager:
    ...
    async def create_risk_assessment(
        ...
        logger.info(
            f"Created risk assessment {assessment.assessment_id} for vendor {vendor_id}"
        )
        return assessment
    ...
    async def update_assessment_scores(
        ...
        assessment.business_continuity_score = domain_scores.get(
        ...
        logger.info(
            f"Updated assessment {assessment_id} with overall score {overall_score}"
        )
        return True

    async def create_risk_scenario(
        self, assessment_id: UUID, scenario: RiskScenario
    ) -> bool:
        ...
        logger.info(
            f"Added risk scenario {scenario.scenario_id} to assessment {assessment_id}"
        )
        return True
    ...
    async def generate_risk_dashboard(self, org_id: UUID) -> dict[str, Any]:
        ...
        if latest_assessments:
            avg_risk_score = sum(
                a.overall_risk_score for a in latest_assessments
        ...
        return {
            "summary": {
                ...
                "high_risk_vendors": risk_distribution.get(
                    VendorRiskLevel.HIGH.value, 0
                ),
                "critical_risk_vendors": risk_distribution.get(
                    VendorRiskLevel.CRITICAL.value, 0
                ),
            },
...
class VendorRiskCalculator:
    ...
    def calculate_inherent_risk(
        ...
        criticality_scores = {"low": 1.0, "medium": 2.5, "high": 4.0, "critical": 5.0}
    ...
    def calculate_residual_risk(
        inherent_risk: float, control_effectiveness: float, monitoring_maturity: float
    ) -> float:
