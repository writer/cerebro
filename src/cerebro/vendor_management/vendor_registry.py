...
class VendorRegistry:
    ...
    async def register_vendor(
        ...
        **vendor_data
        ...
        vendor_id = f"vendor_{name.lower().replace(' ', '_')}_{int(datetime.now().timestamp())}"
        ...
        vendor = Vendor(
            ...
            data_processing_agreements=vendor_data.get("data_processing_agreements", []),
            security_questionnaire_completed=False,
            penetration_test_results=None,
            vulnerability_disclosure_policy=vendor_data.get("vulnerability_disclosure_policy"),
    ...
    async def _assess_vendor_risk(self, vendor: Vendor):
        ...
        if any(data_type in vendor.data_types_processed for data_type in sensitive_data_types):
            risk_score += 0.2
    ...
    async def get_vendors_processing_sensitive_data(self) -> List[Vendor]:
        ...
        return [
            vendor for vendor in self.vendors.values()
            if any(data_type in vendor.data_types_processed for data_type in sensitive_types)
        ]
    ...
    async def generate_vendor_risk_report(self, org_id: str) -> Dict[str, Any]:
        ...
        questionnaire_complete = len([v for v in vendors if v.security_questionnaire_completed])
        ...
        return {
            ...
            "summary": {
                ...
                "compliance_rate": round((certified_vendors / max(len(vendors), 1)) * 100, 1)
            },
            ...
            "compliance_metrics": {
                ...
                "vendors_with_incident_response": len([v for v in vendors if v.incident_response_plan])
            },
            "overdue_reviews": [
                {
                    "vendor_id": vendor.vendor_id,
