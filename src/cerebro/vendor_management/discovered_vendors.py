...
class DiscoveredVendorTracker:
    ...
    async def discover_vendors_from_oauth(self, org_id: str) -> List[DiscoveredVendor]:
        ...
        for app in oauth_apps:
            ...
            existing = next(
                (
                    v
                    for v in self.discovered_vendors.values()
                    if v.domain == vendor_domain
                ),
                None,
            )
            ...
            discovered_vendor = DiscoveredVendor(
                ...
                data_access_detected=any(
                    scope.sensitive_data_access for scope in app.scopes
                ),
            ...
            self.discovered_vendors[discovered_vendor.discovered_vendor_id] = (
                discovered_vendor
            )
    ...
    def _extract_vendor_name(self, domain: str, app_name: str) -> str:
        ...
        if (
            len(app_name) > len(clean_domain)
            and clean_domain.lower() in app_name.lower()
        ):
            return app_name
    ...
    def _calculate_discovery_confidence(self, app: OAuthApp) -> float:
        ...
        if app.publisher_domain.endswith(".io") or app.publisher_domain.endswith(
            ".app"
        ):
            confidence -= 0.1
    ...
    def _estimate_vendor_category(self, app: OAuthApp) -> str:
        ...
        if any(term in app_name_lower for term in ["slack", "teams", "zoom", "meet"]):
            ...
            term in app_name_lower for term in ["github", "gitlab", "jenkins", "ci"]
        ):
    ...
    def _analyze_repo_for_vendors(self, repo: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        integration_topics = [
            ...
            if any(
                term in topic.lower()
                for term in ["api", "integration", "connector", "webhook"]
            )
        ]
                ...
                indicators.append(
                    {
                        ...
                        "auth_detected": "auth" in topic.lower()
                        or "oauth" in topic.lower(),
    ...
    async def review_discovered_vendor(
        ...
        logger.info(
            f"Reviewed discovered vendor {discovered_vendor_id}: {review_result['decision']}"
        )
