...
class FindingManager:
    ...
    async def generate_findings(
        ...
        try:
            ...
            logger.info(
                f"Processing {len(evaluation_results)} resource evaluation results"
            )
            ...
            logger.info(
                f"Finding generation completed: {result.findings_created} created, "
        ...
        for rule_result in rule_results:
            if rule_result.error:
                result.errors.append(
                    f"Rule {rule_result.rule_id} error: {rule_result.error}"
                )
                continue

            if rule_result.matched:
                await self._create_or_update_finding(org, resource, rule_result, result)
            else:
    ...
    async def _create_or_update_finding(
        ...
        stmt = select(Finding).where(
            and_(Finding.org_id == org.org_id, Finding.fingerprint == fingerprint)
        )
        ...
        if existing_finding:
            ...
            logger.debug(
                f"Created finding for rule {rule.name} on resource {resource.external_id}"
            )
    ...
    def _generate_finding_summary(self, rule: Rule, resource: Resource) -> str:
        ...
        return (
            f"Rule '{rule.name}' triggered on {resource.resource_type} "
    ...
    async def accept_risk(self, finding_id: UUID, reason: str) -> bool:
    ...
    async def get_finding_stats(self, org_id: UUID) -> dict[str, Any]:
        ...
        # Get total count
        total_stmt = select(func.count(Finding.finding_id)).where(
        ...
        status_stmt = (
            select(Finding.status, func.count(Finding.finding_id))
        ...
        provider_stmt = (
            select(Finding.provider, func.count(Finding.finding_id))
            .where(Finding.org_id == org_id)
