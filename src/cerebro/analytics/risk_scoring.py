...
class RiskScoringEngine:
    ...
    async def calculate_organization_risk_score(
        self, org_id: UUID
        ...
        overall_score = (
            vulnerability_score
            * self.dimension_weights[RiskDimension.VULNERABILITY_EXPOSURE]
            + identity_score * self.dimension_weights[RiskDimension.IDENTITY_HYGIENE]
            + access_control_score
            * self.dimension_weights[RiskDimension.ACCESS_CONTROL]
            + compliance_score
            * self.dimension_weights[RiskDimension.COMPLIANCE_POSTURE]
            + operational_score
            * self.dimension_weights[RiskDimension.OPERATIONAL_SECURITY]
        )
        ...
        # Analyze trends
        score_trend, trend_confidence = await self._analyze_score_trend(
    ...
    async def _calculate_identity_score(self, org_id: UUID) -> float:
        ...
        result = await self.db.execute(
            stale_admin_query, {"org_id": org_id, "stale_threshold": stale_threshold}
        )
    ...
    async def _calculate_access_control_score(self, org_id: UUID) -> float:
        ...
        access_risk = excessive_perm_users * 4.0 + public_resource_count * 2.0
    ...
    async def _analyze_score_trend(
        self, org_id: UUID, current_score: float
    ) -> tuple[str, float]:
    ...
    async def _identify_quick_wins(self, org_id: UUID) -> List[str]:
        ...
        for pattern_name, pattern in quick_win_patterns:
            ...
            result = await self.db.execute(
                count_query, {"org_id": org_id, "pattern": pattern}
            )
            ...
            improvement_opportunities.append(
                {
                    ...
                    "potential_reduction": current_risk
                    * 0.7,  # Estimate 70% reduction potential
