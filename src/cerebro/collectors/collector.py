...
class ConfigCollector:
    ...
    async def collect_account(
        ...
        result = CollectionResult(account_id=account.account_id, provider=provider.name)
        ...
        try:
            ...
            logger.info(
                f"Collection completed for account {account.external_id}: "
    ...
    async def _collect_configurations(
        ...
        with collection_metrics.time_collection(
            ...
            try:
                ...
                logger.info(
                    f"Starting concurrent config collection for {len(resources)} resources"
                )
                ...
                config_results = await asyncio.gather(
                    *config_tasks, return_exceptions=True
                )
                ...
                if snapshots_to_insert:
                    with collection_metrics.time_bulk_operation(
                        "config_snapshots", len(snapshots_to_insert)
                    ):
                        inserted_count = (
                            await self.bulk_ops.bulk_insert_config_snapshots(
                ...
                logger.info(
                    f"Config collection completed: {result.config_snapshots} new snapshots"
                )
        ...
        async with semaphore:
            try:
                with collection_metrics.time_provider_api(provider.name, "get_config"):
                    ...
                    config_snapshot = await provider.get_resource_configuration(
                        resource_info
                    )
    ...
    async def _collect_iam_edges(
        ...
        with collection_metrics.time_collection(
            ...
            try:
                ...
                logger.info(
                    f"Loaded {len(principal_map)} principals and {len(resource_map)} resources"
                )
                ...
                async for iam_permission in provider.discover_iam_edges():
                    ...
                    principal_id = principal_map.get(
                        iam_permission.principal_external_id
                    )
                    if not principal_id:
                        logger.debug(
                            f"Principal not found in map: {iam_permission.principal_external_id}"
                        )
                        result.errors.append(
                            f"Principal not found: {iam_permission.principal_external_id}"
                        )
                        continue
                    ...
                    if iam_permission.resource_external_id:
                        resource_id = resource_map.get(
                            iam_permission.resource_external_id
                        )
                        if not resource_id:
                            logger.debug(
                                f"Resource not found in map: {iam_permission.resource_external_id}"
                            )
                    ...
                    edge_data = {
                        ...
                        "effective_at": iam_permission.effective_at
                        or datetime.utcnow(),
                    ...
                    if len(edges_to_insert) >= settings.iam_edge_batch_size:
                        with collection_metrics.time_bulk_operation(
                            "iam_edges", len(edges_to_insert)
                        ):
                            inserted = await self.bulk_ops.bulk_insert_iam_edges(
                                edges_to_insert
                            )
                ...
                collection_metrics.record_iam_edges_collected(
                    provider.name, str(account.account_id), result.iam_edges
                )

                logger.info(
                    f"IAM edge collection completed: {result.iam_edges} new edges from {edge_count} discovered"
                )
