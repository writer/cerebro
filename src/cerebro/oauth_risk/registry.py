...
class OAuthAppRegistry:
                ...
                if repo.get("topics"):
                    ...
                    if "ci" in topics or "deployment" in topics:
                        ...
                        app = OAuthApp(
                            ...
                            scopes=[
                                OAuthScope("actions:read", "Read Actions", AppRiskLevel.LOW, False, False),
                                OAuthScope("contents:write", "Write repository contents", AppRiskLevel.HIGH, False, True),
                                OAuthScope("metadata:read", "Read metadata", AppRiskLevel.LOW, False, False)
                            ],
                            requested_permissions=["actions:read", "contents:write", "metadata:read"],
                            granted_permissions=["actions:read", "contents:write", "metadata:read"],
    ...
    async def _create_oauth_app_from_google_data(self, app_data: Dict[str, Any]) -> OAuthApp:
        ...
        return OAuthApp(
            ...
            last_used=datetime.fromisoformat(app_data["last_used"]) if app_data.get("last_used") else None,
    ...
    async def _create_oauth_app_from_m365_data(self, app_data: Dict[str, Any]) -> OAuthApp:
        ...
        for resource in resource_access:
            ...
            if resource_app_id == "00000003-0000-0000-c000-000000000000":  # Microsoft Graph
        ...
        return OAuthApp(
            ...
            is_internal=True if "company.com" in app_data.get("publisher_domain", "") else False,
    ...
    def _parse_google_scope(self, scope_url: str) -> OAuthScope:
        ...
        return OAuthScope(
            ...
            write_permissions="modify" in scope_url or "write" in scope_url or scope_url.endswith("/auth/drive")
        )
    ...
    def _assess_app_risk_level(self, scopes: List[OAuthScope], app_data: Dict[str, Any]) -> AppRiskLevel:
        """Assess overall risk level for OAuth app."""
        # Count scope risk levels
        critical_scopes = len([s for s in scopes if s.risk_level == AppRiskLevel.CRITICAL])
    ...
    def _identify_risk_factors(self, scopes: List[OAuthScope], app_data: Dict[str, Any]) -> List[str]:
    ...
    def _categorize_app(self, app_name: str) -> AppCategory:
        ...
        # Development tools
        if any(term in name_lower for term in ["github", "gitlab", "jenkins", "ci", "deploy"]):
        ...
        # File sharing
        elif any(term in name_lower for term in ["drive", "dropbox", "box", "sharepoint"]):
            return AppCategory.FILE_SHARING
        
        # Analytics
        elif any(term in name_lower for term in ["analytics", "tableau", "powerbi", "looker"]):
        ...
        # Productivity
        elif any(term in name_lower for term in ["office", "workspace", "calendar", "mail"]):
    ...
    async def get_unused_apps(self, org_id: str, unused_days: int = 90) -> List[OAuthApp]:
    ...
    async def get_apps_by_scope(self, org_id: str, scope_pattern: str) -> List[OAuthApp]:
    ...
    async def generate_oauth_risk_report(self, org_id: str) -> Dict[str, Any]:
        ...
        category_stats = {}
        for app in apps:
            category_stats[app.category.value] = category_stats.get(app.category.value, 0) + 1
