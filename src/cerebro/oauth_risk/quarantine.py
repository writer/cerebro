...
class QuarantineManager:
    ...
    async def evaluate_for_quarantine(
        self, 
        toxic_results: List[ToxicCombinationResult]
        ...
        for result in toxic_results:
            ...
            if result.auto_quarantine_eligible and result.toxicity_score >= self.quarantine_thresholds["auto_quarantine_score"]:
    ...
    async def _create_quarantine_action(
        self, 
        toxic_result: ToxicCombinationResult,
        reason: QuarantineReason
    ) -> QuarantineAction:
        """Create quarantine action for toxic app."""
        action_id = f"quarantine_{toxic_result.app_id}_{int(datetime.now().timestamp())}"
        
        # Determine if immediate quarantine is needed
        immediate = toxic_result.toxicity_score >= self.quarantine_thresholds["immediate_quarantine_score"]
        ...
        action = QuarantineAction(
            ...
            toxic_patterns=[pattern.pattern_id for pattern in toxic_result.toxic_patterns],
            approver=None,
            approved_at=None,
            approval_justification=None,
            restoration_conditions=self._generate_restoration_conditions(toxic_result),
            estimated_restoration_date=datetime.now() + timedelta(days=self.quarantine_thresholds["restoration_review_days"]),
        ...
        logger.warning(f"OAuth app quarantined: {toxic_result.app_name} (score: {toxic_result.toxicity_score})")
        
        return action
    
    def _determine_quarantine_reason(self, toxic_result: ToxicCombinationResult) -> QuarantineReason:
    ...
    def _generate_restoration_conditions(self, toxic_result: ToxicCombinationResult) -> List[str]:
    ...
    async def approve_app_restoration(
        ...
        restoration_request.update({
            ...
            "scheduled_restoration": datetime.now() + timedelta(hours=24)  # 24-hour delay
        })
