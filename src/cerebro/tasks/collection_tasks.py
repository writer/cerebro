    ...
    async def _collect():
        ...
        try:
            ...
            async with async_session_factory() as db:
                ...
                # Update progress
                self.update_state(
                    state="PROGRESS", meta={"status": "Collecting data", "progress": 20}
                )
...
@celery_app.task(
    bind=True, name="cerebro.tasks.collection_tasks.collect_organization_task"
)
def collect_organization_task(
    ...
    async def _collect():
        ...
        try:
            self.update_state(
                state="PROGRESS", meta={"status": "Loading accounts", "progress": 0}
            )
            ...
            async with async_session_factory() as db:
                ...
                for i, account in enumerate(accounts):
                    ...
                    provider_config = {"account_id": str(account.account_id)}
                ...
                logger.info(
                    f"Organization collection task {task_id} scheduled {len(account_tasks)} account tasks"
                )
...
@celery_app.task(
    bind=True, name="cerebro.tasks.collection_tasks.batch_collect_resources"
)
def batch_collect_resources(
    ...
    async def _collect():
        task_id = self.request.id
        logger.info(
            f"Starting batch resource collection {task_id} for {len(resource_external_ids)} resources"
        )
