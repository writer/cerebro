    ...
    async def _health_check():
        ...
        try:
            async with async_session_factory() as db:
                ...
                if current_key:
                    next_rotation = current_key.created_at + timedelta(
...
def setup_periodic_jwt_tasks(sender, **kwargs):
    ...
    sender.add_periodic_task(
        3600.0, rotate_jwt_keys_task.s(), name="jwt-key-rotation-check"  # 1 hour
    )
    ...
    sender.add_periodic_task(
        900.0, jwt_health_check_task.s(), name="jwt-health-check"  # 15 minutes
    )
