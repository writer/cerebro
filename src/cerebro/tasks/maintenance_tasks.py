...
@celery_app.task(
    bind=True, name="cerebro.tasks.maintenance_tasks.cleanup_old_snapshots_task"
)
def cleanup_old_snapshots_task(self, days_old: int = 90):
    ...
    async def _cleanup():
        ...
        logger.info(
            f"Starting cleanup task {task_id} for snapshots older than {days_old} days"
        )
        ...
        try:
            ...
            async with async_session_factory() as db:
                ...
                self.update_state(
                    state="PROGRESS",
                    meta={
                        "status": f"Found {total_old} old snapshots to delete",
                        "progress": 20,
                    },
                )
                ...
                logger.info(
                    f"Cleanup task {task_id} completed: deleted {deleted} old snapshots"
                )
    ...
    async def _vacuum():
        ...
        try:
            ...
            async with async_session_factory() as db:
                ...
                logger.info(
                    f"Vacuum task {task_id} completed for {len(tables_to_vacuum)} tables"
                )
...
@celery_app.task(
    bind=True, name="cerebro.tasks.maintenance_tasks.update_identity_clusters_task"
)
def update_identity_clusters_task(self, org_id: str):
    ...
    async def _update():
        ...
        try:
            ...
            self.update_state(
                state="PROGRESS", meta={"status": "Loading principals", "progress": 0}
            )
            ...
            async with async_session_factory() as db:
                ...
                clusters = await identity_stitcher.find_identity_clusters(
                    UUID(org_id), []
                )
                ...
                self.update_state(
                    state="PROGRESS",
                    meta={
                        "status": f"Found {len(clusters)} identity clusters",
                        "progress": 80,
                    },
                )
                ...
                logger.info(
                    f"Identity cluster task {task_id} completed: "
...
def health_check_task(self):
    ...
    async def _health_check():
        ...
        try:
            ...
            # Database check
            self.update_state(
                state="PROGRESS", meta={"status": "Checking database", "progress": 0}
            )
            ...
            # Redis check
            self.update_state(
                state="PROGRESS", meta={"status": "Checking Redis", "progress": 33}
            )
            ...
            self.update_state(
                state="PROGRESS", meta={"status": "Checking providers", "progress": 66}
            )

            try:
                from cerebro.infrastructure.provider_registry import (
            ...
            # Overall health
            all_healthy = all(
                component["status"] == "healthy" for component in health_status.values()
            )
            ...
            logger.info(
                f"Health check task {task_id} completed: {'healthy' if all_healthy else 'degraded'}"
            )
