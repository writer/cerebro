    ...
    async def _generate():
        ...
        try:
            ...
            async with async_session_factory() as db:
                ...
                self.update_state(
                    state='PROGRESS', 
                    meta={'status': 'Loading rules', 'progress': 10}
                )
                ...
                logger.info(f"Finding generation task {task_id} completed: "
...
def evaluate_rule_batch(
    ...
    async def _evaluate():
        task_id = self.request.id
        logger.info(f"Starting rule evaluation task {task_id} for rule {rule_id} against {len(resource_ids)} resources")
        ...
        try:
            ...
            async with async_session_factory() as db:
                ...
                # Get resources
                stmt = select(Resource).where(Resource.resource_id.in_([UUID(rid) for rid in resource_ids]))
                ...
                self.update_state(
                    state='PROGRESS',
                    meta={'status': f'Evaluating rule against {len(resources)} resources', 'progress': 20}
                )
                ...
                for i, resource in enumerate(resources):
                    try:
                        results = await rule_evaluator.evaluate_resource(resource, [rule])
                        ...
                        logger.warning(f"Rule evaluation failed for resource {resource.resource_id}: {e}")
                        errors.append(f"Resource {resource.external_id}: {str(e)}")
                
                logger.info(f"Rule evaluation task {task_id} completed: {len(matches)} matches, {len(errors)} errors")
...
@celery_app.task(bind=True, name='cerebro.tasks.finding_tasks.update_finding_status_batch')
def update_finding_status_batch(
    ...
    async def _update():
        task_id = self.request.id
        logger.info(f"Starting batch finding status update {task_id} for {len(finding_ids)} findings")
        
        try:
            self.update_state(
                state='PROGRESS',
                meta={'status': 'Loading findings', 'progress': 0}
            )
            ...
            async with async_session_factory() as db:
                ...
                # Get findings
                stmt = select(Finding).where(Finding.finding_id.in_([UUID(fid) for fid in finding_ids]))
                ...
                for i, finding in enumerate(findings):
                    try:
                        ...
                        logger.warning(f"Failed to update finding {finding.finding_id}: {e}")
                        errors.append(f"Finding {finding.finding_id}: {str(e)}")
                
                # Commit changes
                await db.commit()
                
                logger.info(f"Batch finding update task {task_id} completed: {updated} updated, {len(errors)} errors")
