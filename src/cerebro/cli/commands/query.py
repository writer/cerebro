...
@async_command
async def execute_sql(sql: Optional[str], file: Optional[str], output: str, limit: Optional[int], timeout: int):
    """
...
async def show_examples(provider: Optional[str]):
    ...
    if not examples:
        click.echo(f"No examples found{f' for provider {provider}' if provider else ''}.")
        return
...
async def handle_special_command(command: str, query_engine: QueryEngine):
    ...
    if cmd == '.tables':
        ...
        for table in tables:
            click.echo(f"  {table['name']} ({table['provider']}) - {table['description']}")
        click.echo()
