...
from cerebro.agents.service import AgentSessionService, ToolApprovalService, AgentAnalyticsService
...
def create(
    ...
    agent_type: str = typer.Option("security_analyst", "--type", "-t", help="Agent type: security_analyst, compliance_auditor, incident_responder"),
    title: Optional[str] = typer.Option(None, "--title", help="Session title"),
    created_by: str = typer.Option("cli_user", "--user", help="User creating the session"),
    context_file: Optional[str] = typer.Option(None, "--context", help="JSON file with agent context"),
):
        ...
        async with async_session_factory() as db:
            ...
            with Progress(
                ...
                try:
                    ...
                    rprint(f"[dim]Valid types: {', '.join([t.value for t in AgentType])}[/dim]")
        ...
        async with async_session_factory() as db:
            ...
            try:
                ...
                if json_output:
                    ...
                    print(json.dumps({"sessions": session_data, "total": total_count}, indent=2))
                    return
                ...
                # Display as table
                table = Table(title=f"Agent Sessions for '{org_name}' ({total_count} total)")
                ...
                if total_count > limit:
                    ...
                    if end < total_count:
                        rprint(f"[blue]Show more: cerebro agents list {org_name} --offset {offset + limit}[/blue]")
...
def chat(
    ...
    org_name: Optional[str] = typer.Option(None, "--org", help="Organization name for access control"),
                ...
                async for response in service.send_message(
                    ...
                    if response["type"] == "error":
                        ...
                        if tool_info.get("input"):
                            rprint(f"[dim]Input: {json.dumps(tool_info['input'], indent=2)}[/dim]")
                        ...
                        rprint(f"[dim]Approval ID: {approval_info['approval_id']}[/dim]")
                        rprint(f"[dim]Use: cerebro agents approve {approval_info['approval_id']}[/dim]")
...
async def _show_session_status(service: AgentSessionService, session_id: UUID):
    ...
    try:
        ...
        status_table.add_row("Created At", session.created_at.strftime("%Y-%m-%d %H:%M:%S"))
        
        if session.last_activity_at:
            status_table.add_row("Last Activity", session.last_activity_at.strftime("%Y-%m-%d %H:%M:%S"))
...
def status(
    ...
    async def _status():
        ...
        if session.last_activity_at:
            table.add_row("Last Activity", session.last_activity_at.strftime("%Y-%m-%d %H:%M:%S"))
        else:
...
def approve(
    ...
    org_name: str = typer.Argument(..., help="Organization name"),
    approved_by: str = typer.Option("cli_user", "--user", help="User approving the tool"),
    ...
    async def _approve():
        ...
        if approval.status != ApprovalStatus.PENDING:
            rprint(f"[yellow]Approval is not pending (status: {approval.status.value})[/yellow]")
            return
        ...
        details_table.add_row("Tool Name", approval.tool_invocation.tool_name if approval.tool_invocation else "Unknown")
        details_table.add_row("Requested By", approval.requested_by)
        details_table.add_row("Requested At", approval.requested_at.strftime("%Y-%m-%d %H:%M:%S"))
        ...
        if approval.expires_at:
            details_table.add_row("Expires At", approval.expires_at.strftime("%Y-%m-%d %H:%M:%S"))
...
def reject(
    approval_id: str = typer.Argument(..., help="Tool approval ID"),
    org_name: str = typer.Argument(..., help="Organization name"),
    rejected_by: str = typer.Option("cli_user", "--user", help="User rejecting the tool"),
    ...
    async def _reject():
        ...
        # Show basic info
        rprint(f"[yellow]Rejecting tool: {approval.tool_invocation.tool_name if approval.tool_invocation else 'Unknown'}[/yellow]")
...
def pending(
    org_name: str = typer.Argument(..., help="Organization name"),
    limit: int = typer.Option(20, "--limit", "-l", help="Maximum number of approvals to show"),
    offset: int = typer.Option(0, "--offset", help="Offset for pagination"),
    json_output: bool = typer.Option(False, "--json", help="Output as JSON"),
):
    ...
    async def _pending():
        ...
        try:
            ...
            if json_output:
                ...
                print(json.dumps({"approvals": approval_data, "total": total_count}, indent=2))
                return
            ...
            # Display as table
            table = Table(title=f"Pending Tool Approvals for '{org_name}' ({total_count} total)")
            ...
            for approval in approvals:
                ...
                table.add_row(
                    str(approval.id)[:36],
                    approval.tool_invocation.tool_name if approval.tool_invocation else "Unknown",
            ...
            if total_count > limit:
                ...
                if end < total_count:
                    rprint(f"[blue]Show more: cerebro agents pending {org_name} --offset {offset + limit}[/blue]")
    ...
    async def _analytics():
        ...
        with Progress(
            ...
            try:
                ...
                # Display analytics
                rprint(f"[bold blue]Agent Analytics for '{org_name}' (Last {days} days)[/bold blue]\n")
                ...
                if analytics.get("agent_type_stats"):
                    ...
                    agent_table.add_column("Unique Users", style="blue", justify="right")
                ...
                if analytics.get("tool_stats"):
                    ...
                    for stats in analytics["tool_stats"]:
                        status_style = "green" if stats["status"] == "completed" else "red"
...
def delete(
    session_id: str = typer.Argument(..., help="Agent session ID"),
    org_name: str = typer.Argument(..., help="Organization name"),
    deleted_by: str = typer.Option("cli_user", "--user", help="User deleting the session"),
    auto_confirm: bool = typer.Option(False, "--yes", "-y", help="Skip confirmation prompt"),
):
    ...
    async def _delete():
        ...
        rprint(f"[dim]Created: {session.created_at.strftime('%Y-%m-%d %H:%M:%S')}[/dim]")
