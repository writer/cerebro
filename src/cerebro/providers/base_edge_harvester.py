            ...
            try:
                ...
                for page in paginator.paginate():
                    for user in page["Users"]:
                        ...
                        attached_paginator = iam.get_paginator(
                            "list_attached_user_policies"
                        )
                        for attached_page in attached_paginator.paginate(
                            UserName=user_name
                        ):
...
def discover_edge_harvesters() -> dict:
    ...
    logger.info(
        f"Discovered {len(harvesters)} edge harvesters: {list(harvesters.keys())}"
    )
    return harvesters
    ...
    for provider_name, harvester_class in harvesters.items():
        try:
            ...
            if await harvester.validate_connection():
                ...
                logger.warning(
                    f"Skipping {provider_name} - connection validation failed"
                )
