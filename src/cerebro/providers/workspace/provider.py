...
class GoogleWorkspaceProvider(BaseProvider):
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            self._admin_service = await loop.run_in_executor(
                None,
                lambda: build('admin', 'directory_v1', credentials=self._credentials, cache_discovery=False)
            )
            
            # Admin SDK Reports API  
            self._reports_service = await loop.run_in_executor(
                None,
                lambda: build('admin', 'reports_v1', credentials=self._credentials, cache_discovery=False)
            )
            
            # Groups Settings API
            self._groups_settings_service = await loop.run_in_executor(
                None,
                lambda: build('groupssettings', 'v1', credentials=self._credentials, cache_discovery=False)
            )
            
            # Test authentication with a simple API call
            await loop.run_in_executor(
                None,
                lambda: self._admin_service.users().list(domain=self.domain, maxResults=1).execute()
            )
            ...
            while True:
                ...
                for device in result.get('chromeosdevices', []):
                    ...
                    yield ResourceInfo(
                        ...
                        name=device.get('annotatedUser') or device.get('serialNumber') or device['deviceId'],
                        ...
                        created_at=self._parse_timestamp(device.get('firstEnrollmentTime')),
                        account_id=self.account_id,
                        metadata={
                            # Basic device info
    ...
    async def _discover_mobile_devices(self) -> AsyncGenerator[ResourceInfo, None]:
        ...
        try:
            ...
            while True:
                ...
                result = await loop.run_in_executor(
                    None,
                    lambda: self._admin_service.mobiledevices().list(**request_params).execute()
                )
                ...
                for device in result.get('mobiledevices', []):
                    ...
                    yield ResourceInfo(
                        ...
                        name=device.get('name') or device.get('deviceId') or device['resourceId'],
                        ...
                        metadata={
                            ...
                            "device_compromised_status": device.get('compromisedStatus'),
                            "default_language": device.get('defaultLanguage'),
                            
                            # Security info
                            "is_compromised": is_compromised,
                            "adb_status": device.get('adbStatus'),
                            "developer_options_status": device.get('developerOptionsStatus'),
                            "unknown_sources_status": device.get('unknownSourcesStatus'),
    ...
    async def _discover_admin_roles(self) -> AsyncGenerator[ResourceInfo, None]:
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None,
                lambda: self._admin_service.roles().list(customer='my_customer').execute()
            )
    ...
    async def _discover_users(self) -> AsyncGenerator[PrincipalInfo, None]:
        ...
        try:
            ...
            while True:
                ...
                result = await loop.run_in_executor(
                    None,
                    lambda: self._admin_service.users().list(**request_params).execute()
                )
                ...
                for user in result.get('users', []):
                    ...
                    yield PrincipalInfo(
                        ...
                        metadata={
                            ...
                            "include_in_global_address_list": user.get('includeInGlobalAddressList', True),
                            ...
                            "change_password_at_next_login": user.get('changePasswordAtNextLogin', False),
                            
                            # Contact info
                            "phones": [{"value": p.get("value"), "type": p.get("type")} for p in phones],
                            "addresses": [{"formatted": a.get("formatted"), "type": a.get("type")} for a in addresses],
    ...
    async def _discover_groups(self) -> AsyncGenerator[PrincipalInfo, None]:
        ...
        try:
            ...
            while True:
                ...
                result = await loop.run_in_executor(
                    None,
                    lambda: self._admin_service.groups().list(**request_params).execute()
                )
                ...
                for group in result.get('groups', []):
                    ...
                    try:
                        members_result = await loop.run_in_executor(
                            None,
                            lambda: self._admin_service.members().list(groupKey=group_email).execute()
                        )
                        ...
                        logger.warning(f"Could not get members for group {group_email}: {e}")
                    ...
                    try:
                        if self._groups_settings_service:
                            settings_result = await loop.run_in_executor(
                                None,
                                lambda: self._groups_settings_service.groups().get(groupUniqueId=group_email).execute()
                            )
                            group_settings = {
                                'who_can_join': settings_result.get('whoCanJoin'),
                                'who_can_view_membership': settings_result.get('whoCanViewMembership'),
                                'who_can_view_group': settings_result.get('whoCanViewGroup'),
                                'who_can_post_message': settings_result.get('whoCanPostMessage'),
                                'message_moderation_level': settings_result.get('messageModerationLevel'),
                                'is_archived': settings_result.get('isArchived', False),
                                'allow_external_members': settings_result.get('allowExternalMembers', False)
                            }
                    except HttpError as e:
                        logger.debug(f"Could not get settings for group {group_email}: {e}")
    ...
    async def _discover_admin_role_assignments(self) -> AsyncGenerator[IamPermission, None]:
        """Discover admin role assignments."""
        try:
            loop = asyncio.get_event_loop()
            ...
            for role in roles_result.get('items', []):
                ...
                try:
                    assignments_result = await loop.run_in_executor(
                        None,
                        lambda: self._admin_service.roleAssignments().list(
                    ...
                    logger.warning(f"Could not get assignments for role {role_name}: {e}")
            ...
            for group in groups_result.get('groups', []):
                ...
                try:
                    ...
                    for member in members_result.get('members', []):
                        ...
                        is_admin = 'admin' in group_email.lower() or member_role in ['OWNER', 'MANAGER']
