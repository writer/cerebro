...
class OktaProvider(BaseProvider):
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Authenticated with Okta org: {org_info.get('companyName', 'Unknown')}")
            return True
    ...
    async def _discover_policies(self) -> AsyncGenerator[ResourceInfo, None]:
        ...
        try:
            ...
            for policy_type in policy_types:
                response = await self._client.get(f"/api/v1/policies?type={policy_type}")
                response.raise_for_status()
    ...
    async def _get_policy_config(self, policy_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get policy rules
            rules_response = await self._client.get(f"/api/v1/policies/{policy_id}/rules")
    ...
    async def discover_iam_edges(
        ...
        try:
            ...
            for user in users:
                ...
                groups_response = await self._client.get(f"/api/v1/users/{user_id}/groups")
                ...
                for group in user_groups:
                    yield IamPermission(
                        ...
                        is_admin="admin" in group.get("profile", {}).get("name", "").lower()
                    )
                
                # Get user's app assignments
                apps_response = await self._client.get(f"/api/v1/users/{user_id}/appLinks")
