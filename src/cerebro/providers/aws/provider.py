...
class AWSProvider(BaseProvider):
        ...
        try:
            ...
            def _create_session():
                ...
                if identity["Account"] != self.aws_account_id:
                    raise ProviderError(
                        f"Expected account {self.aws_account_id}, got {identity['Account']}"
                    )
            ...
            for index in range(0, len(project_names), 100):
                ...
                for project in response.get("projects", []):
                    ...
                    metadata = {
                        ...
                        "created": (
                            project.get("created").isoformat()
                            if isinstance(project.get("created"), datetime)
                            else None
                        ),
                        "last_modified": (
                            project.get("lastModified").isoformat()
                        ...
                        "environment_type": (project.get("environment") or {}).get(
                            "type"
                        ),
                        "privileged_mode": (project.get("environment") or {}).get(
                            "privilegedMode"
                        ),
                        "badge_enabled": (
                            project.get("badge", {}).get("badgeEnabled")
    ...
    async def _get_load_balancer_config(self, lb_arn: str) -> Dict[str, Any]:
        ...
        def _describe_load_balancer():
                            ...
                            "defaultActions": [
                                {
                                    ...
                                    "fixedResponseConfig": action.get(
                                        "FixedResponseConfig"
                                    ),
                                }
                        ...
                        for instance in reservation.get("Instances", []):
                            ...
                            instance_details[instance_id] = {
                                ...
                                "hasPublicInterface": bool(
                                    public_ip or interface_public_ips
                                ),
            ...
            return {
                ...
                "createdTime": (
                    lb.get("CreatedTime").isoformat() if lb.get("CreatedTime") else None
                ),
                "state": {
                    "code": state.get("Code"),
    ...
    async def _get_s3_bucket_config(self, bucket_name: str) -> Dict[str, Any]:
        ...
        def _get_config():
            ...
            try:
                ...
                config["policyAllowsPublic"] = self._check_s3_policy_public(
                    config["policy"]
                )
                config["policyAllowsPublicWrite"] = self._check_s3_policy_public_write(
                    config["policy"]
                )
            except ClientError as e:
                if e.response["Error"]["Code"] != "NoSuchBucketPolicy":
                    logger.warning(
                        f"Could not get policy for bucket {bucket_name}: {e}"
                    )
                config["policy"] = None
            ...
            try:
                ...
                config["blockPublicAccess"]["effective"] = all(
                    [
                        pab["PublicAccessBlockConfiguration"].get(
                ...
                if (
                    ...
                    logger.warning(
                        f"Could not get public access block for bucket {bucket_name}: {e}"
                    )
            ...
            try:
                ...
                logger.debug(
                    f"Could not determine bucket location for {bucket_name}: {e}"
                )
                config["region"] = None
            ...
            try:
                ...
                if e.response["Error"]["Code"] not in {
                    ...
                    logger.debug(
                        f"Could not get encryption for bucket {bucket_name}: {e}"
                    )
            ...
            try:
                ...
                config["logging"] = {
                    ...
                    "target_bucket": (
                        logging_enabled.get("TargetBucket") if logging_enabled else None
                    ),
                }
            except ClientError as e:
                logger.debug(
                    f"Could not get logging configuration for bucket {bucket_name}: {e}"
                )
    ...
    async def _get_codebuild_project_config(self, project_name: str) -> Dict[str, Any]:
        ...
        def _get_project():
            ...
            return {
                ...
                "secondarySources": [
                    _serialize_source(src)
                    for src in project.get("secondarySources", [])
                ],
    ...
    def _check_s3_acl_public_write(self, acl: Dict[str, Any]) -> bool:
        ...
        for grant in acl["Grants"]:
            grantee = grant.get("Grantee", {})
            if (
                grantee.get("URI") in public_uris
                and grant.get("Permission") in writable_permissions
            ):
                return True
    ...
    def _check_s3_policy_public_write(self, policy: Dict[str, Any]) -> bool:
        ...
        for statement in policy["Statement"]:
            ...
            if any(
                action in {"s3:*", "s3:putobject", "s3:putobjectacl", "s3:deleteobject"}
    ...
    def _analyze_user_permissions(
        ...
        for group in user_groups["Groups"]:
            group_permissions = self._analyze_group_permissions_for_user(
                iam, group, user_arn
            )
            permissions.extend(group_permissions)
    ...
    def _is_admin_policy(self, policy_arn: str) -> bool:
        ...
        return (
            policy_arn in admin_policies
            or "Administrator" in policy_arn
            or "FullAccess" in policy_arn
        )
