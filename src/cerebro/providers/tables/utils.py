...
def parse_timestamp_safely(timestamp_str: Optional[str], provider: str = "unknown") -> Optional[datetime]:
    """
    ...
    try:
        ...
        logger.error(f"Unexpected error parsing {provider} timestamp '{timestamp_str}': {e}")
        return None


def validate_string_input(input_str: Optional[str], field_name: str, max_length: int = 255) -> Optional[str]:
    """
    ...
    try:
        ...
        if len(cleaned) > max_length:
            logger.warning(f"{field_name} exceeds max length ({len(cleaned)} > {max_length}): {cleaned[:50]}...")
