...
class AWSEc2InstanceTable(ProviderSecurityTable):
    ...
    def __init__(self):
        ...
        ec2_columns = [
            SecurityColumn("instance_id", ColumnType.TEXT, "EC2 instance ID", required=True, source_field="InstanceId"),
            SecurityColumn("instance_type", ColumnType.TEXT, "EC2 instance type", source_field="InstanceType"),
            SecurityColumn("state", ColumnType.TEXT, "Instance state", source_field="State.Name"),
            SecurityColumn("public_ip", ColumnType.TEXT, "Public IP address", source_field="PublicIpAddress"),
            SecurityColumn("private_ip", ColumnType.TEXT, "Private IP address", source_field="PrivateIpAddress"),
            SecurityColumn("vpc_id", ColumnType.TEXT, "VPC ID", source_field="VpcId"),
            SecurityColumn("subnet_id", ColumnType.TEXT, "Subnet ID", source_field="SubnetId"),
            SecurityColumn("security_groups", ColumnType.JSON, "Security group IDs", source_field="SecurityGroups"),
            SecurityColumn("key_name", ColumnType.TEXT, "Key pair name", source_field="KeyName"),
            SecurityColumn("iam_instance_profile", ColumnType.JSON, "IAM instance profile", source_field="IamInstanceProfile"),
            SecurityColumn("monitoring", ColumnType.JSON, "CloudWatch monitoring", source_field="Monitoring"),
            SecurityColumn("placement", ColumnType.JSON, "Placement information", source_field="Placement"),
        ]
...
class AWSIAMUserTable(ProviderSecurityTable):
    ...
    def __init__(self):
        ...
        iam_columns = [
            SecurityColumn("user_name", ColumnType.TEXT, "IAM username", required=True, source_field="UserName"),
            SecurityColumn("user_id", ColumnType.TEXT, "IAM user ID", source_field="UserId"),
            ...
            SecurityColumn("password_last_used", ColumnType.TIMESTAMP, "Password last used", source_field="PasswordLastUsed"),
            SecurityColumn("mfa_enabled", ColumnType.BOOLEAN, "MFA enabled", transform="check_mfa_enabled"),
            SecurityColumn("access_keys", ColumnType.JSON, "Access keys", transform="get_access_keys"),
            SecurityColumn("attached_policies", ColumnType.JSON, "Attached managed policies", transform="get_attached_policies"),
            SecurityColumn("inline_policies", ColumnType.JSON, "Inline policies", transform="get_inline_policies"),
            SecurityColumn("groups", ColumnType.JSON, "Group memberships", transform="get_user_groups"),
        ]
...
class AWSSecurityGroupTable(ProviderSecurityTable):
    ...
    def __init__(self):
        sg_columns = [
            SecurityColumn("group_id", ColumnType.TEXT, "Security group ID", required=True, source_field="GroupId"),
            SecurityColumn("group_name", ColumnType.TEXT, "Security group name", source_field="GroupName"),
            SecurityColumn("description", ColumnType.TEXT, "Security group description", source_field="Description"),
            SecurityColumn("vpc_id", ColumnType.TEXT, "VPC ID", source_field="VpcId"),
            SecurityColumn("owner_id", ColumnType.TEXT, "Owner account ID", source_field="OwnerId"),
            SecurityColumn("ingress_rules", ColumnType.JSON, "Ingress rules", source_field="IpPermissions"),
            SecurityColumn("egress_rules", ColumnType.JSON, "Egress rules", source_field="IpPermissionsEgress"),
        ]
            ...
            for region in regions:
                ...
                async for page in paginator.paginate():
                    for sg in page.get('SecurityGroups', []):
                        ...
                        sg['Provider'] = 'aws'
                        sg['CreatedAt'] = datetime.now()  # SGs don't have creation time in API
