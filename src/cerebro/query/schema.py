...
class SecuritySchema:
    ...
    CORE_COLUMNS = [
        SecurityColumn("id", ColumnType.TEXT, "Unique resource identifier", required=True),
        SecurityColumn("provider", ColumnType.TEXT, "Security provider (aws, okta, crowdstrike, etc.)", required=True),
        ...
        SecurityColumn("created_at", ColumnType.TIMESTAMP, "Resource creation timestamp"),
        SecurityColumn("updated_at", ColumnType.TIMESTAMP, "Last modification timestamp"),
        SecurityColumn("tags", ColumnType.JSON, "Resource tags and labels"),
        SecurityColumn("metadata", ColumnType.JSON, "Additional provider-specific metadata"),
    ]
    ...
    ALERT_COLUMNS = CORE_COLUMNS + [
        ...
        SecurityColumn("aggregate_id", ColumnType.TEXT, "Aggregate alert group identifier"),
    ...
    IDENTITY_COLUMNS = CORE_COLUMNS + [
        ...
        SecurityColumn("mfa_enabled", ColumnType.BOOLEAN, "Multi-factor authentication enabled"),
        SecurityColumn("locked", ColumnType.BOOLEAN, "Account locked status"),
        SecurityColumn("password_changed", ColumnType.TIMESTAMP, "Last password change"),
    ...
    ASSET_COLUMNS = CORE_COLUMNS + [
        ...
        SecurityColumn("last_seen", ColumnType.TIMESTAMP, "Last communication timestamp"),
        ...
        SecurityColumn("network_interfaces", ColumnType.JSON, "Network interface details"),
        SecurityColumn("installed_software", ColumnType.JSON, "Installed software inventory"),
    ]
    
    # Vulnerability columns (CrowdStrike Spotlight, AWS Inspector)
    VULNERABILITY_COLUMNS = CORE_COLUMNS + [
        SecurityColumn("vulnerability_id", ColumnType.TEXT, "Vulnerability identifier", required=True),
        ...
        SecurityColumn("exploit_available", ColumnType.BOOLEAN, "Known exploits available"),
    ...
    CONFIG_COLUMNS = CORE_COLUMNS + [
        SecurityColumn("resource_type", ColumnType.TEXT, "Resource type", required=True),
        ...
        SecurityColumn("configuration_recorder", ColumnType.TEXT, "Config recorder name"),
    ...
    @classmethod 
    def create_table_schema(cls, table_name: str, custom_columns: Optional[List[SecurityColumn]] = None) -> Dict[str, Any]:
    ...
    def _get_default_indexes(cls, columns: List[SecurityColumn]) -> List[SecurityIndex]:
        ...
        if any(col.name == "provider" for col in columns):
            indexes.append(SecurityIndex("idx_provider_account", ["provider", "account_id"]))
            
        # Index severity and status for filtering
        filterable_columns = [col.name for col in columns if col.name in ["severity", "status"] and col.filterable]
