    ...
    def parse_query(self, sql: str) -> QueryPlan:
        ...
        try:
            ...
            if "*" in table_name:
                ...
                try:
                    max_tables = int(
                        os.getenv("QUERY_ENGINE_MAX_WILDCARD_TABLES", "25")
                    )
                except ValueError:
                    logger.warning(
                ...
                logger.info(
                    f"Expanded wildcard pattern '{table_name}' to {len(matching_tables)} tables: {matching_tables}"
                )
    ...
    def _extract_table_name(self, parsed) -> str:
        ...
        for token in parsed.flatten():
            ...
            if (
                ...
                and token.value.upper() in stop_keywords
            ):
                break
            if token.value == ";":
                break
    ...
    def _extract_selected_columns(self, parsed: Any) -> list[str]:
        ...
        for token in parsed.flatten():
            ...
            # Check for FROM keyword
                ...
                token.ttype is sqlparse.tokens.Keyword
                or token.ttype is sqlparse.tokens.Keyword.DML
            ):
    ...
    def _extract_filters(self, parsed) -> list[QueryFilter]:
        ...
        for token in parsed.flatten():
            if (
                ...
                where_seen
                and token.ttype is sqlparse.tokens.Keyword
                and token.value.upper() in ["ORDER", "LIMIT", "OFFSET"]
            ):
                break
    ...
    def _extract_order_by(self, parsed: Any) -> list[str] | None:
        ...
        for token in parsed.flatten():
            if (
                ...
                order_by_seen
                and token.ttype is sqlparse.tokens.Keyword
                and token.value.upper() in ["LIMIT", "OFFSET"]
            ):
                break
...
class QueryEngine:
    ...
    async def execute_query(
        ...
        try:
            ...
            tables_queried = (
                plan.wildcard_tables if plan.wildcard_tables else [plan.table_name]
            )
    ...
    def _validate_query(self, plan: QueryPlan) -> list[str]:
        ...
        if not table:
            ...
            errors.append(
                f"Table '{plan.table_name}' not found. Available tables: {', '.join(available_tables)}"
            )
            return errors
            ...
            try:
                ...
                async for resource in table.list_resources(ctx_no_limits):
                    ...
                    if ctx_no_limits.columns and ctx_no_limits.columns != ["*"]:
                        resource = {
                            col: resource.get(col) for col in ctx_no_limits.columns
                        }
            ...
            if info:
                ...
                summary = {
                    ...
                    "filterable_columns": len(
                        [c for c in info["columns"] if c["filterable"]]
                    ),
                }
