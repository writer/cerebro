...
class RuleService:
    ...
    async def ensure_library_rules_exist(self, org_id: UUID) -> Dict[str, UUID]:
        ...
        stmt = select(Policy).where(
            and_(Policy.org_id == org_id, Policy.name == "Security Baseline")
        )
        ...
        for template in templates:
            ...
            if existing_rule:
                ...
                new_rule = await self.create_rule_from_template(
                    template.name, policy_id=policy.policy_id
                )
                rule_mapping[self._normalize_rule_name(template.name)] = (
                    new_rule.rule_id
                )
    ...
    async def create_rule_from_producer(
        ...
        rule = Rule(
            ...
            nist_800_53=producer_metadata.get("framework_mappings", {}).get(
                "nist_800_53", []
            ),
    ...
    async def sync_rules_with_producers(self, org_id: UUID) -> Dict[str, Any]:
        ...
        # Get default policy
        stmt = select(Policy).where(
            and_(Policy.org_id == org_id, Policy.name == "Producer Rules")
        )
        policy = await self.db.scalar(stmt)
        ...
        for producer_info in producer_infos:
            ...
            if existing_rule:
                ...
                cel_expression = (
                    f"producer.{producer_info['rule_name']}(resource, config)"
                )

                new_rule = await self.create_rule_from_producer(
                    producer_info, policy.policy_id, cel_expression
                )
...
def get_rule_by_name_sync(rule_name: str) -> UUID:
    ...
    # Check cache first
    rule_name_key = (
        rule_name.lower().replace(" ", "_").replace(":", "").replace("-", "_")
    )
...
async def get_rule_by_name_async(rule_name: str, db_session: AsyncSession) -> UUID:
    ...
    if not rule_id:
        ...
        logger.warning(
            f"Rule not found in DB, using deterministic UUID for {rule_name}: {rule_id}"
        )
