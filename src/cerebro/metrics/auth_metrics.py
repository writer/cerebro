...
class AuthMetrics:
    ...
    def record_login_attempt(
        success: bool, ip_address: str, duration: float, locked: bool = False
    ) -> None:
    ...
    def record_authorization_check(
        endpoint: str, required_scope: str, allowed: bool
    ) -> None:
        """Record API authorization check."""
        result = "allowed" if allowed else "denied"
        authorization_checks.labels(
            endpoint=endpoint, required_scope=required_scope, result=result
        ).inc()
