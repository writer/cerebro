...
class ReachabilityAnalyzer:
    ...
    async def analyze_principal_reachability(
        ...
        overall_risk_score = self._calculate_overall_risk_score(paths_to_targets, simulation_result)
        
        # Identify critical paths
        critical_paths = [
            path for path in paths_to_targets 
            if path.severity.value in ["high", "critical"]
        ]
        
        # Generate recommendations
        mitigations = self._generate_comprehensive_mitigations(paths_to_targets, simulation_result)
        monitoring_recs = self._generate_monitoring_recommendations(principal_id, paths_to_targets)
        ...
        result = ReachabilityResult(
            ...
            shortest_path_length=min((p.path_length for p in paths_to_targets), default=None),
            most_exploitable_path=max(paths_to_targets, key=lambda p: p.exploitability_score, default=None),
            overall_risk_score=overall_risk_score,
            blast_radius_size=simulation_result["blast_radius"]["total_reachable_resources"],
        ...
        logger.info(f"Reachability analysis complete: {len(paths_to_targets)} paths found")
    ...
    async def analyze_resource_reachability(
        ...
        overall_risk_score = self._calculate_resource_exposure_risk(resource_id, paths_to_resource)
        
        # Identify critical access paths
        critical_paths = [
            path for path in paths_to_resource
            if path.severity.value in ["high", "critical"]
        ]
        
        # Generate recommendations
        mitigations = self._generate_resource_protection_mitigations(resource_id, paths_to_resource)
        monitoring_recs = self._generate_resource_monitoring_recommendations(resource_id, paths_to_resource)
        ...
        result = ReachabilityResult(
            ...
            shortest_path_length=min((p.path_length for p in paths_to_resource), default=None),
            most_exploitable_path=max(paths_to_resource, key=lambda p: p.exploitability_score, default=None),
            overall_risk_score=overall_risk_score,
            blast_radius_size=len(paths_to_resource),  # Number of ways to reach resource
            critical_paths=critical_paths,
            mitigations=mitigations,
            monitoring_recommendations=monitoring_recs,
            analysis_duration_ms=analysis_duration,
            ...
            return {
                ...
                "results": {
                    ...
                    "cross_provider_reach": len([
                        p for p in result.paths_found 
                        if len(set(step.source_node.split("_")[0] for step in p.steps)) > 1
                    ]),
                    "automation_risk": "high" if "actions" in service_id.lower() else "medium"
                }
            }
    ...
    def _generate_comprehensive_mitigations(
        ...
        if blast_radius["critical_resources"] > 0:
            all_mitigations.append("Implement additional controls for critical resource access")
        
        if blast_radius["average_steps_to_critical"] < 3:
            all_mitigations.append("Add intermediate approval steps for critical resource paths")
    ...
    def _generate_monitoring_recommendations(
        ...
        if any("service" in step.source_node.lower() for path in paths for step in path.steps):
    ...
    def _generate_resource_protection_mitigations(
        ...
        if paths:
            ...
            if highest_impact.impact_score >= 0.8:
                mitigations.append("Implement break-glass access for high-impact resource")
...
async def analyze_attack_path(
    ...
    return await analyzer.analyze_principal_reachability(org_id, source_principal, max_steps)
