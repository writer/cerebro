...
class ServiceIdentityMapper:
    ...
    async def _discover_azure_managed_identity(self) -> List[ServiceIdentityEdge]:
        ...
        try:
            ...
            if not azure_client:
                logger.warning(
                    "Azure client not available - skipping managed identity discovery"
                )
                return edges

            logger.info(
                "Discovering Azure Managed Identity relationships via Azure ARM API"
            )
                ...
                try:
                    ...
                    for assignment in role_assignments:
                        edge = ServiceIdentityEdge(
                            ...
                            source_service=identity.get(
                                "assigned_resource", identity["name"]
                            ),
                            target_service=assignment.get(
                                "target_resource", "azure_resource"
                            ),
                            ...
                            risk_score=self._calculate_azure_identity_risk(
                                identity, assignment
                            ),
                            exploitability=self._assess_azure_exploitability(
                                assignment
                            ),
                            discovered_at=datetime.now(),
                            last_verified=datetime.now(),
                    ...
                    logger.error(
                        f"Failed to process Azure identity {identity.get('name', 'unknown')}: {e}"
                    )
                    continue

            logger.info(
                f"Discovered {len(edges)} Azure managed identity trust relationships"
            )
            return edges
    ...
    async def _get_azure_client(self):
        ...
        try:
            ...
            if not subscription_id:
                logger.warning(
                    "Azure subscription ID not configured (set AZURE_SUBSCRIPTION_ID)"
                )
                return None
            ...
            logger.warning(
                "Azure SDK not installed - install with 'pip install azure-identity azure-mgmt-authorization azure-mgmt-msi azure-mgmt-resource'"
            )
            return None
        except Exception as e:
    ...
    async def _query_azure_managed_identities(self, azure_client) -> List[Dict]:
        ...
        try:
            ...
            for resource in resource_client.resources.list():
                if (
                    hasattr(resource, "identity")
                    and resource.identity
                    and resource.identity.type
                ):
    ...
    async def _get_identity_role_assignments(
        ...
        try:
            ...
            for assignment in auth_client.role_assignments.list():
                if assignment.principal_id and str(assignment.principal_id) == str(
                    principal_id
                ):
                    ...
                    assignments.append(
                        {
                            "role_name": (
                                role_def.role_name if role_def else "Unknown Role"
                            ),
                            "role_id": assignment.role_definition_id.split("/")[-1],
                            "scope": assignment.scope,
                            "target_resource": self._extract_resource_from_scope(
                                assignment.scope
                            ),
                        }
                    )
            ...
            logger.error(
                f"Failed to get role assignments for principal {principal_id}: {e}"
            )
            return []
    ...
    async def _discover_k8s_service_accounts(self) -> List[ServiceIdentityEdge]:
        ...
        try:
            ...
            if not k8s_client:
                logger.warning(
                    "Kubernetes client not available - skipping service account discovery"
                )
                return edges

            logger.info(
                "Discovering Kubernetes service account mappings via Kubernetes API"
            )
                ...
                try:
                    ...
                    if "iam.gke.io/gcp-service-account" in annotations:
                        ...
                        edge = ServiceIdentityEdge(
                            ...
                            risk_score=self._calculate_k8s_workload_identity_risk(
                                sa, annotations
                            ),
                            exploitability=self._assess_k8s_exploitability(
                                sa, annotations
                            ),
                            discovered_at=datetime.now(),
                            last_verified=datetime.now(),
                    ...
                    logger.error(
                        f"Failed to process Kubernetes service account {sa.get('name', 'unknown')}: {e}"
                    )
                    continue

            logger.info(
                f"Discovered {len(edges)} Kubernetes service account trust relationships"
            )
            return edges
    ...
    async def _query_k8s_service_accounts(self, k8s_client) -> List[Dict]:
        ...
        try:
            ...
            for sa in sa_list.items:
                ...
                if sa.metadata.name in ["default"] or sa.metadata.namespace.startswith(
                    "kube-"
                ):
                    continue
    ...
    def _calculate_k8s_workload_identity_risk(
        ...
        if any(
            sys_ns in namespace for sys_ns in ["kube-system", "kube-public", "default"]
        ):
            risk_score += 0.3
    ...
    def _assess_k8s_exploitability(self, sa: Dict, annotations: Dict) -> str:
        ...
        if any(
            admin in (role_arn + gcp_sa).lower()
            for admin in ["admin", "poweruser", "owner"]
        ):
            return "high"
    ...
    async def analyze_service_identity_risks(self, org_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": self._generate_service_identity_recommendations(
                service_edges
            ),
        }
            ...
            patterns.append(
                {
                    ...
                    "severity": "high",
                    "examples": [
                        edge.edge_id for edge in unconditioned_cross_provider[:3]
                    ],
                }
            )
    ...
    def _generate_service_identity_recommendations(
        ...
        broad_access_edges = [
            edge for edge in edges if "*" in edge.allowed_repositories
        ]
