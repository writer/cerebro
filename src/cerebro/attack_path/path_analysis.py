...
class PathAnalyzer:
    ...
    async def _find_shortest_paths(self, query: PathQuery) -> List[AttackPath]:
        ...
        if query.source_principal and query.target_resource:
            try:
                ...
                logger.debug(f"No path found from {query.source_principal} to {query.target_resource}")
        
        # If only target specified, find paths from all high-privilege principals
        elif query.target_resource:
            high_privilege_principals = self.attack_graph.get_high_privilege_principals(10)
    ...
    async def _find_k_step_escalation_paths(self, query: PathQuery) -> List[AttackPath]:
        ...
        if query.target_resource:
            ...
            try:
                ...
                k_step_paths = [path for path in all_paths if len(path) == k + 1]  # +1 because path includes start node
    ...
    async def _find_privilege_escalation_paths(self, query: PathQuery) -> List[AttackPath]:
    ...
    async def _convert_to_attack_path(self, path: List[str], query: PathQuery) -> Optional[AttackPath]:
        ...
        impact_score = self._calculate_impact_score(path[-1])  # Impact of reaching target
    ...
    def _calculate_path_severity(self, exploitability: float, impact: float) -> PathSeverity:
    ...
    def _generate_path_mitigations(self, steps: List[AttackStep]) -> List[str]:
        ...
        has_service_account_step = any("service" in step.source_node.lower() for step in steps)
        has_high_privilege_step = any(step.privilege_level >= 2 for step in steps)
        has_cross_provider_step = len(set(
    ...
    async def simulate_principal_compromise(
            ...
            if depth < max_steps:
                ...
                for neighbor in self.graph.successors(current_node):
                    if neighbor not in visited:
                        ...
                        if neighbor_node and neighbor_node.node_type.value == "resource":
        ...
        reachable_resources.sort(key=lambda r: (
            {"critical": 4, "high": 3, "medium": 2, "low": 1}.get(r["criticality"], 1),
        ...
        return {
            ...
            "blast_radius": {
                ...
                "critical_resources": len([r for r in reachable_resources if r["criticality"] == "critical"]),
                "high_risk_resources": len([r for r in reachable_resources if r["criticality"] == "high"]),
                "average_steps_to_critical": sum(
                    r["steps_to_reach"] for r in reachable_resources if r["criticality"] == "critical"
                ) / max(len([r for r in reachable_resources if r["criticality"] == "critical"]), 1)
            },
