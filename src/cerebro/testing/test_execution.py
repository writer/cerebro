...
class TestExecution(Base):
    ...
    # Execution metadata
    execution_status: Mapped[str] = mapped_column(String(50), nullable=False, default=ExecutionStatus.PENDING.value)
    priority: Mapped[str] = mapped_column(String(20), nullable=False, default=TestPriority.MEDIUM.value)
    environment: Mapped[str] = mapped_column(String(50), nullable=False)
    
    # Timing
    scheduled_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=func.now())
...
class TestExecutor:
    ...
    async def execute_test(
        ...
        try:
            ...
            logger.info(f"Test {test_id} completed successfully in {execution_time_ms:.1f}ms")
            return test_result
            ...
            execution.error_message = f"Test execution timed out after {timeout_seconds} seconds"
    ...
    async def execute_test_suite(
        ...
        if parallel:
            return await self._execute_tests_parallel(
                test_ids, org_id, environment, executed_by, test_functions, max_concurrent
            )
        else:
    ...
    async def get_execution_history(
        ...
        stmt = stmt.order_by(desc(TestExecution.scheduled_at)).offset(offset).limit(limit)
    ...
    async def get_execution_metrics(self, org_id: UUID) -> Dict[str, Any]:
        ...
        stmt = select(TestExecution).where(
            and_(
                ...
                TestExecution.scheduled_at >= since_date
            )
        )
        ...
        successful_executions = len([e for e in executions if e.test_status == TestStatus.PASS.value])
    ...
    async def execute_scheduled_tests(self, environment: str) -> List[TestResult]:
        ...
        for execution in scheduled_executions:
            try:
                ...
                logger.error(f"Failed to execute scheduled test {execution.test_id}: {e}")
