...
class TestEntityManager:
    ...
    async def create_test_entity(
        ...
        logger.info(
            f"Created test entity {entity.entity_id} of type {entity_type.value}"
        )
        return entity
    ...
    async def _cleanup_entity_resources(self, entity: TestEntity) -> None:
        ...
        logger.debug(
            f"Cleaned up resources for {entity_type.value} entity {entity.entity_id}"
        )
    ...
    async def get_entity_dependencies(self, entity_id: UUID) -> List[TestEntity]:
        ...
        stmt = select(TestEntity).where(TestEntity.entity_id.in_(entity.depends_on))
    ...
    async def get_test_entity_summary(self, org_id: UUID) -> Dict[str, Any]:
        ...
        total_stmt = (
            select(func.count())
            .select_from(TestEntity)
            .where(TestEntity.org_id == org_id)
        )
        total = await self.db.scalar(total_stmt) or 0

        # Count by type
        type_stmt = (
            select(TestEntity.entity_type, func.count().label("count"))
            .where(TestEntity.org_id == org_id)
            .group_by(TestEntity.entity_type)
        )
        ...
        # Count by status
        status_stmt = (
            select(TestEntity.status, func.count().label("count"))
            .where(TestEntity.org_id == org_id)
            .group_by(TestEntity.status)
        )
        ...
        env_stmt = (
            select(TestEntity.environment, func.count().label("count"))
            .where(TestEntity.org_id == org_id)
            .group_by(TestEntity.environment)
        )
