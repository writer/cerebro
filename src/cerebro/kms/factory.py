...
def get_kms() -> BaseKMS:
    ...
    if kms_provider == "aws":
        ...
        return GCPKMS(
            key_name=key_name, credentials_path=settings.google_application_credentials
        )
        ...
        return AzureKeyVaultKMS(vault_url=vault_url, key_name=key_name)
        ...
        return VaultTransitKMS(
            vault_url=vault_url, mount_path=mount_path, key_name=key_name
        )
...
def get_available_kms_providers() -> list:
    ...
    # Check AWS
    if hasattr(settings, "aws_kms_key_id") and hasattr(settings, "aws_access_key_id"):
        providers.append("aws")

    # Check GCP
    if hasattr(settings, "gcp_kms_key_name") and hasattr(
        settings, "google_application_credentials"
    ):
        providers.append("gcp")

    # Check Azure
    if hasattr(settings, "azure_vault_url") and hasattr(settings, "azure_key_name"):
        providers.append("azure")

    # Check Vault
    if hasattr(settings, "vault_url") and hasattr(settings, "vault_key_name"):
