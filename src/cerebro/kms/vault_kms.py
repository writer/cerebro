...
class VaultTransitKMS(BaseKMS):
    ...
    def __init__(
        ...
        if not self.token:
            raise ValueError("Vault token required (VAULT_TOKEN env var or token parameter)")
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    raise Exception(f"Vault encrypt failed: {response.status_code} {response.text}")
    ...
    async def decrypt(self, ciphertext: bytes) -> bytes:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    raise Exception(f"Vault decrypt failed: {response.status_code} {response.text}")
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Vault Transit connection test: {'passed' if success else 'failed'}")
            return success
