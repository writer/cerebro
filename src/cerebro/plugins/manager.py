...
class PluginManager:
    ...
    async def load_plugin(self, plugin_name: str) -> bool:
        ...
        plugin_metadata = next(
            (p for p in available_plugins if p.name == plugin_name), 
            None
        )
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                plugin_name,
                Path(plugin_metadata.module_path) / "__init__.py"
            )
            ...
            if not self._validate_plugin_interface(plugin_class, plugin_metadata.plugin_type):
                raise ValueError(f"Plugin {plugin_name} does not implement required interface")
    ...
    def get_loaded_plugins(self, plugin_type: Optional[str] = None) -> List[PluginMetadata]:
