...
class IdentityService:
    ...
    async def stitch_identities(self, org_id: UUID) -> Dict:
        ...
        if not self.stitcher:
            logger.warning(
                "Identity stitcher not available - database session required"
            )
    ...
    async def get_risky_identities(self, org_id: UUID, limit: int = 20) -> List[Dict]:
        ...
        if not self.identity_analyzer:
            logger.warning(
                "Identity analyzer not available - database session required"
            )
            return []
    ...
    async def analyze_privilege_sprawl(self, org_id: UUID) -> Dict:
        ...
        if not self.sprawl_detector:
            logger.warning(
                "Privilege sprawl detector not available - database session required"
            )
            return {
                "total_identities": 0,
        ...
        try:
            ...
            sprawl_analysis = await self.sprawl_detector.analyze_privilege_sprawl(
                org_id
            )
            ...
            return {
                ...
                "recommendations": self._generate_sprawl_recommendations(
                    sprawl_analysis
                ),
            }
    ...
    def _calculate_sprawl_score(self, analysis) -> float:
        ...
        factors = {
            "cross_provider_ratio": analysis.cross_provider_identities
            / analysis.total_identities,
            "high_privilege_ratio": analysis.high_privilege_identities
            / analysis.total_identities,
            "avg_permissions_factor": min(
                analysis.avg_permissions_per_identity / 50.0, 1.0
            ),
            "max_permissions_factor": min(
                analysis.max_permissions_per_identity / 200.0, 1.0
            ),
        }
    ...
    def _generate_sprawl_recommendations(self, analysis) -> List[str]:
        ...
        if analysis.cross_provider_identities > analysis.total_identities * 0.3:
            recommendations.append(
                "Implement centralized identity federation to reduce cross-provider sprawl"
            )

        if analysis.high_privilege_identities > analysis.total_identities * 0.1:
            recommendations.append(
                "Review and revoke excessive administrative privileges"
            )

        if analysis.avg_permissions_per_identity > 30:
            recommendations.append(
                "Implement role-based access control to reduce individual permission grants"
            )

        if analysis.max_permissions_per_identity > 100:
            recommendations.append(
                "Audit identities with excessive permissions (>100 per identity)"
            )

        recommendations.append(
            "Schedule regular access reviews for high-privilege identities"
        )
        recommendations.append(
