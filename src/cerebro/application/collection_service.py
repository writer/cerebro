...
class CollectionService:
    ...
    async def collect_account(
        ...
        job = CollectionJobEntity(
            org_id=org_id, provider=provider.name, resource_types=resource_types or []
        )

        try:
            job.start()
            logger.info(
                f"Starting collection job {job.job_id} for provider {provider.name}"
            )

            # Step 1: Discover and save resources
            resources = await self._collect_resources(provider, resource_types)
            if resources:
                resource_id_map = await self.repository.save_resources(
            ...
            if principals:
                principal_id_map = await self.repository.save_principals(
                    account_id, principals
                )
            ...
            if configs:
                configs_saved = await self.repository.save_configurations(
                    resource_id_map, configs
                )
    ...
    async def _stitch_identities(
        ...
        try:
            clusters = await self.identity_stitcher.find_identity_clusters(
                org_id, principals
            )
