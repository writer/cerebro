...
class UserService:
    ...
    async def add_user_scopes(
        ...
        for scope_name in missing_scopes:
            scope = Scope(
                name=scope_name, description=f"Auto-created scope: {scope_name}"
            )
            self.db.add(scope)
            await self.db.flush()
    ...
    async def create_default_scopes(self) -> None:
        ...
        default_scopes = [
            ...
            ("view:integrations", "View integration synchronization status"),
        ]
    ...
    async def create_admin_user(
        ...
        return AdminUserCreationResult(
            user=admin_user, generated_password=generated_password
        )
