...
class IdentityRepository:
    ...
    async def get_cluster(self, org_id: UUID, cluster_name: str) -> Optional[IdentityCluster]:
    ...
    async def create_cluster(self, org_id: UUID, cluster_name: str, **kwargs) -> IdentityCluster:
    ...
    async def get_cluster_members(self, cluster_id: UUID) -> List[IdentityClusterMember]:
        """Get all members of an identity cluster."""
        stmt = select(IdentityClusterMember).where(
            IdentityClusterMember.cluster_id == cluster_id
...
class ResourceRepository:
    ...
    async def get_by_external_id(self, account_id: UUID, external_id: str) -> Optional[Resource]:
...
class FindingRepository:
    ...
    async def get_active_by_severity(self, org_id: UUID, severity: str) -> List[Finding]:
    ...
    def _calculate_sla_breaches(self, findings: List[Finding]) -> dict:
        ...
        for finding in findings:
            if finding.resolved_at and finding.created_at:
                resolution_hours = (finding.resolved_at - finding.created_at).total_seconds() / 3600
