...
class Scope(Base):
    ...
    # Relationships
    user_scopes: Mapped[List["UserScope"]] = relationship(back_populates="scope", cascade="all, delete-orphan")
...
class UserScope(Base):
    ...
    user_scope_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    user_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("users.user_id", ondelete="CASCADE"))
    scope_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("scopes.scope_id", ondelete="CASCADE"))
