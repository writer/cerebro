...
class AccessReviewManager:
    ...
    async def create_quarterly_review(
        ...
        for principal in principals:
            principal_items = await self._create_review_items_for_principal(principal, created_by)
        ...
        logger.info(f"Created quarterly access review {review_id} with {len(review_items)} items")
    ...
    async def _create_review_items_for_principal(
        ...
        async with async_session_factory() as db:
            ...
            for edge in edges:
                ...
                risk_level = self._calculate_permission_risk(edge.permission, edge.provider)
                
                # Generate business justification prompt
                justification = await self._generate_justification_prompt(edge, principal)
    ...
    def _calculate_permission_risk(self, permission: str, provider: str) -> str:
        ...
        high_risk_patterns = [
            "admin", "owner", "root", "superuser", "full_access",
            "delete", "create", "modify", "write", "*"
        ]
    ...
    async def record_review_decision(
        ...
        decision_record = {
            ...
            "exception_expiry": exception_expiry.isoformat() if exception_expiry else None
        }
