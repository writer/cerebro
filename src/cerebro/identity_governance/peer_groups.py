...
class PeerGroupAnalyzer:
    ...
    def _normalize_department(self, department: str) -> DepartmentType:
        ...
        elif any(term in dept_lower for term in ["ops", "operations", "infrastructure"]):
            return DepartmentType.OPERATIONS
        elif any(term in dept_lower for term in ["security", "infosec", "cybersecurity"]):
        ...
        elif any(term in dept_lower for term in ["support", "customer success", "help"]):
    ...
    def _normalize_role(self, role: str) -> str:
        ...
        if "engineer" in role_lower or "developer" in role_lower:
            return "engineer"
        elif "manager" in role_lower or "director" in role_lower or "lead" in role_lower:
            return "manager"  
    ...
    async def _calculate_group_baseline(
        ...
        for principal in principals:
            principal_permissions = await self._get_principal_permissions(principal["principal_id"])
        ...
        elevated_permissions = [
            perm for perm in permission_frequency.keys()
            if any(term in perm.lower() for term in ["admin", "owner", "full", "delete", "create"])
        ]
        
        # Get typical resources (simplified)
        typical_resources = list(set([
        ...
        baseline = PeerGroupBaseline(
            ...
            admin_permission_rate=admin_count / total_principals if total_principals > 0 else 0,
    ...
    async def _get_principal_permissions(self, principal_id: str) -> List[str]:
        ...
        try:
            github_result = await self.query_engine.execute_query(f"""
            ...
            for repo in github_result.rows:
                ...
                if isinstance(perms, dict):
                    for perm_type, has_perm in perms.items():
                        if has_perm:
                            permissions.append(f"github:{perm_type}:{repo['repository']}")
        ...
        try:
            aws_result = await self.query_engine.execute_query(f"""
    ...
    async def _analyze_principal_vs_peers(
        ...
        for perm, frequency in baseline.permission_frequency.items():
            if frequency > 0.8 and perm not in principal_permissions:  # 80%+ of peers have it
    ...
    def _calculate_outlier_risk_score(
        ...
        outlier_count_score = min(len(outlier_permissions) / 10, 1.0)  # Max at 10 outliers
        
        # Severity bonus for high-severity outliers
        high_severity_count = len([p for p in outlier_permissions if p["severity"] == "high"])
        severity_score = min(high_severity_count / 5, 1.0)  # Max at 5 high-severity
        
        # Admin permission bonus
        admin_outliers = [
            p for p in outlier_permissions 
            if any(term in p["permission"].lower() for term in ["admin", "owner", "root"])
        ]
        admin_score = min(len(admin_outliers) / 3, 1.0)  # Max at 3 admin outliers
        
        # Combine scores (weighted)
        risk_score = (
            outlier_count_score * 0.4 +
            severity_score * 0.4 +
            admin_score * 0.2
        )
    ...
    def _generate_outlier_recommendations(
        ...
        if outlier_permissions:
            high_severity_outliers = [p for p in outlier_permissions if p["severity"] == "high"]
    ...
    async def generate_peer_group_report(self, org_id: str) -> Dict[str, Any]:
        ...
        for group_key, baseline in baselines.items():
            ...
            dept_summary["admin_permission_rate"] = max(
                dept_summary["admin_permission_rate"], 
                baseline.admin_permission_rate
            )
