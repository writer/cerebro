...
class ExceptionManager:
    ...
    async def request_access_exception(
        ...
        exception = AccessException(
            ...
            metadata={
                ...
                "risk_assessment": self._assess_exception_risk(exception_type, permission, provider),
                "approval_required": self._requires_approval(exception_type, permission)
            }
        )
    ...
    async def revoke_exception(
        ...
        if exception.status not in [ExceptionStatus.ACTIVE, ExceptionStatus.APPROVED]:
            raise ValueError(f"Exception {exception_id} cannot be revoked (status: {exception.status})")
    ...
    async def process_expired_exceptions(self, org_id: str) -> Dict[str, Any]:
        ...
        for exception in expired_exceptions:
            try:
                ...
                elif exception.revalidation_due and exception.revalidation_due <= current_time:
                ...
                logger.error(f"Failed to process expired exception {exception.exception_id}: {e}")
    ...
    def _assess_exception_risk(self, exception_type: ExceptionType, permission: str, provider: str) -> str:
        """Assess risk level of exception request."""
        # High-risk permissions
        if any(term in permission.lower() for term in ["admin", "owner", "delete", "full"]):
            return "high"
        ...
        if exception_type in [ExceptionType.CROSS_TRAINING, ExceptionType.TEMPORARY_PROJECT]:
            return "medium"
        
        return "low"
    
    def _requires_approval(self, exception_type: ExceptionType, permission: str) -> bool:
        ...
        if exception_type in [ExceptionType.CONTRACTOR_ACCESS, ExceptionType.COMPLIANCE_EXCEPTION]:
            return True
    ...
    async def _get_expired_exceptions(self, org_id: str, current_time: datetime) -> List[AccessException]:
