...
class JMLCampaignManager:
    ...
    async def detect_jml_events(
        ...
        identity_changes = await self._detect_identity_changes(org_id, start_date, end_date)
        ...
        for change in identity_changes:
            ...
            if event_type:
                ...
                affected_access = await self._get_affected_access(change["principal_id"])
                
                # Calculate review deadline based on event type
                review_deadline = self._calculate_review_deadline(event_type, change["change_date"])
                ...
                if user.get("password_last_used"):
                    ...
                    if last_used >= start_date:
                        changes.append({
                            ...
                            "previous_attributes": {},
                            "new_attributes": {"password_last_used": user["password_last_used"]},
                            "source": "iam_api",
    ...
    async def _get_affected_access(self, principal_id: str) -> List[str]:
        ...
        async with async_session_factory() as db:
            ...
            for edge in edges:
                access_description = f"{edge.provider}:{edge.permission}@{edge.resource_id}"
    ...
    async def _analyze_access_staleness(
        ...
        # Role changes may make role-specific access stale
            ...
            if old_role and old_role != new_role:
                ...
                if any(keyword in access["permission"].lower() for keyword in role_keywords):
                    risk_score = 0.7
    ...
    async def create_jml_campaign(
        ...
        campaign = {
            ...
            "summary": {
                ...
                "high_risk_items": len([item for item in stale_access if item.risk_score >= 0.8]),
                "immediate_action_required": len([
                    item for item in stale_access 
    ...
    async def get_campaign_recommendations(self, campaign_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        for principal_id, access_items in campaign_data["stale_access_by_principal"].items():
            immediate_actions = [
                item for item in access_items 
                if item["recommended_action"].startswith("Revoke") or item["risk_score"] >= 0.9
            ]
        ...
        for principal_id, access_items in campaign_data["stale_access_by_principal"].items():
            review_actions = [
                item for item in access_items
                if item["recommended_action"].startswith("Review") and item["risk_score"] >= 0.5
            ]
...
async def detect_jml_events_for_org(org_id: str, lookback_days: int = 7) -> List[JMLEvent]:
