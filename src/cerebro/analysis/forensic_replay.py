...
class ForensicReplayEngine:
    ...
    async def reconstruct_state_at_time(
        ...
        active_findings = await self._reconstruct_active_findings(
            org_id, target_time
        )
        ...
        logger.info(f"Reconstructed state: {len(historical_principals)} principals, "
    ...
    async def _reconstruct_resources(
        ...
        for resource in all_resources:
            ...
            config_stmt = select(ConfigSnapshot).where(
                and_(
            ...
            access_stmt = select(IamEdge, Principal).join(Principal).where(
                and_(
    ...
    def _generate_security_summary(
        ...
        admin_principals = len([p for p in principals if any("admin" in perm["permission"] for perm in p.permissions)])
        
        # Resource analysis
        total_resources = len(resources)
        high_risk_resources = len([r for r in resources if r.security_posture["overall_score"] < 0.3])
        public_resources = len([r for r in resources if "public" in str(r.security_posture.get("issues", []))])
        
        # Finding analysis
        active_findings = [f for f in findings if f["would_be_active"]]
        critical_findings = len([f for f in active_findings if f["severity"] == "critical"])
        ...
        return {
            "principals": {
                ...
                "admin_percentage": round((admin_principals / max(total_principals, 1)) * 100, 1)
            },
            "resources": {
                ...
                "avg_security_score": round(
                    sum(r.security_posture["overall_score"] for r in resources) / max(total_resources, 1), 2
                )
            },
            "findings": {
                ...
                "by_severity": {
                    severity: len([f for f in active_findings if f["severity"] == severity])
    ...
    async def investigate_incident(
        ...
        logger.info(f"Investigating incident at {incident_time}, looking back {lookback_hours} hours")
    ...
    async def _find_changes_in_period(
        ...
        config_stmt = select(ConfigSnapshot, Resource).join(Resource).join(Resource.account).where(
            and_(
                Resource.account.has(org_id=org_id),
        ...
        if focus_resource:
            config_stmt = config_stmt.where(ConfigSnapshot.resource_id == focus_resource)
        ...
        permission_stmt = select(IamEdge, Principal, Resource).join(Principal).outerjoin(Resource).where(
            and_(
        ...
        # Audit events
        audit_stmt = select(AuditEvent).join(AuditEvent.account).where(
            and_(
    ...
    def _analyze_incident_changes(
        ...
        baseline_critical = len([f for f in baseline_state.active_findings if f["severity"] == "critical"])
        incident_critical = len([f for f in incident_state.active_findings if f["severity"] == "critical"])
        ...
        if changes["permission_changes"]:
            ...
            if admin_grants:
                analysis["potential_causes"].append("Admin privileges granted during incident window")
        
        if changes["configuration_changes"]:
            public_changes = [c for c in changes["configuration_changes"] if "s3" in c["resource_type"]]
            if public_changes:
    ...
    async def compare_states(
        ...
        return {
            ...
            "changes": {
                ...
                "findings_net_change": len(state2.active_findings) - len(state1.active_findings),
