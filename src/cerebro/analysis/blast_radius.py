...
class CompromiseScenario:
    ...
    scenario_type: str = (
        "credential_theft"  # credential_theft, privilege_escalation, insider_threat
    )
...
class BlastRadiusAnalyzer:
    ...
    async def analyze_principal_compromise(
        ...
        logger.info(
            f"Analyzing blast radius for {principal.external_id} at {compromise_time}"
        )
        ...
        business_impact = self._calculate_business_impact(
            all_impacted, escalation_paths
        )

        # 5. Generate mitigation recommendations
        recommendations = self._generate_mitigations(
            scenario, all_impacted, escalation_paths
        )
        ...
        logger.info(
            f"Blast radius analysis complete: {total_resources} resources at risk, "
    ...
    async def _find_directly_accessible_resources(
        ...
        stmt = (
            select(IamEdge, Resource)
            .join(Resource)
            .where(
                and_(
                    IamEdge.principal_id == principal_id,
                    IamEdge.effective_at <= at_time,
                    or_(IamEdge.expires_at.is_(None), IamEdge.expires_at > at_time),
                )
            )
        )
        ...
        for iam_edge, resource in edges_and_resources:
            ...
            config = await self._get_latest_config_at_time(
                resource.resource_id, at_time
            )

            sensitivity_score = self._calculate_sensitivity_score(resource, config)
            access_level = self._determine_access_level(
                iam_edge.permission, iam_edge.is_admin
            )
            potential_actions = self._determine_potential_actions(
                resource, iam_edge, config
            )
                ...
                escalation_paths.append(
                    {
                        ...
                        "escalation_score": (
                            0.8 if "admin" in resource.name.lower() else 0.6
                        ),
    ...
    async def _find_cross_provider_impact(
        ...
        stmt = (
            select(IdentityClusterMember, IdentityCluster)
            .join(IdentityCluster)
            .where(IdentityClusterMember.principal_id == principal_id)
        )
    ...
    def _determine_access_level(self, permission: str, is_admin: bool) -> str:
        ...
        if is_admin or "admin" in permission.lower():
            ...
            verb in permission.lower() for verb in ["write", "put", "create", "delete"]
        ):
            return "write"
        else:
            return "read"
    ...
    def _determine_potential_actions(
        ...
        if resource.resource_type == "aws.s3.bucket":
            ...
            if "admin" in iam_edge.permission or iam_edge.is_admin:
                actions.extend(
                    ["modify_settings", "add_collaborators", "disable_protections"]
                )

        elif resource.resource_type == "aws.ec2.instance":
            if iam_edge.is_admin or "ec2:RunInstances" in iam_edge.permission:
                actions.extend(
                    ["launch_instances", "lateral_movement", "cryptocurrency_mining"]
                )
    ...
    async def _analyze_role_permissions(
        ...
        try:
            ...
            if not accessible_list:
                policy_based_resources = await self._analyze_role_policies(
                    role_resource
                )
                        ...
                        for statement in policy_doc["Statement"]:
                            if statement.get("Effect") == "Allow":
                                ...
                                for resource_arn in resources:
                                    matching_resources = (
                                        await self._find_resources_by_arn_pattern(
                                            resource_arn
                                        )
                                    )
    ...
    def _calculate_business_impact(
        ...
        for resource in impacted_resources:
            access_multiplier = {"read": 0.3, "write": 0.7, "admin": 1.0}[
                resource.access_level
            ]
            total_impact += resource.sensitivity_score * access_multiplier

        # Add escalation path impact
        escalation_impact = sum(
            path.get("escalation_score", 0) for path in escalation_paths
        )
    ...
    def _generate_mitigations(
        ...
        if admin_resources:
            mitigations.append(
                f"Review admin access to {len(admin_resources)} high-privilege resources"
            )

        public_resources = [
            r for r in impacted_resources if "public" in str(r.potential_actions)
        ]
        if public_resources:
            mitigations.append(
                "Enable additional monitoring for public-facing resources"
            )
        ...
        if escalation_paths:
            ...
            mitigations.append(
                "Enable CloudTrail/audit logging for privilege escalation detection"
            )

        # Cross-provider mitigations
        cross_provider_resources = [
            r for r in impacted_resources if "identity_cluster" in r.access_path[0]
        ]
        if cross_provider_resources:
            mitigations.append(
                "Coordinate incident response across all linked provider accounts"
            )
    ...
    async def batch_analyze_high_risk_principals(
        ...
        for row in high_risk_principals:
            try:
                assessment = await self.analyze_principal_compromise(
    ...
    async def generate_blast_radius_report(
        ...
        high_impact_count = len(
            [a for a in assessments if a.business_impact_score > 0.7]
        )
        avg_resources_at_risk = sum(
            a.total_resources_at_risk for a in assessments
        ) / max(total_principals_analyzed, 1)
        ...
        report = {
            ...
            "summary": {
                ...
                "max_impact_score": max(
                    [a.business_impact_score for a in assessments], default=0.0
                ),
            },
            "top_risk_principals": [
                {
                    ...
                    "top_actions": [
                        action
                        for resource in a.directly_accessible[:3]
        ...
        logger.info(
            f"Generated blast radius report for {org.name}: "
    ...
    def _analyze_cross_provider_risks(
        ...
        for assessment in assessments:
            cross_resources = [
                r for r in assessment.cross_provider_impact if r.access_path
            ]
            if cross_resources:
        ...
        return {
            ...
            "risk_level": (
                "high" if cross_provider_count > len(assessments) * 0.3 else "medium"
            ),
        }
