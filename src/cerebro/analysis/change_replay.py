...
class ChangeReplayEngine:
    ...
    async def replay_rule_historically(
        ...
        for snapshot, resource in snapshots:
            try:
                ...
                # Evaluate rule
                result = self.rule_engine.evaluate_rule(rule_id, rule.expression, context)
                evaluations += 1
                ...
                logger.warning(f"Rule evaluation failed for snapshot {snapshot.snapshot_id}: {e}")
        ...
        logger.info(f"Rule replay complete: {matches}/{evaluations} matches, "
    ...
    async def replay_all_rules_for_quarter(
        ...
        stmt = select(Rule).join(Rule.policy).where(
            and_(
                Rule.policy.has(org_id=org_id),
                Rule.is_active == True
            )
        )
        ...
        logger.info(f"Replaying {len(rule_list)} rules for Q{quarter_start.month//3 + 1} {quarter_start.year}")
        ...
        summary = {
            ...
            "top_finding_producers": sorted(
                replay_results, 
                key=lambda r: r.new_findings_count, 
                reverse=True
            )[:10]
        }
    ...
    async def _get_historical_snapshots(
        ...
        stmt = select(ConfigSnapshot, Resource).join(Resource).join(Resource.account).where(
            and_(
    ...
    async def what_if_rule_analysis(
        ...
        temp_rule_id = UUID('00000000-0000-0000-0000-000000000001')  # Deterministic temp ID
        ...
        for snapshot, resource in snapshots:
            try:
                ...
                result = self.rule_engine.evaluate_rule(temp_rule_id, rule_expression, context)
                evaluations += 1
        ...
        return {
            ...
            "findings_by_resource_type": self._group_findings_by_resource_type(findings),
            "temporal_pattern": self._analyze_temporal_pattern(findings),
            "sample_findings": findings[:10]  # First 10 for review
        }
    
    def _extract_relevant_config(self, config: Dict[str, Any], rule_expression: str) -> Dict[str, Any]:
    ...
    def _group_findings_by_provider(self, findings: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    def _group_findings_by_resource_type(self, findings: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    def _analyze_temporal_pattern(self, findings: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def generate_rule_effectiveness_report(
        ...
        for rule in rules:
            ...
            rule_effectiveness.append({
                ...
                "match_rate": (result.matches_found / max(result.total_evaluations, 1)) * 100,
                "performance": result.performance_metrics,
                "recommendation": self._get_rule_recommendation(effectiveness_score, result)
            })
        ...
        return {
            ...
            "summary": {
                ...
                "high_effectiveness_rules": len([r for r in rule_effectiveness if r["effectiveness_score"] > 0.7]),
                "low_effectiveness_rules": len([r for r in rule_effectiveness if r["effectiveness_score"] < 0.3]),
                "avg_effectiveness": sum(r["effectiveness_score"] for r in rule_effectiveness) / max(len(rule_effectiveness), 1)
            }
        }
    ...
    def _get_rule_recommendation(self, effectiveness_score: float, result: RuleReplayResult) -> str:
