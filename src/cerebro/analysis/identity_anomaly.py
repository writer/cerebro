...
class IdentityAnomalyDetector:
    ...
    async def analyze_identity_anomalies(
        ...
        try:
            ...
            permission_anomalies = await self._detect_permission_anomalies(behavioral_data)
            ...
            resource_anomalies = await self._detect_resource_access_anomalies(behavioral_data)
            ...
            cross_provider_anomalies = await self._detect_cross_provider_anomalies(behavioral_data)
    ...
    async def _establish_baselines(self, org_id: str, principal_id: Optional[str] = None):
    ...
    async def _calculate_baseline(self, db: AsyncSession, principal: Principal) -> BehavioralBaseline:
        """Calculate behavioral baseline for a specific principal."""
        end_date = datetime.now()
        start_date = end_date - timedelta(days=self.lookback_days)
        
        # Query login patterns using SQL engine
        login_data = await self._get_login_patterns(principal.principal_id, start_date, end_date)
        
        # Query permission patterns
        permission_data = await self._get_permission_patterns(db, principal, start_date, end_date)
        
        # Query resource access patterns
        resource_data = await self._get_resource_access_patterns(db, principal, start_date, end_date)
    ...
    async def _get_login_patterns(
        ...
        try:
            ...
            result = await self.query_engine.execute_query(f"""
    ...
    async def _collect_behavioral_data(
        ...
        for pid, baseline in self.baselines.items():
            ...
            feature_vector = {
                ...
                'login_hour_spread': max(baseline.typical_login_hours) - min(baseline.typical_login_hours) if baseline.typical_login_hours else 0,
                'weekend_activity': sum(1 for hour in baseline.typical_login_hours if hour in [0, 1, 2, 3, 4, 5, 6, 23]),
                
                # Permission patterns
                'unique_permissions': len(baseline.permission_levels),
                'max_permission_level': max(baseline.permission_levels.values()) if baseline.permission_levels else 0,
                'admin_permissions': sum(1 for level in baseline.permission_levels.values() if level >= 3),
                
                # Resource access patterns
                'unique_resources': len(baseline.resource_access_frequency),
                'high_frequency_access': sum(1 for freq in baseline.resource_access_frequency.values() if freq > 0.8),
                'resource_diversity': len(set(res.split('_')[0] for res in baseline.resource_access_frequency.keys())),
    ...
    async def _detect_login_anomalies(self, data: pd.DataFrame) -> List[AnomalyResult]:
        ...
        login_features = ['typical_hours_count', 'login_hour_spread', 'weekend_activity']
    ...
    async def _detect_permission_anomalies(self, data: pd.DataFrame) -> List[AnomalyResult]:
    ...
    async def _detect_access_time_anomalies(self, data: pd.DataFrame) -> List[AnomalyResult]:
    ...
    async def _detect_resource_access_anomalies(self, data: pd.DataFrame) -> List[AnomalyResult]:
    ...
    async def _detect_velocity_anomalies(self, data: pd.DataFrame) -> List[AnomalyResult]:
        """Detect rapid successive action patterns."""
        # This would require time-series analysis of audit events
        # Simplified implementation for now
        return []
    
    async def _detect_cross_provider_anomalies(self, data: pd.DataFrame) -> List[AnomalyResult]:
        """Detect cross-provider correlation anomalies."""
        anomalies = []
        
        # Group by principal and check for multi-provider unusual patterns
        multi_provider_users = data.groupby('principal_id').agg({
            'provider': 'nunique',
            'admin_permissions': 'sum',
            'unique_permissions': 'sum'
        }).reset_index()
        
        multi_provider_users = multi_provider_users[multi_provider_users['provider'] > 1]
        ...
        for _, row in multi_provider_users.iterrows():
            ...
            if row['admin_permissions'] > 1:  # Admin in multiple providers
                anomaly = AnomalyResult(
                    ...
                    detected_at=datetime.now(),
                    baseline_period=(datetime.now() - timedelta(days=self.lookback_days), datetime.now()),
                    affected_resources=[],
                    recommended_actions=[
    ...
    def _extract_typical_hours(self, login_data: List[Dict[str, Any]]) -> List[int]:
        ...
        for login in login_data:
            if login.get('last_login'):
                try:
                    dt = datetime.fromisoformat(str(login['last_login']).replace('Z', '+00:00'))
                    hours.append(dt.hour)
                except Exception:
                    continue
        return list(set(hours))
    
    def _extract_access_patterns(self, login_data: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    def _extract_permission_levels(self, permission_data: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    def _extract_resource_frequencies(self, resource_data: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    async def get_anomaly_summary(self, org_id: str) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'by_type': {},
            'top_principals': {},
            'summary_period': (datetime.now() - timedelta(days=self.lookback_days), datetime.now())
        }
        ...
        for anomaly in anomalies:
            ...
            if pid not in summary['top_principals']:
                summary['top_principals'][pid] = {'count': 0, 'max_risk': anomaly.risk_level.value}
            summary['top_principals'][pid]['count'] += 1
            
            if RiskLevel(summary['top_principals'][pid]['max_risk']).value < anomaly.risk_level.value:
                summary['top_principals'][pid]['max_risk'] = anomaly.risk_level.value
        
        # Sort top principals by count and risk
        summary['top_principals'] = dict(sorted(
...
async def get_identity_anomaly_summary(org_id: str, lookback_days: int = 30) -> Dict[str, Any]:
