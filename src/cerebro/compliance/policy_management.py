...
class PolicyDocument:
    ...
    target_audiences: List[str] = field(default_factory=list)  # all_employees, contractors, admins
...
class PolicyManagementSystem:
    ...
    def approve_policy(
        ...
        approved_roles = {
            appr["role"] for appr in policy.approvals 
    ...
    def get_published_policies(
        ...
        if audience:
            policies = [
                p for p in policies 
                if audience in p.target_audiences or "all_employees" in p.target_audiences
            ]
        
        return sorted(policies, key=lambda p: p.published_at or datetime.min, reverse=True)
    ...
    def get_attestation_compliance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "pending": pending,
                "overdue": overdue,
                "compliance_rate": (acknowledged / total_attestations * 100) if total_attestations > 0 else 0
            },
            "policy_breakdown": {
                pid: {
                    **stats,
                    "compliance_rate": (stats["acknowledged"] / stats["total"] * 100) if stats["total"] > 0 else 0
                }
