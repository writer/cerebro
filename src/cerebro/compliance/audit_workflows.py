...
class AuditWorkflowManager:
    ...
    def create_request_list(
        ...
        if not name:
            name = f"{framework_name.upper()} {request_type.value} - {period_start.year}"
    ...
    def review_task(
        ...
        comment = f"Task {'approved' if approved else 'rejected'} by {reviewer or 'reviewer'}"
        if reviewer_notes:
    ...
    def get_request_list_status(self, request_list_id: str) -> Optional[Dict[str, Any]]:
        ...
        for task_id in request_list.task_ids:
            if task_id in self._tasks:
                ...
                if task.due_date and task.due_date < datetime.now() and task.status not in [
    ...
    def get_auditor_portal_data(
        ...
        for exception in self._exceptions.values():
            if (exception.framework_name == request_list.framework_name and
                exception.request_list_id == request_list_id):
    ...
    def _update_request_list_progress(self, request_list_id: str):
        ...
        if request_list.total_tasks > 0:
            request_list.progress_percentage = int((completed / request_list.total_tasks) * 100)
...
def create_soc2_type2_request_list(
    ...
    request_list.opinion_date = period_end + timedelta(days=45)  # Standard 45-day opinion
