...
class EvidenceRecord(Base):
    ...
    )  # Requirement IDs this evidence supports
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "source_system", "source_id", "observed_at", name="uq_evidence_source_time"
        ),
    )
...
class EvidenceSchema(Base):
    ...
    normalization_rules = Column(
        JSONType, default=dict
    )  # How to transform raw to normalized

    created_at = Column(DateTime(timezone=True), default=datetime.now)
    created_by = Column(String(255))

    __table_args__ = (UniqueConstraint("entity_type", "source_system", "version"),)
...
class RequirementMapping(Base):
    ...
    # Cross-mapping
    equivalent_requirements = Column(
        JSONType, default=list
    )  # Other framework reqs that map
...
class EvidenceDataFabric:
    ...
    def ingest_evidence(
        ...
        with self.session_factory() as session:
            ...
            existing = (
                ...
                .filter_by(entity_id=entity_id, content_hash=content_hash)
                .first()
            )
    ...
    def cross_evidence_analysis(
        ...
        query = EvidenceQuery(
            entity_ids=[entity_id], requirements=requirements, include_derived=True
        )
    ...
    def _analyze_access_patterns(
        ...
        return {
            ...
            "records_analyzed": len(evidence_records),
        }
