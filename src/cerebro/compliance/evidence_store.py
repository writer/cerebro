...
class EvidenceStore:
    ...
    async def store_evidence(
        ...
        if self._chain_tip:
            ...
            metadata.chain_hash = hashlib.sha256(f"{metadata.id}:{content_hash}".encode()).hexdigest()
    ...
    async def get_evidence(self, evidence_id: str) -> Optional[tuple[bytes, EvidenceMetadata]]:
        ...
        blob_path = self.blobs_path / f"{metadata.content_hash[:2]}" / f"{metadata.content_hash}.blob"
    ...
    async def _find_by_content_hash(self, content_hash: str) -> Optional[EvidenceMetadata]:
...
def create_evidence_store(storage_path: str, signing_key_path: Optional[str] = None) -> EvidenceStore:
