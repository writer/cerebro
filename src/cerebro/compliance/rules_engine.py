...
class RuleResult:
    ...
    def pass_rate(self) -> float:
        if self.evidence_count == 0:
            return 1.0
        return max(0.0, (self.evidence_count - self.violation_count) / self.evidence_count)
...
class PolicyStatementParser:
    ...
    TERM_MAPPINGS = {
        "multi-factor authentication": {"field": "mfa_enabled", "operator": "equals", "value": True},
        "two-factor authentication": {"field": "mfa_enabled", "operator": "equals", "value": True},
        "2fa": {"field": "mfa_enabled", "operator": "equals", "value": True},
        "strong password": {"field": "password_policy.min_length", "operator": "greater_than", "value": 8},
        "encrypted": {"field": "encryption_enabled", "operator": "equals", "value": True},
        "backup": {"field": "backup_enabled", "operator": "equals", "value": True},
        "monitoring": {"field": "monitoring_enabled", "operator": "equals", "value": True},
        "access review": {"field": "last_access_review", "operator": "within_days", "value": 90}
    }
            ...
            for pattern in self.SHALL_PATTERNS:
                ...
                for match in matches:
                    statement = {
                        ...
                        "suggested_rules": self._suggest_rules_for_statement(match.strip())
                    }
        ...
        for term, mapping in self.TERM_MAPPINGS.items():
            if term in statement_lower:
                rule_suggestion = {
                    ...
                    "rule_type": "existence" if mapping["operator"] == "equals" else "threshold",
                    "conditions": [{
...
class NoCodeRulesEngine:
    ...
    def create_rule_from_template(
        ...
        for cond_template in template["conditions"]:
            condition = RuleCondition(
                ...
                value=parameters.get(cond_template["value_param"], cond_template.get("default_value")),
        ...
        rule = RuleDefinition(
            ...
            severity=RuleSeverity(parameters.get("severity", template.get("default_severity", "medium"))),
            conditions=conditions,
            requirements=requirements or [],
            remediation_guidance=template.get("remediation_guidance", "").format(**parameters)
        )
        ...
        for i, statement in enumerate(statements):
            if statement["suggested_rules"]:
                ...
                rule = self.policy_parser.generate_rule_from_statement(
                    ...
                    rule_name=rule_name,
                    requirements=requirements,
                    entity_types=[EvidenceEntityType.IDENTITY, EvidenceEntityType.CONFIGURATION]
                )
    ...
    def evaluate_rule(
        ...
        if rule.evidence_filters:
            if "entity_types" in rule.evidence_filters:
                query.entity_types = [
                    EvidenceEntityType(et) for et in rule.evidence_filters["entity_types"]
                ]
    ...
    def evaluate_all_rules(
        ...
        for rule in self._rules.values():
            ...
            if requirements and not any(req in rule.requirements for req in requirements):
                continue
    ...
    def get_no_code_builder_config(self) -> Dict[str, Any]:
        ...
        return {
            "field_paths": [
                {"path": "normalized_data.mfa_enabled", "type": "boolean", "label": "MFA Enabled"},
                {"path": "normalized_data.last_login", "type": "datetime", "label": "Last Login"},
                {"path": "normalized_data.permissions", "type": "array", "label": "Permissions"},
                {"path": "normalized_data.password_policy.min_length", "type": "number", "label": "Min Password Length"},
    ...
    def _evaluate_conditions(
        ...
        for record in evidence_records:
            ...
            elif rule.logic_operator == "OR" and len(record_violations) == len(rule.conditions):
                violations.extend(record_violations)
