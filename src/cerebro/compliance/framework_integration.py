...
class FrameworkIntegration:
    ...
    def create_control_tests_for_framework(self, framework_name: str) -> List[ControlTest]:
    ...
    def get_framework_coverage(self, framework_name: str) -> Dict[str, any]:
        ...
        semi_automated = len([c for c in framework.controls if c.automation_level == "semi-automated"])
        ...
        controls_with_rules = len([
            test for test in control_tests if test.rule_ids
        ])
        ...
        framework_rules = len([
            ...
            if framework_name in rule.framework_mappings
        ])
        ...
        return {
            ...
            "coverage_percentage": (controls_with_tests / total_controls) * 100 if total_controls > 0 else 0,
            ...
            "rule_utilization": (len(used_rules) / framework_rules) * 100 if framework_rules > 0 else 0
        }
    ...
    def get_control_gaps(self, framework_name: str) -> List[Dict[str, any]]:
        ...
        for control in framework.controls:
            if control.control_id not in tested_control_ids:
                ...
                test = next(t for t in control_tests if t.control_id == control.control_id)
                if not test.rule_ids:
                    gaps.append({
                        ...
                        "suggested_actions": ["Create CEL rule for automated evaluation"]
                    })
    ...
    def suggest_new_rules_for_framework(self, framework_name: str) -> List[Dict[str, any]]:
        ...
        for control in framework.controls:
            if control.control_id not in covered_controls:
                suggestion = self._analyze_control_for_rule_creation(control, framework_name)
                if suggestion:
    ...
    async def validate_control_test_effectiveness(
        ...
        return {
            ...
            "category_breakdown": {
                cat: {
                    **data,
                    "pass_rate": (data["passing"] / data["total"]) * 100 if data["total"] > 0 else 0
                }
    ...
    def _create_control_test(
        ...
        # Create test ID
        test_id = f"{framework_name}_{control.control_id}".replace(".", "_").replace(" ", "_")
    ...
    def _suggest_providers_for_control(self, control: ComplianceControl) -> List[str]:
        ...
        if any(word in keywords for word in ["okta", "authentication", "sso", "identity"]):
    ...
    def _generate_effectiveness_recommendations(
        ...
        if pass_rate < 50:
            recommendations.append("Overall pass rate is low - review control implementations and remediation processes")
        elif pass_rate < 80:
            recommendations.append("Consider strengthening security controls - pass rate could be improved")
        
        if error_rate > 20:
            recommendations.append("High error rate indicates test reliability issues - review test configurations")
        
        # Category-specific recommendations
        for category, data in category_analysis.items():
            cat_pass_rate = (data["passing"] / data["total"]) * 100 if data["total"] > 0 else 0
            cat_error_rate = (data["errors"] / data["total"]) * 100 if data["total"] > 0 else 0
            
            if cat_pass_rate < 60:
                recommendations.append(f"{category} controls need attention - low pass rate ({cat_pass_rate:.1f}%)")
            
            if cat_error_rate > 25:
                recommendations.append(f"{category} control tests have reliability issues - high error rate")
