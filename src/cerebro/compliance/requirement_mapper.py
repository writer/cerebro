...
class RequirementMapping:
    ...
    applicable_scopes: List[str] = field(default_factory=list)  # subsidiary, region, etc.
...
class RequirementMappingService:
    ...
    def get_requirement_mappings(
        ...
        for mapping in self._mappings.values():
            if (mapping.source_requirement.framework_name == source_framework and
                mapping.target_requirement.framework_name == target_framework and
                self._confidence_level(mapping.confidence) >= self._confidence_level(confidence_threshold)):
    ...
    def find_equivalent_requirements(
        ...
        for mapping in self._mappings.values():
            source_matches = (
                mapping.source_requirement.framework_name == framework_name and
                mapping.source_requirement.requirement_id == requirement_id
            )
            
            if source_matches:
                if target_frameworks is None or mapping.target_requirement.framework_name in target_frameworks:
        ...
        for source_framework, source_reqs in reqs_by_framework.items():
            for target_framework, target_reqs in reqs_by_framework.items():
                if source_framework != target_framework:
                    mappings = self.get_requirement_mappings(source_framework, target_framework)
                    
                    for mapping in mappings:
                        if (mapping.source_requirement.requirement_id in source_reqs and
                            mapping.target_requirement.requirement_id in target_reqs):
    ...
    def validate_requirement_coverage(
        ...
        for req_id in requirements:
            ...
            coverage_percentage = (satisfied_weight / total_weight) if total_weight > 0 else 0.0
            ...
            coverage_results[req_id] = {
                ...
                "satisfied_evidence_types": sum(1 for e in evidence_details if e["satisfied"])
            }
    ...
    def generate_cross_framework_report(
        ...
        for target_framework in target_frameworks:
            framework_mappings = self.get_requirement_mappings(base_framework, target_framework)
            
            mapped_reqs = {m.source_requirement.requirement_id for m in framework_mappings}
            unmapped_reqs = [
                req.requirement_id for req in base_requirements
            ...
            target_reqs = [
                req.requirement_id for req in self._requirements.values()
        ...
        return {
            ...
            "overall_stats": {
                "avg_mapping_percentage": sum(
                    ms["mapping_percentage"] for ms in mapping_summary.values()
                ) / len(mapping_summary) if mapping_summary else 0,
    ...
    def _load_framework_requirements(self):
        ...
        soc2_requirements = [
            RequirementDefinition(
                ...
                evidence_types=["user_provisioning_logs", "access_management_config", "identity_provider_config"],
                sufficiency_criteria={"min_evidence_items": 3, "time_coverage": "full_period"},
            ...
            RequirementDefinition(
                ...
                control_family="Access Control",
                control_type="preventive",
                evidence_types=["access_change_logs", "approval_workflows", "access_review_reports"],
            ...
            RequirementDefinition(
                ...
                control_type="preventive",
                evidence_types=["mfa_configuration", "authentication_logs", "user_mfa_status"],
        ...
        iso_requirements = [
            RequirementDefinition(
                ...
                evidence_types=["user_registration_procedures", "access_provisioning_logs", "hr_integration_config"],
                sufficiency_criteria={"procedural_documentation": True, "implementation_evidence": True},
            ...
            RequirementDefinition(
                ...
                evidence_types=["password_policy_config", "authentication_system_config", "mfa_enforcement"],
                sufficiency_criteria={"technical_controls": True, "policy_enforcement": True}
            )
        ]
    ...
    def _generate_cross_mappings(self):
        ...
        self._mappings["soc2_cc611_iso_a9211"] = RequirementMapping(
            ...
            differences=["ISO requires more procedural documentation", "SOC2 focuses on technical controls"]
        )
    ...
    def _count_matching_evidence(
        ...
        if query_filter:
            ...
            if "entity_types" in query_filter:
                query.entity_types = [EvidenceEntityType(et) for et in query_filter["entity_types"]]
...
# Factory function
def create_requirement_mapper(evidence_fabric: EvidenceDataFabric) -> RequirementMappingService:
