...
def create_pagination_schema() -> Type[BaseModel]:
    ...
    class PaginationSchema(BaseModel):
        ...
        page_cursor: Optional[str] = Field(
            default=None,
            description=PAGE_CURSOR_DESCRIPTION
        )
...
def create_id_with_pagination_schema(param_name: str, description: str) -> Type[BaseModel]:
    ...
    if custom_fields:
        for field_name, field_config in custom_fields.items():
            if isinstance(field_config, dict):
                ...
                setattr(FilterSchema, field_name, Field(field_default, description=field_description))
...
def create_compliance_request_schema() -> Type[BaseModel]:
    ...
    class ComplianceRequestSchema(BaseModel):
        ...
        period_start: Optional[datetime] = Field(None, description=PERIOD_START_DESCRIPTION)
        period_end: Optional[datetime] = Field(None, description=PERIOD_END_DESCRIPTION)
        control_ids: Optional[List[str]] = Field(None, description="Specific controls to assess")
        include_evidence: bool = Field(False, description=INCLUDE_EVIDENCE_DESCRIPTION)
        include_remediation: bool = Field(True, description=INCLUDE_REMEDIATION_DESCRIPTION)
