...
class ISO27001FrameworkProvider(FrameworkProvider):
        ...
        return FrameworkDefinition(
            ...
            control_families={
                "A.5 Information Security Policies": [
                    c.control_id for c in controls if c.control_id.startswith("A.5")
                ],
                "A.6 Organization of Information Security": [
                    c.control_id for c in controls if c.control_id.startswith("A.6")
                ],
                "A.7 Human Resource Security": [
                    c.control_id for c in controls if c.control_id.startswith("A.7")
                ],
                "A.8 Asset Management": [
                    c.control_id for c in controls if c.control_id.startswith("A.8")
                ],
                "A.9 Access Control": [
                    c.control_id for c in controls if c.control_id.startswith("A.9")
                ],
                "A.10 Cryptography": [
                    c.control_id for c in controls if c.control_id.startswith("A.10")
                ],
                "A.11 Physical and Environmental Security": [
                    c.control_id for c in controls if c.control_id.startswith("A.11")
                ],
                "A.12 Operations Security": [
                    c.control_id for c in controls if c.control_id.startswith("A.12")
                ],
                "A.13 Communications Security": [
                    c.control_id for c in controls if c.control_id.startswith("A.13")
                ],
                "A.14 System Acquisition, Development and Maintenance": [
                    c.control_id for c in controls if c.control_id.startswith("A.14")
                ],
                "A.15 Supplier Relationships": [
                    c.control_id for c in controls if c.control_id.startswith("A.15")
                ],
                "A.16 Information Security Incident Management": [
                    c.control_id for c in controls if c.control_id.startswith("A.16")
                ],
                "A.17 Information Security Aspects of BCM": [
                    c.control_id for c in controls if c.control_id.startswith("A.17")
                ],
                "A.18 Compliance": [
                    c.control_id for c in controls if c.control_id.startswith("A.18")
                ],
            },
    ...
    def _validate_user_lifecycle(self, evidence_data: Any) -> bool:
        ...
        try:
            ...
            inactive_users = [
                u for u in users if u.get("status") in ["inactive", "suspended"]
            ]
    ...
    def _validate_incident_management(self, evidence_data: Any) -> bool:
        ...
        try:
            ...
            return has_incident_tracking and (
                has_resolution_process or len(open_incidents) == 0
            )
