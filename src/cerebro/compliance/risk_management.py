...
class Risk:
    ...
    def risk_reduction_percentage(self) -> float:
        ...
        if self.inherent_risk_score == 0:
            return 0.0
        return (
            (self.inherent_risk_score - self.residual_risk_score)
...
class RiskManagementSystem:
    ...
    def add_vendor(
        ...
        vendor = Vendor(
            ...
            + timedelta(days=assessment_frequency[risk_tier]),
        )
    ...
    def get_risk_dashboard_data(self) -> dict[str, Any]:
        ...
        for treatment in self._treatments.values():
            ...
            if (
                ...
                and treatment.target_completion_date < datetime.now()
                ...
                overdue_treatments.append(
                    {
                        ...
                        "days_overdue": (
                            datetime.now() - treatment.target_completion_date
                        ).days,
                    }
                )
        ...
        for vendor in self._vendors.values():
            ...
            if (
                ...
                and vendor.next_assessment_due <= datetime.now() + timedelta(days=30)
            ):
                assessments_due.append(
                    {
                        ...
                        "days_until_due": (
                            vendor.next_assessment_due - datetime.now()
                        ).days,
                    }
                )
        ...
        return {
            ...
            "key_metrics": {
                "avg_risk_score": (
                    sum(r.residual_risk_score for r in self._risks.values())
                    / total_risks
                    if total_risks > 0
                    else 0
                ),
                "risks_with_treatments": len(
                    [
                        r
                        for r in self._risks.values()
                        if any(t.risk_id == r.id for t in self._treatments.values())
                    ]
                ),
                ...
                "vendor_assessment_coverage": (
                    len([v for v in self._vendors.values() if v.last_assessment_date])
                    / total_vendors
                    * 100
                    if total_vendors > 0
                    else 0
    ...
    def generate_risk_report(
                ...
                risk_treatments = [
                    {
                        ...
                        "status": t.status,
                        "completion_date": (
                            t.target_completion_date.isoformat()
                            if t.target_completion_date
                            else None
                        ),
                    }
                    for t in self._treatments.values()
                    if t.risk_id == risk.id
                ]
        ...
        return {
            ...
            "summary": {
                ...
                "avg_inherent_score": (
                    sum(r.inherent_risk_score for r in filtered_risks)
                    / len(filtered_risks)
                    if filtered_risks
                    else 0
                ),
                "avg_residual_score": (
                    sum(r.residual_risk_score for r in filtered_risks)
                    / len(filtered_risks)
                    if filtered_risks
                    else 0
                ),
                "avg_risk_reduction": (
                    sum(r.risk_reduction_percentage for r in filtered_risks)
                    / len(filtered_risks)
                    if filtered_risks
                    else 0
                ),
    ...
    def map_risks_to_controls(self, control_framework: str) -> dict[str, Any]:
        ...
        return {
            ...
            "mapping_coverage": (
                len(risk_control_mappings) / len(self._risks) * 100
                if self._risks
                else 0
            ),
        }
    ...
    def _calculate_vendor_risk_scores(
        ...
        overall_score = (
            security_score + privacy_score + operational_score + financial_score
        ) / 4
            ...
            "probability_levels": [
                {
                    "level": 1,
                    "label": "Rare",
                    "description": "<20% chance in next 12 months",
                },
                {
                    "level": 2,
                    "label": "Unlikely",
                    "description": "20-40% chance in next 12 months",
                },
                {
                    "level": 3,
                    "label": "Possible",
                    "description": "40-60% chance in next 12 months",
                },
                {
                    "level": 4,
                    "label": "Likely",
                    "description": "60-80% chance in next 12 months",
                },
                {
                    "level": 5,
                    "label": "Very Likely",
                    "description": ">80% chance in next 12 months",
                },
            ],
            "risk_levels": {
                "1-3": "Low Risk",
...
# Factory function
def create_risk_management_system(
