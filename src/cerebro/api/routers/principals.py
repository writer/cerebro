...
async def get_principal(principal_id: UUID, db: AsyncSession = Depends(get_db)):
    """Get principal by ID."""
    return await get_entity_by_id_or_404(
        db, Principal, principal_id, "Principal not found"
    )
    ...
    if active_only:
        ...
        stmt = stmt.where(
            and_(
                ...
                or_(IamEdge.expires_at.is_(None), IamEdge.expires_at > now),
            )
        )
