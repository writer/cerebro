...
class RestorationRequest(BaseModel):
    ...
    business_justification: str = Field(..., description="Business justification for restoration")
    mitigation_plan: List[str] = Field(..., description="List of mitigation steps taken")
...
class RestorationApproval(BaseModel):
    ...
    approval_conditions: List[str] = Field(..., description="Conditions for restoration")
...
async def list_oauth_apps(
    ...
    try:
        ...
        if risk_level:
            ...
            filtered_apps = [app for app in filtered_apps if app.risk_level == risk_enum]
...
async def detect_toxic_combinations(
    ...
    try:
        ...
        filtered_results = [r for r in toxic_results if r.toxicity_score >= min_toxicity_score]
        ...
        return {
            ...
            "auto_quarantine_eligible": len([r for r in filtered_results if r.auto_quarantine_eligible]),
            "results": [
                {
...
async def get_oauth_risk_report(
    ...
    try:
        ...
        quarantine_summary = await quarantine_manager.get_quarantine_summary(str(org_id))
            ...
            "apps": [
                {
                    ...
                    "sensitive_data_access": any(s.sensitive_data_access for s in app.scopes)
                }
