...
async def receive_forklift_event(
    ...
    if forklift_webhook_secret:
        ...
        if not verify_forklift_signature(
            body, x_forklift_signature, forklift_webhook_secret
        ):
        ...
        logger.warning(
            "FORKLIFT_WEBHOOK_SECRET not configured - skipping signature verification"
        )
    ...
    logger.info(
        f"Received Forklift event: {event_type}",
        extra={
...
async def handle_drift_detected(
    ...
    account_stmt = select(Account).where(
        Account.org_id == org.org_id, Account.provider == "github"
    )
    ...
    finding_stmt = select(Finding).where(
        Finding.org_id == org.org_id, Finding.fingerprint == fingerprint
    )
...
async def handle_plan_applied(
    ...
    finding_stmt = select(Finding).where(
        Finding.fingerprint.like(f"forklift-drift-{event.repository}%")
    )
    finding = await db.scalar(finding_stmt)
