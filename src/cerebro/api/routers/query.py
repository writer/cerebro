...
async def execute_sql_query(
    ...
    try:
        ...
        if result.errors:
            raise HTTPException(
                status_code=400,
                detail={"message": "Query execution failed", "errors": result.errors},
            )
        ...
        freshness_payload = {
            provider: {
                "last_synced_at": (
                    summary.last_synced_at.isoformat()
        ...
        warnings = [
            summary.warning
            for summary in provider_freshness.values()
            if summary.warning
        ]
...
async def get_table_schema(table_name: str) -> TableDetailResponse:
    ...
    try:
        ...
        if not table_info:
            raise HTTPException(
                status_code=404, detail={"message": f"Table '{table_name}' not found"}
            )
