...
class WORMEvidenceBundle:
    ...
    def add_sql_query_evidence(
        ...
        logger.info(
            f"Added SQL query evidence {item_id} to bundle {self.metadata.bundle_id}"
        )
    ...
    async def seal_bundle(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Evidence bundle {self.metadata.bundle_id} sealed with {len(self.evidence_items)} items"
        )
        ...
        with zipfile.ZipFile(bundle_path, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            for item in self.evidence_items:
                if item.item_type == "sql_query":
                    ...
                    zf.writestr(
                        f"evidence/{item.item_id}_config.json",
                    ...
                    if "content" in item.data:
                        ...
                        zf.writestr(
                            f"evidence/{item.file_path or item.item_id}", content_bytes
                        )
...
class EvidenceBundleManager:
    ...
    async def create_compliance_evidence_bundle(
        ...
        """
        bundle_id = (
            f"compliance_{framework}_{control_id}_{int(datetime.now().timestamp())}"
        )
        bundle = WORMEvidenceBundle(bundle_id, created_by, organization_id)
