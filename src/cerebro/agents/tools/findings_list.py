...
class ListFindingsInput(BaseModel):
    ...
    created_after: Optional[str] = Field(None, description="Filter findings created after this ISO timestamp")
    created_before: Optional[str] = Field(None, description="Filter findings created before this ISO timestamp")
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                if list_inputs.severity:
                    ...
                    filtered_severities = [s for s in list_inputs.severity if s in valid_severities]
                ...
                if list_inputs.status:
                    ...
                    filtered_statuses = [s for s in list_inputs.status if s in valid_statuses]
                ...
                if list_inputs.created_after:
                    try:
                        after_dt = datetime.fromisoformat(list_inputs.created_after.replace('Z', '+00:00'))
                ...
                if list_inputs.created_before:
                    try:
                        before_dt = datetime.fromisoformat(list_inputs.created_before.replace('Z', '+00:00'))
                ...
                if list_inputs.resource_type:
                    ...
                    conditions.append(Resource.resource_type == list_inputs.resource_type)
                ...
                paginated_query = query.limit(list_inputs.limit).offset(list_inputs.offset)
                ...
                if list_inputs.resource_type:
                    count_query = count_query.join(Resource, Finding.resource_id == Resource.resource_id)
                if conditions:
