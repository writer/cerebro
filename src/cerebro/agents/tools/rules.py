...
class CompileRuleInput(BaseModel):
    ...
    validate_only: bool = Field(
        default=True, description="Only validate compilation, don't store"
    )
...
class RulesTool(Tool):
    ...
    async def execute(self, inputs: BaseModel, context: AgentContext) -> ToolResult:
        ...
        try:
            ...
            logger.exception(
                "Rules tool execution failed", operation=operation, error=str(e)
            )
            return ToolResult(
                success=False,
    ...
    async def _compile_rule(
        ...
        try:
            ...
            if "undefined identifier" in error_msg.lower():
                ...
                suggestions.append(
                    "Available context: resource, config, principal, org_config"
                )
            elif "syntax error" in error_msg.lower():
                suggestions.append(
                    "Verify CEL syntax - use '&&' not 'and', '||' not 'or'"
                )
                suggestions.append("String literals require double quotes")
            elif "type error" in error_msg.lower():
                suggestions.append(
                    "Check data types - use string(), int(), float() for conversions"
                )
            ...
            for i, test_context in enumerate(inputs.test_contexts):
                ...
                try:
                    ...
                    test_result = {
                        ...
                        "result": None,
                        "expected": (
                            inputs.expected_results[i]
                            if inputs.expected_results
                            and i < len(inputs.expected_results)
                            else None
                        ),
