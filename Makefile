...
rust-coverage: ## Enforce the Rust workspace line-coverage floor with the pinned coverage tool.
	...
	$(CARGO) llvm-cov --workspace --all-features --locked --ignore-filename-regex '(^|/)(wasm_abi\.rs|crates/action-store/src/lib\.rs|crates/cerebro-platform/src/(generated/.*|main|oidc|slack_agent|append_log_consumer|cutover_command|parity_command)\.rs|crates/organizational-store/src/(lib|neo4j|postgres)\.rs|crates/source-runtime-next/src/http\.rs|tools/policycataloggen/src/main\.rs)$$' --fail-under-lines 90 --summary-only
