...
.PHONY: rust-workspace-policy rust-fmt-check rust-clippy rust-test rust-doc-check rust-deny rust-coverage rust-benchmark-smoke rust-wasm-check rust-wasm-manifest-generate rust-wasm-manifest-check rust-validator-properties rust-validator-fuzz-smoke rust-security-path-properties rust-security-path-fuzz-smoke graphagent-static-validator-generate graphagent-static-validator-check sourcecoverage-evaluator-generate sourcecoverage-evaluator-check panopticon-resource-extractor-generate panopticon-resource-extractor-check mitre-context-evaluator-generate mitre-context-evaluator-check sourceruntime-record-kernel-generate sourceruntime-record-kernel-check rust-source-kernel-evidence finding-rule-evaluator-generate finding-rule-evaluator-check security-path-evaluator-generate security-path-evaluator-check
...
ORGANIZATIONAL_BENCH_SAMPLE_MS ?= 500
ORGANIZATIONAL_BENCH_SAMPLES ?= 5
ifndef CARGO_TARGET_DIRECTORY
...
slack-agent-rust-eval: ## Run deterministic Rust Slack agent evaluation contracts.
	$(CARGO) test -p cerebro-platform slack_agent_eval --locked
...
mcp-tool-eval: ## Score the nine advertised task tools against the checked-in hillclimb baseline.
...
rust-coverage: ## Enforce the Rust workspace line-coverage floor with the pinned coverage tool.
	...
	# The migration planner has a separate explicit floor while its deletion and CLI adapters grow;
	...
	# The organizational store's Postgres page-publication and promotion-evidence drivers are
	# exercised by the rust-only e2e fail-closed promotion boundary; the crate keeps a focused
...
rust-organizational-platform-benchmark: ## Compare the Go and Rust organizational projection and in-memory admission paths.
	@mkdir -p tmp
...
rust-organizational-store-benchmark: ## Measure durable commits, recovery, traversal, and tenant concurrency against disposable stores.
	...
	@test -n "$(CEREBRO_TEST_NEO4J_URI)" || { echo "CEREBRO_TEST_NEO4J_URI is required" >&2; exit 2; }
	@test -n "$(CEREBRO_TEST_NEO4J_USERNAME)" || { echo "CEREBRO_TEST_NEO4J_USERNAME is required" >&2; exit 2; }
	@test -n "$(CEREBRO_TEST_NEO4J_PASSWORD)" || { echo "CEREBRO_TEST_NEO4J_PASSWORD is required" >&2; exit 2; }
	@mkdir -p tmp
	@$(CARGO) test --locked -p cerebro-organizational-store --test durable_benchmark --release -- --ignored --nocapture > tmp/organizational-store-durable-benchmark.txt 2>&1; status=$$?; cat tmp/organizational-store-durable-benchmark.txt; exit $$status

projection-parity-test: ## Compare legacy and Rust semantic projection facts over the same provider records.
	@go run ./cmd/projection-parity-adapter < testdata/projection-parity/box-content-assets.json | $(CARGO) run --locked --quiet -p cerebro-platform -- compare-projection | grep -q '"status":"match"'
...
finding-rule-evaluator-generate: ## Rebuild the embedded Rust finding-rule evaluator.
	CARGO="$(CARGO)" $(PYTHON) scripts/embedded_wasm.py generate finding-rule-evaluator

finding-rule-evaluator-check: rust-fmt-check rust-clippy rust-test ## Verify the embedded Rust finding-rule evaluator is current.
	CARGO="$(CARGO)" $(PYTHON) scripts/embedded_wasm.py check finding-rule-evaluator
...
sourceruntime-record-kernel-generate: ## Rebuild the embedded source record kernel.
	CARGO="$(CARGO)" $(PYTHON) scripts/embedded_wasm.py generate sourceruntime-record-kernel

sourceruntime-record-kernel-check: rust-fmt-check rust-clippy rust-test ## Verify the embedded source record kernel is current.
	CARGO="$(CARGO)" $(PYTHON) scripts/embedded_wasm.py check sourceruntime-record-kernel
...
web-route-bugbash: workspace-install ## Exercise every local web route and scoped parameter in Chromium.
	npm run bugbash:routes --workspace @writer/cerebro-web
...
rust-product-demo-check: workspace-install ## Validate the Rust product demo in Chromium and write a redacted receipt.
	npm run demo:rust:check --workspace @writer/cerebro-web
...
release-train-test: ## Validate release notes and release-train workflow contracts.
	...
	bash scripts/release/test_render_smoke_receipt.sh
	bash scripts/release/test_verify_smoke_receipt.sh
