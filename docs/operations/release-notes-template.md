...
Record exactly one evidence mode: `machine_verified_portable` for a signed receipt bound to the candidate runtime digest, `machine_verified_github_actions` for a legacy same-repository Ephemeral Cerebro run without that digest binding, or `operator_attested_external` for an explicitly allowlisted external origin. The latter does not machine-prove receipt success, candidate binding, digest binding, or immutability.
