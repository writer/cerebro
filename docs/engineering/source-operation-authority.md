...
- Required entrypoint-to-route and route-to-security-boundary call edges must remain present with their reviewed cardinality.
...
`make source-operation-authority-check` fails closed on unknown fields, unknown families, duplicate overrides, inventory drift, selector outcome or normalization drift, contextual declaration drift, build-context variation, duplicate declarations, required call-edge drift, direct-callsite drift, invalid pull/push ownership, missing implementation evidence, or event-contract drift. Review the catalog and runtime change, update the smallest exact authority override, selector policy, or implementation binding, and rerun the check. A required call-edge change also requires an explicit checker-contract review; refreshing a declaration digest alone does not accept a route around the reviewed authority path. Do not resolve a failure by assigning both Go and Rust to one operation.
