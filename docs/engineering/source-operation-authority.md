...
The checker imports and executes the compiled `sourceworker.RustAuthoritativeFamily` and `sourceworker.PreviewRustFamily` functions. It compares their results with the ledger for every pull and push catalog pair, every pull source with an empty family, every pull source with an unknown-family sentinel, and empty or unknown source sentinels. A selected result must also return the exact normalized family declared by policy. These semantic checks prove the current compiled selectors produce the declared outcomes for the bounded corpus.

The semantic policy is paired with reviewed contextual declaration bindings. The bindings cover both selectors; the transitive `TailscaleFamily` selector; runtime-plan validation, creation, durable pull dispatch, compatibility reads, worker output conversion, and durable `Put`, `PutRuntimes`, `Sync`, and `SyncWithLease` entrypoints; preview selection, execution, discovery, and the `Check`, `Discover`, and `Read` entrypoints; and the immediate credential-resolution, context-fence read, durable fence refresh, and fence-binding helpers used by those routes. Canonicalization includes each declaration, package, and complete import context while ignoring comments, formatting, and import order. Bound files may not use Go build constraints, and a bound symbol must be declared exactly once across every non-test file in its package directory, including mutually exclusive build-tagged files. This keeps Linux, Darwin, amd64, and arm64 releases on the same reviewed selector, entrypoint, routing, credential-boundary, and fence declarations. Any implementation change fails until its exact binding is deliberately refreshed; refreshing a binding cannot make a semantic policy mismatch pass.

The checker also parses repository non-test Go files, excluding its own self-observation package and dependency or build-output directories, and requires the exact closed direct-reference set. Durable routing may consult `RustAuthoritativeFamily` only from runtime-plan validation, runtime creation, durable read dispatch, worker output conversion, and `PreviewRustFamily`. Preview routing may consult `PreviewRustFamily` only from the source-operations adapter. Every allowed caller must also have a full declaration binding. The checker additionally requires direct structural call edges from production entrypoints through their dispatch, selector, output-conversion, immediate credential-resolution, and lease-fence declarations. Its bounded reachability rules exclude nested function literals; branches and short circuits disabled by evaluable local or package constants; switch clauses excluded by a guaranteed constant match unless reached by `fallthrough`; and statements skipped by an unconditional `return`, `break`, `continue`, or forward `goto`. Shadowed package qualifiers, shadowed same-package functions, and calls on a shadowed receiver also do not satisfy an edge. New direct callers and missing required edges fail the check.

The call-edge check is a structural defense paired with deliberate review of every declaration digest change. Its reachability analysis is limited to the syntax listed above; it does not prove arbitrary dead-code absence, runtime dataflow, infinite-loop behavior, or that a return value controls a later branch. It also does not defend against an intentionally malicious contract change that preserves a structurally reachable call solely to satisfy the checker and refreshes its reviewed digest. Reviewers must inspect the full bound declaration and the checker edge contract whenever either changes.
...
- Both selectors, transitive selector helpers, production entrypoints, route helpers, and immediate credential and lease-fence dependencies must match their reviewed contextual declaration bindings.
- Bound declarations must be build-context invariant, unique across package files, and bound to their package and import context.
- Required entrypoint-to-route and route-to-security-boundary direct structural call edges must remain reachable under the bounded AST rules above, unshadowed, and present with their reviewed cardinality.
...
`make source-operation-authority-check` fails closed on unknown fields, unknown families, duplicate overrides, inventory drift, selector outcome or normalization drift, contextual declaration drift, build-context variation, duplicate declarations, required structural call-edge drift, direct-callsite drift, invalid pull/push ownership, missing implementation evidence, or event-contract drift. Review the catalog and runtime change, update the smallest exact authority override, selector policy, or implementation binding, and rerun the check. A required call-edge change also requires an explicit checker-contract review; refreshing a declaration digest alone does not accept a missing edge, one hidden by the bounded dead-code forms above, or a shadowed edge. Do not resolve a failure by assigning both Go and Rust to one operation.
...
5. For deliberate Go routing changes, update the relevant durable or preview rule, contextual declaration binding, and closed callsite contract in the same reviewed change. Refresh only the digest printed by the checker after reviewing the full bound declaration and import context.
