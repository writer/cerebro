...
### LLM-authored Cypher is read-only, LIMIT-bounded, tenant-scoped, and APOC-allowlisted.

- The embedded Rust Cypher safety validator rejects every query that contains write or bulk-load tokens (`CREATE`, `MERGE`, `DELETE`, `REMOVE`, `SET`, `DROP`, `FOREACH`), `LOAD CSV`, or `USING PERIODIC`. APOC is fail closed against an exact APOC Core 5.26 catalog: classified deterministic value functions, bounded value-shaping procedures, path-shaping functions, node and relationship predicates, and five bounded traversal procedures are allowlisted; unknown names remain blocked. `apoc.path.expandConfig`, `apoc.path.subgraphNodes`, and the admitted `apoc.neighbors` procedures require literal `RELATION` filters, `Entity` label filtering, breadth-first node-global uniqueness, tenant predicates over every yielded node and relationship, and a maximum of six hops. Raw `apoc.algo` shortest-path procedures remain blocked; weighted paths are ranked from the bounded result set with native Cypher expressions. Trigger, scheduler, dynamic-Cypher, I/O, schema, mutation, unbounded traversal, metadata-scan, and sleep surfaces are not allowed. Every node pattern must carry the `Entity` label and inline `tenant_id: $tenant_id`, and every query requires a numeric `LIMIT` ≤ `MaxRows` (default 100). These constraints will not be loosened to support agent-authored data mutations or autonomous remediation through the graph.
...
- Enforced in: the Rust policy and tenant-scope parser under [`internal/graphagent/staticvalidator`](../../internal/graphagent/staticvalidator), the independent read guard in `internal/graphstore/neo4j`, the Rust `FactQuery` constructor in `crates/agent-context`, and the parameter-only query compiler in `crates/organizational-store`.
- What would change this: nothing for write tokens. A new APOC function or procedure requires a concrete read-query need, classification as deterministic and read-only, bounded resource behavior, native and embedded-Wasm tests, and matching graph-store enforcement. The bounded traversal exception exists for dependency, reachability, and attack-path reads that cannot be expressed as a fixed one-hop pattern; it does not permit arbitrary graph exploration. New agent capabilities must travel through typed Action and workflow event surfaces, not through Cypher.
