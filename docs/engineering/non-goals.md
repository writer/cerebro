...
- What would change this: nothing for write tokens. A new APOC function or procedure requires a concrete read-query need, classification as deterministic and read-only, bounded resource behavior, native and embedded-Wasm tests, and matching graph-store enforcement. New agent capabilities must travel through typed Action and workflow event surfaces, not through Cypher.
