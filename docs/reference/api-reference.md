...
`GET /.well-known/agent-card.json` publishes public-safe A2A discovery metadata, not resolved context. Authenticated A2A JSON-RPC calls use `POST /api/v1/a2a`; `SendMessage` returns discovery metadata by default, creates a durable readiness-metadata task for the compatibility `agent-evidence-packet` skill, or resolves and persists the canonical DecisionPacket for the `decision-packet` skill. `GetTask` and `ListTasks` read those tenant-scoped A2A work tasks from the platform job store. Streaming, cancellation, and push methods still return A2A-style unsupported-operation errors until backed by replayable runtime adapters.
