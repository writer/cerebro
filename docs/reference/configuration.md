...
Graph action support is catalog-driven. Add or review supported actions in `crates/action-catalog/action_catalog.yaml`, regenerate the Rust authority and Go compatibility registry with `make graph-action-generate`, and keep `make graph-action-check` green. New provider execution belongs in a Rust provider adapter. The retiring Go `internal/graphactions.ActionProvider` adapter remains a compatibility consumer until its routes are cut over. When onboarding a provider, add a focused provider test, an agent-platform eval case that exercises the same finding-scoped approval boundary, and dashboard labels that stay bounded to provider/action/status rather than tenant or resource identifiers.
