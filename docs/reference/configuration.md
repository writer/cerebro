...
Graph action support is catalog-driven. Add or review supported actions in `internal/graphactions/action_catalog.yaml`, regenerate the Go registry with `make graph-action-generate`, and keep `make graph-action-check` green. Non-Okta providers should implement an `internal/graphactions.ActionProvider` adapter so execution and reconciliation keep the same finding eligibility, target validation, idempotency, workflow-event, external-reference, and `cerebro.graph_action.recorded` metric safeguards. When onboarding a provider, add a focused provider test, an agent-platform eval case that exercises the same finding-scoped approval boundary outside Okta, and dashboard labels that stay bounded to provider/action/status rather than tenant or resource identifiers.
