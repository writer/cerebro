...
| `GRAPH_STORE_ALLOW_IN_MEMORY` | `getEnvBool` | `false` | `GraphStoreAllowInMemory` | `GRAPH_STORE_BACKEND=memory is restricted to tests and explicit local opt-in` |
...
| `GRAPH_STORE_DUAL_WRITE_RECONCILIATION_PATH` | `getEnv` | `""` | `GraphStoreDualWriteReconciliationPath` | `when a secondary backend is configured, dual-write mode must be valid and best-effort reconciliation must have a queue path` |
| `GRAPH_STORE_DUAL_WRITE_REPLAY_BATCH_SIZE` | `getEnvInt` | `0` | `GraphStoreDualWriteReplayBatchSize` | `-` |
| `GRAPH_STORE_DUAL_WRITE_REPLAY_ENABLED` | `getEnvBool` | `false` | `GraphStoreDualWriteReplayEnabled` | `-` |
| `GRAPH_STORE_DUAL_WRITE_REPLAY_INTERVAL` | `getEnvDuration` | `0` | `GraphStoreDualWriteReplayInterval` | `-` |
...
| `GRAPH_STORE_SECONDARY_NEPTUNE_POOL_DRAIN_TIMEOUT` | `getEnvDuration` | `0` | `GraphStoreSecondaryNeptunePoolDrainTimeout` | `-` |
| `GRAPH_STORE_SECONDARY_NEPTUNE_POOL_HEALTHCHECK_INTERVAL` | `getEnvDuration` | `0` | `GraphStoreSecondaryNeptunePoolHealthCheckInterval` | `-` |
| `GRAPH_STORE_SECONDARY_NEPTUNE_POOL_HEALTHCHECK_TIMEOUT` | `getEnvDuration` | `0` | `GraphStoreSecondaryNeptunePoolHealthCheckTimeout` | `-` |
| `GRAPH_STORE_SECONDARY_NEPTUNE_POOL_MAX_CLIENT_LIFETIME` | `getEnvDuration` | `0` | `GraphStoreSecondaryNeptunePoolMaxClientLifetime` | `-` |
| `GRAPH_STORE_SECONDARY_NEPTUNE_POOL_MAX_CLIENT_USES` | `getEnvInt` | `0` | `GraphStoreSecondaryNeptunePoolMaxClientUses` | `-` |
| `GRAPH_STORE_SECONDARY_NEPTUNE_POOL_SIZE` | `getEnvInt` | `0` | `GraphStoreSecondaryNeptunePoolSize` | `-` |
| `GRAPH_STORE_SECONDARY_NEPTUNE_REGION` | `getEnv` | `""` | `GraphStoreSecondaryNeptuneRegion` | `-` |
...
| `JOB_NATS_CONSUMER` | `getEnv` | `"job-worker"` | `JobNATSConsumer` | `-` |
| `JOB_NATS_STREAM` | `getEnv` | `"CEREBRO_JOBS"` | `JobNATSStream` | `-` |
| `JOB_NATS_SUBJECT` | `getEnv` | `"cerebro.jobs"` | `JobNATSSubject` | `-` |
| `JOB_POLL_WAIT` | `getEnvDuration` | `10 * time.Second` | `JobPollWait` | `-` |
| `JOB_QUEUE_URL` | `getEnv` | `""` | `JobQueueURL` | `-` |
| `JOB_REGION` | `getEnv` | `""` | `JobRegion` | `-` |
...
| `NATS_URLS` | `getEnv` | `"nats://127.0.0.1:4222"` | `NATSJetStreamURLs` | `when NATS_JETSTREAM_ENABLED=true, required values must be present and timing/retention values must be positive` |
